...
fn expands_default_and_named_slots_in_caller_scope() {
    ...
    let document = parse_document(
        ...
        <Component name="Host" state="caller: '调用方'">
          <Panel title="标题">
            <Text>{caller}</Text>
...
fn allows_empty_slot_projection() {
    ...
    let document = parse_document(
        // 自闭合调用不提供任何投影内容。
        r#"<Component name="Panel"><Container><Text>标题</Text><Slot /></Container></Component><Panel />"#,
    )
    // 空插槽调用必须解析成功。
...
#[test]
fn expands_pure_slot_component_inside_for() {
    // 解析由 For 局部 item 驱动的纯插槽行组件。
    let document = parse_document(
        // Row 没有 props/state，因此保持既有 For 内允许边界。
        r#"
        <Component name="Row"><Container><Slot /></Container></Component>
        <Column>
          <For {item} in {items}>
...
#[test]
fn forwards_slot_projection_through_nested_components() {
    // 解析 Inner 容器、Outer 转发模板与拥有调用方状态的 Host。
    let document = parse_document(
        // Outer 的 Slot 作为 Inner 的默认插槽子节点继续转发。
        r#"
        <Component name="Inner"><Container><Slot /></Container></Component>
        <Component name="Outer"><Inner><Slot /></Inner></Component>
        <Component name="Host" state="label: '嵌套内容'"><Outer><Text>{label}</Text></Outer></Component>
        <Host />
        "#,
    )
    // 合法嵌套转发文档必须解析成功。
...
fn rejects_invalid_slot_declarations() {
    ...
    let duplicate_default = parse_document(
        // 同一组件模板声明两个默认占位。
        r#"<Component name="Bad"><Container><Slot /><Slot /></Container></Component><Bad />"#,
    )
    // 重复默认占位必须在声明阶段失败。
    ...
    let dynamic_name = parse_document(
        // 插槽登记必须在编译期闭合。
        r#"<Component name="Bad"><Slot name={target} /></Component><Bad />"#,
    )
    // 动态插槽名必须失败。
...
fn rejects_unknown_or_dynamic_slot_targets() {
    ...
    let unknown = parse_document(
        // Panel 只声明默认插槽。
        r#"<Component name="Panel"><Container><Slot /></Container></Component><Panel><Text slot="footer">操作</Text></Panel>"#,
    )
    // 声明语法本身合法。
    ...
    let dynamic = parse_document(
        ...
        r#"<Component name="Panel"><Container><Slot name="footer" /></Container></Component><Panel><Text slot={target}>操作</Text></Panel>"#,
    )
    // 声明语法本身合法。
