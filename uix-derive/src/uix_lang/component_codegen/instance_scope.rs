...
// 引入父模块拥有的组件展开器。
use super::ComponentExpander;
// 引入 UIX 元素语法树。
use super::super::Element;

// 实现 Component 静态声明与动态实例作用域投影。
impl ComponentExpander {
    // 按调用位置与最近 For 路径准备可复用的组件状态作用域。
    pub(super) fn prepare_component_scope(
        // 可变借用当前文档展开状态。
        &mut self,
        // 接收被调用组件的声明名称。
        component_name: &str,
        ...
        let scope_ident = self.fresh_ident("component_scope", component_name);
        ...
        if inside_for {
            ...
            let base_scope_ident = self.fresh_ident("component_scope_base", component_name);
            // 静态基座必须在循环外只申请一次，避免出现序号随行顺序漂移。
            self.setup.push(quote! {
                // 为当前组件声明取得跨 reconcile 稳定的静态作用域基座。
                let #base_scope_ident = ::uix::ui::__private::uix_component_scope(
            ...
            self.push_setup(quote! {
                ...
                let #scope_ident = ::uix::ui::__private::uix_component_child_scope(
                    // 继承静态调用点、声明与捕获命名空间。
                    &#base_scope_ident,
                    // 继续保留组件调用声明维度。
                    #declaration_id,
