...
#[test]
fn nested_for_component_uses_complete_instance_path() {
    // 解析外层业务 key 与内层位置共同拥有的状态组件。
    let document = parse_document(
        // Leaf 同时使用当前内层项 prop、computed 与私有 state。
        r#"
        <Component name="Leaf" props="label: String" state="count: 0" computed="summary: label">
    ...
    // 嵌套动态组件文档必须解析成功。
    .expect("嵌套 For Component 应可解析");
    // 生成完整实例路径与逐迭代准备语句。
    let tokens = generate_document_view(&document)
        // keyed 与位置身份组合必须完成生成。
        .expect("嵌套 For Component 应生成成功")
        // 规范化令牌便于检查结构。
        .to_string();
    // 输出必须保留两层真实 Rust 循环。
...
#[test]
fn for_component_slot_projection_keeps_caller_scope() {
    ...
    let document = parse_document(
        ...
        <Component name="Panel" state="open: true"><Column><Slot /></Column></Component>
        <Column><For {item} in {items} key={item.id}><Panel><Leaf label={item.label} /></Panel></For></Column>
        "#,
    )
    // 插槽与调用方循环作用域必须解析成功。
    .expect("For Component Slot 应可解析");
    // 生成调用方投影与被调用方私有状态。
    let tokens = generate_document_view(&document)
        // 两种作用域必须在同一实际迭代中闭合。
        .expect("For Component Slot 应生成成功")
        // 规范化令牌便于检查边界。
        .to_string();
