...
fn generates_position_insets_without_resetting_other_edges() {
    ...
    let tokens = generate_document_view(&document)
        ...
        .to_string();
    // 顶边应生成显式像素调用。
    assert!(tokens.contains("top (10f32)"));
    // 右边应保留负浮点值。
    assert!(tokens.contains("right (- 2.5f32)"));
    // bottom auto 应只清除底边。
    assert!(tokens.contains("bottom_auto"));
    // 左边应生成显式像素调用。
    assert!(tokens.contains("left (4f32)"));
...
fn pseudo_position_inset_generates_single_edge_reset() {
    ...
    let tokens = generate_document_view(&document)
        ...
        .to_string();
    // 基础类保留左边像素声明。
    assert!(tokens.contains("left (12f32)"));
    // hover 分支只清除顶边。
    assert!(tokens.contains("top_auto"));
    // hover 不得清除未声明的左边。
...
// 验证百分比、无单位、calc 与非有限值都被明确拒绝。
#[test]
fn rejects_unsupported_position_inset_values() {
    // 覆盖首批契约之外的全部代表值。
    for source in ["10%", "10", "calc(10px + 2px)", "NaNpx"] {
        // 构造单一顶边值文档。
        let document = parse_document(&format!(
            // 保留原值供映射层精确诊断。
        ...
        // 诊断必须说明 auto 或有限 px 边界。
        assert!(error.message.contains("只支持 auto 或有限 px 长度"));
    }
}
