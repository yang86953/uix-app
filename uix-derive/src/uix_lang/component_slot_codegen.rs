...
use super::{AttributeValue, ComponentDeclaration, Diagnostic, Element, Node};
        ...
        for child in call.children.iter().filter(|node| is_renderable_node(node)) {
            ...
            if !projections.contains_key(&key) {
                ...
                return Err(Diagnostic::new(
                    ...
                    if key.is_empty() {
                        // 说明组件没有默认插槽。
                        format!("<{}> 未声明可接收子节点的默认 Slot", component.name)
                    } else {
                        // 说明具体具名插槽不存在。
                        format!("<{}> 未声明名为 {key} 的 Slot", component.name)
                    },
                    // 给出声明或改名修复动作。
    ...
    pub(super) fn expand_slot_placeholder(
        ...
        let Some(projections) = self.slot_projection_stack.last() else {
            ...
            return Err(Diagnostic::new(
                ...
                slot.span,
                // 说明缺少组件调用投影上下文。
                "<Slot> 只能出现在 Component 模板中",
                // 给出合法结构。
                "把 <Slot /> 移入顶层 <Component> 声明体",
            ));
        };
