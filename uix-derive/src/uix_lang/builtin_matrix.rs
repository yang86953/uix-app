...
pub(crate) fn planned_builtin_diagnostic(element: &Element) -> Option<Diagnostic> {
    ...
    Some(Diagnostic::new(
        ...
        // 给出当前真实支持集合。
        "当前使用 Text、Label、Button、ButtonGroup、FloatButton、Icon、Divider、Space、Typography、ThemeToggle、WindowControl、Container、Row、Column、Grid、ScrollView、VirtualScroll、Input、InputNumber、InputGroup、Slider、RangeSlider、Rate、Checkbox、Switch、Radio、Segmented 或 Select；Col 仅作为 Row/Grid 的直接子项，或先完成该组件的属性映射 Gate",
    ))
}
