...
// 验证 Breadcrumb 类型化条目、末项当前页与公共属性生成。
#[test]
// 声明完整 Breadcrumb 生成测试。
fn generates_breadcrumb_contract() {
    // 生成覆盖类型化数据和公共属性的面包屑。
    let snapshot = generate(
        r#"<Breadcrumb items={breadcrumb_items} width="480px" automationId="page-path" />"#,
    )
...
fn rejects_invalid_breadcrumb_shape_and_attributes() {
    ...
    // 运行时 separator 尚未登记为 UIX 文档属性。
    let separator = generate(r#"<Breadcrumb items={breadcrumb_items} separator=">" />"#)
        // 文档外属性必须失败。
        .expect_err("未知 Breadcrumb 属性必须被拒绝");
    // 诊断必须包含具体未知属性名。
    assert!(separator.message.contains("separator"));
}
