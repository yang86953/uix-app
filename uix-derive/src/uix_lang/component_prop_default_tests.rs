...
fn generates_literal_and_data_constructor_defaults() {
    // 构造包含字符串与日期默认值的组件。
    let source = r#"
        <Component name="Defaults" props="label: String = '确定', date: Date = Date(2026, 8, 15), required: bool">
...
fn explicit_prop_overrides_default_and_ast_tracks_optional() {
    ...
    let source = r#"
        <Component name="Greeting" props="label: String = '默认'"><Text>{label}</Text></Component>
        <Greeting label="显式" />
    "#;
    // 解析文档以检查声明事实。
    ...
    // 默认表达式必须保存在对应 prop 上。
    assert!(component.props[0].default.is_some());
    // 生成显式覆盖调用。
...
fn rejects_missing_required_prop() {
    ...
    let error = generate(
        // 构造一个可选和一个必填 prop。
        r#"<Component name="Mixed" props="label: String = '默认', required: bool"><Text>{label}</Text></Component><Mixed />"#,
    )
    // 提取预期必填诊断。
...
fn rejects_dynamic_state_and_callback_defaults() {
    ...
    let dynamic = parse_document(
        // 构造读取外部名称的默认值。
        r#"<Component name="Bad" props="label: String = externalValue"><Text>{label}</Text></Component><Bad />"#,
    )
    // 提取预期白名单诊断。
    ...
    let state = parse_document(
        // 构造非法 State 默认值。
        r#"<Component name="Bad" props="value: State<String> = 'x'"><Text>A</Text></Component><Bad />"#,
    )
    // 提取预期所有权诊断。
    ...
    let callback = parse_document(
        // 构造非法回调默认值。
        r#"<Component name="Bad" props="onClick: () = true"><Text>A</Text></Component><Bad />"#,
    )
    // 提取预期所有权诊断。
