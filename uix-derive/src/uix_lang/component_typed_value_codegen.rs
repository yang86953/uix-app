...
// 引入类型、表达式、对象字段与生成辅助。
use super::{
    ComponentValueType, Diagnostic, Expression, ExpressionKind, ObjectField, generate_expression,
...
impl ComponentExpander {
    ...
    pub(super) fn complex_typed_initial_tokens(
        ...
        let initial = match value_type {
            ...
            ComponentValueType::OptionalString => self.optional_string_tokens(expression)?,
            // 字符串集合按元素生成拥有型 String 后收集。
            ComponentValueType::HashSetOfString => {
                // 生成字符串数组元素。
                let items = self.array_items_tokens(expression, &ComponentValueType::String)?;
                // 收集为确定类型 HashSet。
                quote! {
            ...
            // 数值向量支持非空数组字面量。
            ComponentValueType::VecOfNumber => {
                // 生成 f64 项目。
                let items = self.array_items_tokens(expression, &ComponentValueType::Number)?;
                // 构造类型可推断向量。
                quote! { ::std::vec![#(#items),*] }
            }
            ...
            ComponentValueType::VecOfRecord(name) => {
                // 构造 record 元素目标类型。
                let item_type = ComponentValueType::Record(name.clone());
                // 生成全部 record 项目。
                let items = self.array_items_tokens(expression, &item_type)?;
                // 构造 record 向量。
                quote! { ::std::vec![#(#items),*] }
            }
            // 上传队列仍只允许空数组，文件由运行时选择流程产生。
            ComponentValueType::VecOfUploadFile => {
                // 要求数组形状。
                let ExpressionKind::Array(items) = &expression.kind else {
                    // 返回上传队列形状诊断。
                    return Err(array_shape_diagnostic(expression, "Vec<UploadFile>"));
                };
                // 非空上传文件不能由 UIX 字面量安全构造。
                if !items.is_empty() {
                    // 返回运行时所有权诊断。
                    return Err(Diagnostic::new(
                        ...
                        expression.span,
                        // 陈述上传文件来源边界。
                        "Vec<UploadFile> state 只接受空数组初始值",
                        // 给出运行时更新方式。
                        "使用 []，由 Upload 运行时选择流程写入文件",
                    ));
                }
                // 生成空上传队列。
    ...
    fn typed_value_tokens(
        ...
        match value_type {
            ...
            // record 字段递归物化嵌套对象。
            ComponentValueType::Record(_) => self.struct_literal_tokens(expression, value_type),
            // 其余复合类型递归使用统一复合生成器。
            ComponentValueType::OptionalString
            | ComponentValueType::HashSetOfString
            | ComponentValueType::VecOfString
            | ComponentValueType::VecOfNumber
            | ComponentValueType::VecOfRecord(_)
            | ComponentValueType::VecOfUploadFile
            | ComponentValueType::CascaderValue => self
                // 生成复合初始值并丢弃重复类型令牌。
            ...
            | ComponentValueType::Point => generate_expression(expression, None),
        }
    }
    ...
    fn struct_target_fields(
        ...
        // 接收目标结构类型。
        value_type: &ComponentValueType,
    ) -> Result<(TokenStream, Vec<(String, ComponentValueType)>), Diagnostic> {
        // 按结构类型返回公开类型路径和字段声明。
        match value_type {
            // CascaderValue 使用公开固定字段集合。
            ComponentValueType::CascaderValue => Ok((
                // 生成公开路径令牌。
...
fn validate_object_fields(
    ...
    fields: &[ObjectField],
    // 接收目标字段声明。
    target_fields: &[(String, ComponentValueType)],
) -> Result<(), Diagnostic> {
    // 拒绝所有未知字段。
    for field in fields {
        // 当前字段必须存在目标声明。
