...
fn generates_extended_typed_state_values() {
    ...
    let source = r#"
        ...
        <Component name="Typed" state="names: Vec<String> = ['甲', '乙'], scores: Vec<number> = [1, 2.5], addresses: Vec<Address> = [{ city: '上海' }, { city: '杭州' }], selected: Option<String> = Some('active'), profile: Profile = { name: '用户', address: { city: '北京' }, tags: ['核心', '稳定'], scores: [3, 4.5], addresses: [{ city: '深圳' }], alias: Some('owner') }">
...
fn rejects_unknown_record_vector_element_type() {
    ...
    let error = parse_document(
        ...
        r#"<Component name="Bad" state="items: Vec<Missing> = []"><Text>A</Text></Component><Bad />"#,
    )
    // 提取预期引用诊断。
...
fn rejects_invalid_some_and_record_vector_values() {
    ...
    let some = generate(
        // 构造数值 Some。
        r#"<Component name="Bad" state="selected: Option<String> = Some(1)"><Text>A</Text></Component><Bad />"#,
    )
    // 提取预期可空字符串诊断。
    ...
    let record = generate(
        // 构造字符串替代 record 对象。
        r#"<Record name="Item" fields="name: String" /><Component name="Bad" state="items: Vec<Item> = ['x']"><Text>A</Text></Component><Bad />"#,
    )
    // 提取预期结构形状诊断。
...
fn rejects_invalid_nested_record_fields() {
    ...
    let missing = generate(
        // 构造缺字段嵌套对象。
        r#"<Record name="Address" fields="city: String" /><Record name="Profile" fields="address: Address" /><Component name="Bad" state="profile: Profile = { address: {} }"><Text>A</Text></Component><Bad />"#,
    )
    // 提取预期缺字段诊断。
    ...
    let unknown = generate(
        // 构造额外 zip 字段。
        r#"<Record name="Address" fields="city: String" /><Record name="Profile" fields="address: Address" /><Component name="Bad" state="profile: Profile = { address: { city: 'x', zip: 'y' } }"><Text>A</Text></Component><Bad />"#,
    )
    // 提取预期未知字段诊断。
