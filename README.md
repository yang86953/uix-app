...
> **当前**：Windows P6 功能基线已闭合，`0.0.1` 范围内 110 项已完成工作树定向实现与行为验证，版本仍未发布；现有真窗数据来自 Windows / NVIDIA / 96 DPI 工作树，Linux / macOS 源码在树，移动端未做。→ [`产品`](docs/产品.md) · [`使用`](docs/使用.md) · [`架构`](docs/架构.md) · [`0.0.1 状态`](docs/进度/首发状态.md#当前状态) · [`变更记录`](CHANGELOG.md)
