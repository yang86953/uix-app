...
组件体系的定位是 **uix-app 对应 React 的框架角色，uix-widgets 对应 Ant Design 的可复用组件库角色**：框架提供通用组件机制，组件库基于框架公开能力提供可供多个应用复用的控件与设计系统。应用也可以直接用框架创建自己的组件和组件库，详见[框架与组件库定位](docs/产品/定位与原则.md#框架与组件库定位)。
