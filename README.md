...
UIX（/ˈjuːɪks/）是一个 Rust 原生跨平台应用框架：让同一套界面与应用逻辑运行在多个平台的原生窗口上。

> UIX 是专有闭源软件，仅供内部授权使用。

## 当前状态

- 当前正式版本为 `0.0.7`，已于 2026-09-04 通过私有 Gitea Release 发布；独立消费者使用私有 Cargo Registry 中带 checksum 的正式包，共同养护框架的第一方应用可按[交付与许可](docs/产品/交付与许可.md#第一方源码联调通道)固定私有 Git 完整提交。
...
正式包消费者的 `Cargo.lock` 必须保留 Registry 来源和 SHA-256 checksum；第一方源码联调不得跟随移动分支或提交本地 `path`，只允许固定完整 Git commit，详见[第一方源码联调通道](docs/产品/交付与许可.md#第一方源码联调通道)。
...
检查矩阵包含 Linux x64、Windows x64、Apple Silicon macOS 与 Intel macOS。Windows 可从非 Windows 宿主经冻结版本的 `cargo-xwin` 完成真实 PE/COFF 链接；macOS 的最终链接必须在持有合法 Apple SDK 的 macOS 宿主执行。macOS 仍不是当前交付平台，交叉检查通过也不替代真实窗口与 GPU 验收。完整前置条件、输出和失败语义见[交叉编译](docs/使用/入门/交叉编译.md)。
...
产物写入 `target/internal-release/uix-0.0.7-internal-linux-x64.tar.gz`。构建器会调用独立校验器核对精确载荷、规范路径、归档元数据与 SHA-256；候选包可生成不表示版本已正式发布，完整契约见[交付与许可](docs/产品/交付与许可.md)。
...
Linux x64 宿主也可使用冻结的 `cargo-xwin 0.23.1` 与 LLVM 交叉链接并装配同一候选包：

```bash
bash scripts/build_internal_windows_cross_release.sh
```

两个入口均生成并校验 `uix-0.0.7-internal-win-x64.zip` 的载荷、路径与摘要；这是交付物完整性操作，不属于项目测试。交叉链接能证明 PE/COFF 产物，不替代真实 Windows 主机上的 Vulkan、D3D11、窗口、输入与 Agent 运行验收，实际证据边界由 Gitea Release 持有。
...
| `uix-lang-lsp/` | 独立 UIX Lang 语言服务器（stdio LSP，二进制 `uix-lang-ls`） |
| `uix-derive/` | UIX 派生宏 |
| `tests/` | 仅允许外部消费者视角的 `*_public_api.rs` 公开 API 测试 |
| `scripts/` | 内部打包工具、Agent Hub/MCP 连接器、交叉编译与内部候选包校验 |
| `docs/` | 产品、架构、使用文档 |
