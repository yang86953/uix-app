...
- Scheme 主方案及已冻结的 P0 合同继续有效；本补充方案暂不实现、不进入当前实施排期，不改变主方案的依赖与验收条件。

### 文档：软件动态扩展目标与实施计划

- 将运行时软件动态扩展纳入产品目标，按小贝的 Scheme R7RS-small 方向设计扩展业务、命令、事件、服务与动态界面；补充能力注入、热替换、状态迁移、回收与失败契约。
- 新增[目标设计](docs/架构/app/extensions.md)与[实施计划](docs/架构/动态扩展实施计划.md)，明确先无界面软件功能、再动态 UI 与完整生命周期的交付顺序，并区分 `.uix` AOT / 开发期热重载。
- 本条只记录设计文档变更；没有实现扩展运行时、新增依赖或发布版本，不表示上述能力已经可用。
