...
> **AI**：改 API / 命名 / 争议时查本表 `#dN`；**勿通读**。系统行为 → [`架构.md`](架构.md) 任务表；进度 → [`进度.md`](进度.md)。
>
> **唯一决策台账**（#1–#180；新决策 **#181+**）。锚点 `#d{N}`。未 supersede 的裁决语义保留；正文已压成结论行。
...
| **渲染** | [#59](#d59)、[#82](#d82)、[#107](#d107)、[#122](#d122)、[#129](#d129)、[#162](#d162)–[#164](#d164)、[#169](#d169)、[#172](#d172) |
...
| **架构 / 可组合** | [#168](#d168)–[#169](#d169)、[#172](#d172) |
| **浮层** | [#96](#d96)–[#100](#d100) |
...
| <a id="d23"></a>23 | Ripple | **不做** → 已由 [#176](#d176) 取代（Button Material ripple） |
...
| <a id="d105"></a>105 | **核心理念（最高规则）** | **用最少资源，做最好效果** — Demand-Driven Zero Idle Work；**统领**六域依赖与一切设计；冲突以本规则为准；细则 [`AGENTS.md`](../AGENTS.md#核心理念最高规则-105) |
...
| <a id="d113"></a>113 | 零闲置豁免 | 无法框架托管的定时/轮询 → **`决策.md` #181+** 公开条目 + 测试证明；默认不豁免；当前无豁免见 [#158](#d158) |
...
| <a id="d154"></a>154 | 实现路线图 | 分阶段落地顺序；**#105 三态/Registry 优先**；详见 [implementation · 分阶段路线图](进度.md#分阶段路线图) |
...
| <a id="d157"></a>157 | P0 落地清单 | P0 阶段目标与验收摘要（历史 per-file 清单已归档）；详见 [implementation · 已落地阶段](进度.md#已落地阶段p0p5) |
| <a id="d158"></a>158 | 零闲置豁免台账 | 当前 **无豁免**；新增豁免必须追加公开条目（当前从 **#181+** 起），并写明触发源、wake 频率、允许工作范围、无法 register 的理由与测试边界 |
...
| <a id="d174"></a>174 | 布局子项两阶段快照契约 | `WidgetLayout::measure_children(frame, ids, tree)` 在每次父级重排前按当前约束生成 pass-local `Vec<LayoutChild>`；`WidgetLayout::layout_children(frame, &[LayoutChild], tree)` 从快照读取子项测量结果/metadata 并返回子 frame，禁止递归 `measure`。Arrange 可读取非测量 tree 状态；ScrollView 在动态后代尚未把展开高度传播到 `measure` 时可用上一轮 arranged height 推进 viewport 收敛，但不得把它写回 `LayoutChild.measured_size`。快照不跨 convergence pass 持久缓存，resize、内容变化与 expand/shrink 重排均重新测量；默认 preparation 只把 id 包装成零尺寸 `LayoutChild`，避免 exact-fill 容器无谓测量（[#105](#d105)）。`LayoutChild.measured_size` 忠实等于约束下的 `measure` 结果，不回退旧 frame。旧 `layout_children(frame, &[ComponentId], tree)` 签名直接替换，不保留兼容桥。详见 [架构 · 源码入口](架构.md#源码入口)。 |
| <a id="d175"></a>175 | 运行时全局主题命令 | 启动主题仍由 `App::theme(Theme)` builder 指定；运行中由任意存活窗口的 `AppHandle::set_theme(Theme) -> Result<()>` 替换 **App 全局**主题。连续未消费请求合并为最终值且只制造一次 event-loop wake；消费时主窗与全部存活副窗各派发一次 `ThemeChanged`，仅 palette 节点 Paint invalidate，禁止 Layout / reconcile 与调用方逐窗标脏。切换后新建窗口直接读取当前主题，不补发历史事件；关闭 handle 返回 `Errc::InvalidState`。与 OS 跟随同时启用时，显式主题保持到下一次真实 `ThemeChanged`，届时重新采用系统模式；全程无后台 poll。导航 → [架构 · 源码入口](架构.md#源码入口) · `src/app/` · `src/ui/theme/`。 |
| <a id="d176"></a>176 | Button Material ripple | **取代 [#23](#d23)**：Button（含 default/primary/ghost/danger）在 PointerDown / Enter·Space 于触点（键盘为中心）启动半透明圆形水波，约 320ms 扩至盖住按钮，松手后约 200ms 淡出；经 `WidgetAnimation` + ActiveWorkRegistry 按需续帧，扩满按住时不空转。**点击/聚焦不改变** fill/border/text（focused/pressed 预设无色变叠层）；唯一点击视觉反馈为 ripple。hover 仍可微调控件本体。**不做**内缩/整钮透明度叠层；**不做**全局焦点外框（见 [#177](#d177)）。逻辑在 `ui`，绘制走 `PaintContext` clip + `fill_circle`。 |
| <a id="d177"></a>177 | 取消全局焦点环 | `LayerTree` **不再**绘制外扩半透明圆角焦点环；聚焦态无视觉外框。Button 点击反馈仅保留 Material ripple（[#176](#d176)）；控件自身边框/填充态（如 Input focused 加粗边）不受影响。后续若需无障碍键盘焦点指示，另开决策，默认不恢复全局环。 |
| <a id="d178"></a>178 | 应用作者门面收口 | **目标**：框架使用者优雅书写，不承担门面债。**(1) 子节点同质化**：`column`/`row`/`grid` 接受 `IntoViewChildren` — 同质数组/`Vec`（`Item: View`），异质用元组（至 12 元）或 `views![…]`；禁止再要求手写 `.into()`。**(2) 点击默认路径**：`button(…).on_click(&state, \|s\| …)` 为默认 API，自动 `with_state_capture` 指纹（[#159](#d159)–[#160](#d160)）；无 State 用 `on_click_fn`（保守重绑）；`on_click_capture` 保留为兼容别名。**(3) 文本统一入口**：`label` 经 `IntoLabelContent` 接受 `&str`/`String` 或 `Fn() -> String`；`dynamic_label` 为别名；`State::map_text(\|v\| …)` 内部 clone 句柄，Counter 级示例只需一个 `count` 名字。**(4) 后续**：多 State / AppHandle 的语法层 capture 仍走 `semantic_handler!`；不引入重型 DSL 宏。导航 → [架构 · 术语与公开面](架构.md#术语与公开面) · `src/prelude.rs`。 |
| <a id="d179"></a>179 | 应用作者 API 演进方向 | **战略方向**（不取代 [#178](#d178)；#178 是门面收口的已落地切片，本条裁决后续优先级）。**(1) 优先**：继续优化 Rust 写法 / `prelude` 门面，让应用作者优雅使用、少承担复杂度（State clone 噪音、子节点拼装、label 统一、默认 `on_click` 等路径沿 #178 推进）。**(2) 现阶段不做**：不发明独立 UIX UI 描述语言，不主推 XML 类 DSL 作为作者主路径。**(3) 体验目标**：追求「像 XML 那样清晰的声明式体验」，**载体仍是 Rust**——语义像声明式，不是另起一门语言。**(4) 远期条件**：仅当出现设计器 / 热更新 / 非 Rust 作者等强信号时，才考虑**编译到现有 View 的子集 DSL**；禁止并行运行时或第二套组件模型。**(5) 对齐**：[#105](#d105) 与 [`AGENTS.md`](../AGENTS.md)——最少资源最好效果；门面债与平行 DSL 均视为浪费。**待落地清单** → [#180](#d180)。详见 [架构 · 术语与公开面](架构.md#术语与公开面)。 |
| <a id="d180"></a>180 | 应用作者门面待落地 | **性质**：[#179](#d179) 战略下的**可执行待落地台账**（对照 README Counter、`public-api`、#178 已落地门面、prelude/`combinators`、demo 典型写法后的摩擦点）。**不**在本条落地实现；落地时按条开切片并回写本表「已落地」。对齐 [#105](#d105)：减门面债，不另起 DSL。完整条目与优先级 → [架构 · 术语与公开面](架构.md#术语与公开面)。**摘要**：**(P0)** State 变更写法（`update` 优于 `set(get()+1)`）；多 State / `root` / Timer 的 clone 面；任意 View 的 `on_click` 与 Button 对齐；条件/列表 View 组合子。**(P1)** `on_click_fn` 命名；Style/主题 token 链式噪音；`column` 默认 `flex_grow(1)` 心智；View 缺 `child()` 与 widget 双路径；宏边界（`views!` / `semantic_handler!` / `component!`）；可运行示例 vs 编译探针分离。 |
