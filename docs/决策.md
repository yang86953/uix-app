...
| <a id="d162"></a>162 | 多图形 API 抽象与选型 | 多 API 经 `IGraphicsContext` + `GraphicsEngine`；identity 仅配置/诊断/factory（[#172](#d172)）。选型仅 `bootstrap_graphics_engine` probe（#163），失败回退 `SoftwareEngine`；组装轴见 #168/#169。禁止每帧 probe/热切换。状态 → [P6.7](进度.md#p67-图形后端架构) |
| <a id="d163"></a>163 | 可插拔图形后端 registry | `GraphicsBackend`=API 身份；`native/factory` 表驱动；`try_create_context` 单条目；probe 仅 bootstrap。不含热切换/`dlopen`。非法组合诊断+fallback（#172）。Backlog → [#169](#d169) |
| <a id="d164"></a>164 | 图形 API 源码目录 | `native/graphics/<api>/` = peer；`backends/<os>/` = OS 壳；上层禁止 `use native::graphics::*`。cfg 仅 `graphics/**/platform/`、`backends/`、`factory/` |
...
| <a id="d168"></a>168 | 可组合组件模型 | 正交能力经 trait/registry 受 caps 组合；渲染轴 `RasterMode` × `PresentMode` × `GraphicsBackend`（不承诺笛卡尔积，[#172](#d172)）。旧 `RenderPipelineProfile` 已拒。硬约束摘要 → [架构 · 硬约束](架构.md#本仓硬约束uix) · [#169](#d169) |
| <a id="d169"></a>169 | 可组合渲染轴落地 | 分派仅按 caps 合法轴组合；删 `RenderPipelineProfile`；Windows 先行 D3D11 GpuNative，Metal/D3D12 随后；`BackendKind` 与轴对齐、表驱动装配。状态 → [进度 · P6.8](进度.md#p68-可组合渲染轴) |
| <a id="d170"></a>170 | 平台可选窗口能力 Result-only | 可失败窗口能力一律 `Result`；`Errc::NotImplemented` 为能力缺失 canonical；无 void 兼容面 |
| <a id="d171"></a>171 | 六域直接依赖与 cfg 边界可执行化 | 依赖表与 cfg 边界以 [架构 · 硬约束](架构.md#架构硬约束) 为准；守卫忽略注释/Rustdoc，不得 blanket 放行整个 `native/graphics/**` |
| <a id="d172"></a>172 | 图形 identity 与合法组合边界 | API identity 仅配置/诊断/factory/registry；engine/pipeline 不散落 API 分支。合法组合由 caps+registry；失败继续候选，终态 SoftwareEngine |
...
| <a id="d180"></a>180 | 应用作者门面 | [#179](#d179) 台账。**已落地**：`State::update`；任意 View `on_click`/`on_click_fn`；`show` / `Option`；`TimerHandle::detach`；`with_cloned!`；`column_fit`；`StyleExt` 委托 `ViewNode`。**P1 裁决**：保留 `on_click_fn`（Rust 无法重载）；`on_click_capture` 仅兼容；宏分层 / 示例 vs 探针 → [架构 · 应用作者入口](架构.md#应用作者入口)。导航 → [术语与公开面](架构.md#术语与公开面) |
