...
> **AI 何时打开**：跟缺陷、风险或待决事项时。生命周期台账，不是单次测试报告；证据在来源报告。阻塞里程碑才链 [`进度.md`](进度.md)。
>
> 通用「问题」台账：产品缺陷、验证风险、待决取舍均可入账。本仓当前以 GUI/产品缺陷为主；跨测试轮次去重；单次报告保留完整环境与证据。
...
- 严重度按用户/项目影响判断，不按修复难度判断；禁止把单机或单后端结果外推为跨平台结论。
