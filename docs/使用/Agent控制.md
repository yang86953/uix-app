...
- 窗口坐标动作：`press_key`、`click_at`、`pointer_move`、`pointer_down`、`pointer_up`。后三者用于观察真实 hover / pressed 视觉状态；当前只表达左键，`pointer_down` 与匹配的 `pointer_up` 组成一次完整输入。
...
详见[架构](../架构/Agent控制.md#大模型控制协议)；缺口 → [`AGENT-R1`](../进度/平台与硬件.md#agent-附能力)。
