...
`uix!` 生成的根容器（如 `Column`）默认**透明**：框架的离屏纹理清除为透明黑，若根视图不声明背景，深色文字会落在透明底上不可见（窗口呈现全黑）。当前需要像仓库演示那样在 Rust 薄入口为根 `ViewNode` 显式提供主题布局背景：

```rust
// 根视图提供主题布局背景，避免透明底上文字不可见。
uix!("src/main.uix").bg(ColorValue::Neutral(NeutralRole::BgLayout))
```

改进方向（待跟踪）：由 `App` 组合根为根视图**默认应用主题布局背景**，使未声明背景的简单视图开箱可见；涉及框架默认行为变更，由 Vikunja 项目跟踪。
