...
>
> **状态声明**：公开 `uix!` 转换器已经实现，并支持当前登记的 View 子集；本教程仍是**目标设计**，因为完整示例依赖规划中的 `App`、`Input`、主题生成及数组初始值等能力。当前可编译入口见 [uix-lang 快速开始](uix-lang%20快速开始.md#当前可编译的第一个-view)，完整应用运行对照见[教程（Rust 版）](教程.md)。
...
`App` 根和应用启动仍为目标设计，以上片段当前不可交给 `uix!` 生成。当前公开宏返回 `ViewNode`，可编译的文件入口与应用组合方式见[uix-lang 快速开始](uix-lang%20快速开始.md#当前可编译的第一个-view)。
...
持久化、定时器、多窗口等**框架能力不属于界面描述**，继续由 Rust 组合根配置（uix-lang 只描述界面，符合「Rust 优先——没有第二门运行语言」）。当前 `uix!` 生成 `ViewNode`，不会生成 `App` builder，也不提供 `settings`、`on_start` 或 `run` 链式方法；应用生命周期与配置用法以[配置](配置.md)及 [Rust 使用文档](../使用.md)为准。把 `App` 根直接转换为可链式配置 builder 仍是目标设计。

## 7. 完整示例

以下完整示例汇总本教程的目标语法；由于包含 `App`、`Input`、主题生成和数组状态初始值，**当前不可直接通过 `uix!` 编译**。
