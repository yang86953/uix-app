...
>
> **状态声明**：公开 `uix!` 转换器已经实现，并支持当前登记的 View 子集；本教程仍是**目标设计**，因为完整示例依赖规划中的 `App` 应用入口与主题生成能力。当前可编译入口见 [uix-lang 快速开始](uix-lang%20快速开始.md#当前可编译的第一个-view)，完整应用运行对照见[教程（Rust 版）](教程.md)。
