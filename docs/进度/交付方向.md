...
- **wgpu 已做**：生产只有一个 `draw::Renderer`，GPU 模式只是选择 GPU 后端。LayerTree 在该后端绕过 CPU `CommandRecorder`，直接写入严格 Native GPU canvas；相邻同类 op 在 API 中立层合批，再由内部 `WgpuExecutor` 生成顶点并保持 painter order。主路径经帧间保留色缓冲绘制、present 全幅 blit（`FullOnly`）；`retained_framebuffer` 解锁绘制侧 partial dirty。全屏 overlay 可对保留色缓冲 GPU 快照/恢复（无 CPU readback）只绘浮层；多块脏区逐矩形 clear + clip 重绘。真实 Vulkan 首帧约 14.6 ms、稳定后采样帧约 4.7 ms；该数字是本机启动样本，不替代完整帧基准
...
- **已做**：
  ...
  - **GPU offscreen / Picture**：`NativeRasterCaps::wgpu_full()` 启用 `offscreen_targets`；`WgpuExecutor` 提供 RT create/bind/blit，Picture 可分配 GPU RT 并在 swapchain 目标上采样合成；`blit_offscreen_target` 已转发父画布 opacity
  ...
  - **GPU 可分离模糊**：共享 wgpu 两趟（水平→垂直）高斯模糊，大半径先 2× 降采样；`IGraphicsContext::blur_offscreen_target` / `RenderBackend::try_blur_offscreen` / `RenderTarget::try_blur_offscreen` / `scene::blur_picture_region`，`sigma = radius / 3` 与 CPU 一致；禁止 PixelUpload 冒充
...
- **已有数据**：当前工作树的 Windows 11 build 26200 / NVIDIA / 96 DPI release 真窗包含 `Auto → Vulkan`、强制 Vulkan、最小化 / 隐藏后的恢复，以及测试夹具强制耗尽 GPU probe 后的 Software 整体回退；报告标记为 `UNVERIFIED`。device lost 与 uncaptured wgpu error 已接入 typed 恢复 / 日志边界
- **数据缺口**：最低支持 build 26100 实机、144 / 192 DPI、真实 compositor occlusion、AMD、Intel、Wayland 和 macOS 暂无当前报告；Vulkan 报告 `present_occlusion=unsupported`，现有最小化 / 隐藏恢复数据不包含真实遮挡
- **组件视觉数据**：工作树中的 92 项组件矩阵包含 847 条证据行、329 张原图和 22 张联系表，报告标记为 `UNVERIFIED`
