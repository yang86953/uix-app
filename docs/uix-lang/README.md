...
UIX Lang 让界面保持可声明、可检查和可生成；Rust 仍是应用实现语言。公开 `uix!` 生成已登记 View 子集，`uix_app!` 把 `<App>` 文档组装成现有 App builder；Rust 组合根和应用 Systems 继续持有业务流程、领域状态、服务调用、导航与窗口生命周期。UIX Lang 的事件表达式和同步 action 只用于局部界面状态转换或调用显式传入的 Rust 回调，不承载业务决策。
