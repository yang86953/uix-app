...
图表 `BarChart` / `LineChart` / `PieChart` / `AreaChart` / `ScatterChart` / `FunnelChart` / `Treemap` / `Gauge` / `Heatmap` / `WaterfallChart` 已登记类型化静态数据与基础视觉配置；气泡、多系列、高级交互及其余图表继续以 [Rust 图表使用文档](../使用/图表.md)为边界，尚未登记时会在编译期拒绝。
