...
>
> **定位**：UIX Lang 面向 TSX 式的类型化组件编程体验，将界面表达与组件逻辑自然组合，服务 `uix-app` 的组件层和可复用组件库。不是仅给 Rust UI API 增加标签外壳；React 类比只适用于组件层，框架整体还承担网络、ORM 等完整应用开发能力，语言目标通过公开接口复用这些服务。目标及可调整的语法边界见[语言定位与语法演进](设计/语言边界.md#语言定位与语法演进)；新语法尚未实现，当前旧 UI 与可移植模块入口仍按各自规范运行。

## 两类入口与实施边界

**组件演进目标（2026-09-12，尚未实施）**：按 TSX 式的组件语言定位支持纯 `.uix` 与按需配套 `.rs`，同一份组件定义既能静态编译，也能运行期动态加载。发现现有语法设计不佳时可以替换、简化或删除，不将已实现写法视为永久约束。具体语法、绑定和入口统一方案尚待设计；下表说明当前实现，不表示所有 Widget 已支持动态加载。目标正文见[组件创作与双路径目标](设计/语言边界.md#01-组件创作与双路径目标)。
