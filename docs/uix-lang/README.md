...
>
> **定位**：UIX Lang 面向 TSX 式的类型化组件编程体验，将界面表达与组件逻辑自然组合，服务 `uix-app` 的 React 式框架和可复用组件库。不是仅给 Rust UI API 增加标签外壳。目标及可调整的语法边界见[语言定位与语法演进](设计/语言边界.md#语言定位与语法演进)；新语法尚未实现，当前旧 UI 与可移植模块入口仍按各自规范运行。
