...
**组件演进目标（2026-09-12，尚未实施）**：提供前端式的简便组件开发体验，支持纯 `.uix` 与按需配套 `.rs`，同一份组件定义既能静态编译，也能运行期动态加载。文件组织、绑定和入口统一方案尚待设计；下表说明当前实现，不表示所有 Widget 已支持动态加载。目标正文见[组件创作与双路径目标](设计/语言边界.md#01-组件创作与双路径目标)。
...
| 事实类型 | 唯一所有者 | 其他文档的责任 |
|---|---|---|
| 产品定位、组件创作与双路径目标、UI/Rust 职责与非目标 | [语言边界](设计/语言边界.md) | 规范和指南只引用，不重新定义 |
