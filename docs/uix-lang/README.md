...
> **定位**：UIX Lang 是 UIX 的应用开发语言，也是普通应用的推荐主入口——使用方以 `.uix` 声明界面、状态与应用业务流程，正式构建时转换为 Rust，框架负责可靠执行生成代码。当前已实现范围以下方状态声明和各规范的分项状态为准；产品边界见[应用开发语言目标](变更/应用开发语言目标.md)，详细架构、迁移与验收见[语言完善计划](变更/语言完善计划.md)。
...
| | [语言完善计划](变更/语言完善计划.md) | 共享前端、目标语法、渐进实施、风险门禁与完成标准 |

## 状态声明

当前发布基线仍以界面声明、组件状态和事件表达式为主；无参数同步 action 已支持兼容的单表达式主体和结构化 `do` 块，异步业务流程、类型化服务与开发期动态解析仍属于目标设计，不能把本页产品方向理解为已全部完成。

[语法](规范/语法.md)现已同时持有冻结的目标契约和现行可编译文法；其中标记为“目标”或“迁移中”的结构不是当前可复制能力。指南与组件规范中的“已实现”示例仍是当前编译入口的依据。
...
当前 UIX Lang 是 UIX Rust API 的文本投影，**转换由 uix-app 框架在编译时自动完成**：转换器按下列规则把 `.uix` 结构展开为 Rust 声明式代码，再交由 cargo 编译；不生成任何运行期解释逻辑。目标架构将在保持 AOT 契约的前提下引入共享 Typed IR，详见[编译契约](规范/编译契约.md)与[语言完善计划](变更/语言完善计划.md)。
...
| `actions="save: expression, submit: do { ... }"` | 在事件调用位静态内联并生成 Rust | 已实现（无参数同步 action；`do` 块支持局部值、条件与返回；仅事件位置） |
