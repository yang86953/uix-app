...
当前生成矩阵真实支持上述登记标签，以及只在 `Row` / `Grid` 直接子项位置有效的 `Col` 和文档内自定义 `Component` 展开。表达式面支持数组字面量（`['female', 'male']`）与十四类数据类型构造链（SelectOption / CascaderOption / TreeNode / TimelineItem / Step / DescriptionsItem / BreadcrumbItem / AnchorItem / Tab / MenuItem / Date / Time / Color / Point，含 `Step.status` 字符串语义值，见[标签语法](标签语法.md#13-字面量)）；`<Record>` 声明可定义类型化业务模型并由 `uix_items!` 生成模块级结构体供 Rust 侧回调引用，组件私有 state 支持语义类型（Date / Time / Color / Point / CascaderValue / HashSet&lt;String&gt; / Vec&lt;String&gt; / Option&lt;String&gt; / record 名）注解与对象字面量初始值（见[标签语法](标签语法.md#44-类型化业务模型record)）。展示组件除 `Avatar`、`Image`、`ImageGroup`、`List`、`Skeleton`、`Empty`、`ResultView`、`Tag`、`Card`、`Descriptions`、`Timeline`、`Calendar`、`Carousel`、`Tree`、`Table`、`QRCode`、`Watermark`、`RichText` 外的 PascalCase 标签、反馈组件除 `Alert`、`Modal`、`Drawer`、`Popover`、`Tooltip`、`FocusTrap` 与 `Spin` 外的 PascalCase 标签、导航组件除 `Menu`、`Steps`、`Pagination`、`Breadcrumb`、`Anchor`、`Tabs` 外的 PascalCase 标签，以及输入 / 布局文档中未登记的标签（如 `Upload`）均已登记为“规划中”：使用时会在宏展开期报告组件名、所属文档类别与修复建议，不会静默生成占位 View。
