...
>
> **定位**：UIX 是 Rust 原生跨平台应用框架；UIX Lang 是其中的 UI 描述语言，也是使用方编写界面的推荐方式。`.uix` 只声明界面结构、样式、展示数据、界面状态和局部交互；业务规则、领域状态、I/O、异步流程与应用生命周期由 Rust 实现，并通过 UIX 的应用运行时与平台契约跨平台复用。
...
| 补充方案（暂不实现） | [应用模块与动态执行](设计/应用模块与动态执行.md)：探索扩展 UIX 自身能力，保留 Scheme 动态扩展主线 |
