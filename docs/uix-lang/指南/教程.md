...
>
> **状态声明**：公开 `uix!` 转换器已经实现，并支持当前登记的 View 子集；`<App>` 应用入口与主题生成能力已由 `uix_app!` 实现。本教程完整示例使用已登记组件与属性，可直接通过 `uix_app!` 编译；仅包含未登记组件或属性的片段仍会被编译期拒绝。当前可编译入口见 [uix-lang 快速开始](快速开始.md#当前可编译的第一个-view)，Rust 写法对照见[教程（Rust 版）](../../使用/教程.md)。
...
持久化、定时器、多窗口等**框架能力不属于界面描述**，继续由 Rust 组合根配置（uix-lang 只描述界面，符合「Rust 优先——没有第二门运行语言」）。`uix!` 生成 `ViewNode`；`uix_app!` 把 `<App>` 根转换为现有 `App` builder，调用方可继续链式配置 `settings`、`on_start`、Agent 控制等扩展点，并显式决定何时调用 `run()`。应用生命周期与配置用法以[配置](../../使用/配置.md)及 [Rust 使用文档](../../使用.md)为准。
