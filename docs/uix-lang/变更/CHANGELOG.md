...
- `VirtualScroll` 的惰性行模板现可使用嵌套 `<For>`、引用当前行字段的数据源
  与字符串字面量比较；编译器在行工厂闭包内建立完整的实例路径、逐迭代
  准备语句和事件捕获克隆，跨物化窗口重建时仍按绝对索引保持身份稳定。
- `<For key={...}>` 的纯表达式子语言明确拒绝字符串字面量；稳定键需使用
  当前数据项的字符串字段。错误在生成前给出定向诊断，不再生成不可解析的
  闭包外转换器引用。
- 语言版本及 uix-derive、uix-lang-compiler、uix-lang-cli、uix-lang-lsp
  与门禁消费者随根 crate 对齐到 `0.0.7`。
...
- `Input` 新增 `rows` 与 `maxLength`；`Select`、`Checkbox` 新增 `@change`；
  `Tag` 新增与具体 `color` 互斥的 `semanticColor`，均保持编译期类型与
  capability 门禁。
- 下方 2026-08-30 的同名小节属于已删除碎片标签的历史记录，不代表当前
  正式发布序列中的 `v0.0.3`。

## 0.0.6（2026-08-30）

- 语言版本随根 crate 升级到 `0.0.6`；本版本没有新增语法、组件登记或生成映射，既有 `0.0.5`
  语言基线保持兼容。

## 0.0.5（2026-08-30）

- 语言版本随根 crate 升级到 `0.0.5`；本版本没有新增语法、组件登记或生成映射，既有 `0.0.4`
  语言基线保持兼容。

## 0.0.4（2026-08-30）

- 语言版本随根 crate 升级到 `0.0.4`；本版本没有新增语法、组件登记或生成映射，既有 `0.0.3`
  语言基线保持兼容。
