...
见 [目标设计](目标设计.md)：内容插槽、错误行号回映与 `external` 声明、数组操作扩展与受限闭包、computed 派生状态、主题 token 全量扩展、状态伪类、语法小项（If/ElseIf/Else、事件载荷表、props 默认值、类型白名单）。对应 Vikunja 任务 #2157-#2161。
