...
| [已知缺陷](进度/已知缺陷.md) | 窗口布局、自定义标题栏、内容区域等外观与行为缺陷 |
...
| 3.11 | 输入与表单 | Upload | `Upload::dragger()` — 完整拖放区域 | L426-434 |
| 3.13 | 输入与表单 | Upload | `.show_upload_list(true)` — 上传列表 | L426-434 |
| 3.14 | 输入与表单 | Upload | `.preview_image(true)` — 图片缩略图 | L426-434 |
| 3.15 | 输入与表单 | Upload | `.manual(true)` + `.upload()` — 手动上传 | L426-434 |
