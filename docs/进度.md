...
- **wgpu 基线**：生产 module graph 不再编译 D3D11 / D3D12 / OpenGL / Vulkan 的平行 UI raster 实现；一套 WGSL、两个 pipeline、一个按需扩容 vertex buffer 和一张 1024×1024 R8 字形 atlas 服务全部 adapter。严格 GPU canvas 不分配 CPU 主 framebuffer 或 PixelUpload staging
...
- **wgpu 已做**：LayerTree 在 GPU 模式下绕过 CPU FrameRecordingEngine，直接写入严格 Native GPU canvas；相邻同类 op 在 API 中立层合批，再由唯一 wgpu renderer 生成顶点并保持 painter order。真实 Vulkan 首帧约 14.6 ms、稳定后采样帧约 4.7 ms；该数字是本机启动样本，不替代完整帧基准
...
- **已做**：
  ...
  - **严格边界**：GPU 模式不再用局部 CPU tile 冒充 GPU 实现；尚未进入共享 wgpu pipeline 的 image / fractional / target-dependent 操作 typed 失败，由恢复状态机切换整套 CPU engine
...
- **瓶颈**：`ab_glyph` CPU shaping / coverage 生成；image、非 identity / fractional 几何与 Picture GPU composition 尚未闭合；首帧 layout 尖峰。严格模式下这些是功能差距，不是帧内 CPU 中转
- **目标**：稳定 60Hz
