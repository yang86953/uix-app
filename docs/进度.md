...
- **已做**：
  ...
  - **真正 GPU glyph coverage**：字体后端提取轮廓并展平为 NonZero 边列表；严格 GPU 经 `blit_glyph_outline` → 共享 `glyph_cover` **MSDF** pass 写入 RGBA8 atlas，采样 `median` → coverage；tofu / 失败回退仍可走 CPU coverage → R8；soft 保持解析 AA（1:1）
  - **MSDF 多通道字形**：Chlumsky `edgeColoringSimple` 真边着色（角点切换 CMY 双通道）；GPU cover 与 CPU `msdf_from_edges` / `colorize_edges` 同公式；大字号缩放相对单通道 AA 与旧 120° 扇区近似保角更好；跨硬件视觉矩阵仍待补
