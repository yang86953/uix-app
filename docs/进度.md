...
> 使用文档中 `text` fence → `rust uix-compile` 的落地批次。每批独立交付、可并行。
...
|---|------|------|------|-------------------|
| 1.1 | 输入与表单 | Button | `.loading(true)` — 显示加载旋转器，禁用交互 | L385-391 |
| 1.2 | 输入与表单 | Button | `Button::icon("plus")` — 纯图标按钮构造函数 | L385-391 |
| 1.3 | 输入与表单 | Button | `ButtonGroup::new().buttons(...)` — 视觉连体按钮组 | L385-391 |
| 1.4 | 输入与表单 | Input | `Input::search()` — 搜索输入框变体 | L60-65 |
| 1.5 | 输入与表单 | Input | `InputGroup::new()` — 组合输入框+附加元素 | L60-65 |
| 1.6 | 输入与表单 | Input | `.status(InputStatus::Error)` — 状态色边框+消息 | L60-65 |
| 1.7 | 数据展示 | Tag | `.icon("star")` — Lucide 图标 | L681 |
| 1.8 | 数据展示 | Tag | `.color(TagColor::Blue \| Cyan \| ...)` — 扩展 10 色 | L680 |
| 1.9 | 数据展示 | Badge | `.color(BadgeColor::Blue \| Green \| ...)` — 预设色 | L97-104 |
| 1.10 | 数据展示 | Badge | `Badge::ribbon(text, color)` — 角标丝带 | L97-104 |

### 第 2 批：表单系统补全

| # | 文档 | 组件 | 功能 | `text` fence 位置 |
|---|------|------|------|-------------------|
| 2.1 | 输入与表单 | Form | `form.reset()` — 重置所有字段到初始值 | L449-458 |
| 2.2 | 输入与表单 | Form | `form.scroll_to_first_error()` — 滚到首个错误字段 | L449-458 |
| 2.3 | 输入与表单 | Form | `Form::new().initial_values(hashmap)` — 批量初始值 | L449-458 |
| 2.4 | 输入与表单 | Form | `.field(...).initial("default")` — 逐字段初始值 | L449-458 |
| 2.5 | 输入与表单 | Form | `.validate_trigger(Trigger::Submit)` — 仅提交时校验 | L665-669 |
| 2.6 | 输入与表单 | Form | `.depends_on("field", \|val\| rule)` — 简化依赖校验 | L697-704 |

### 第 3 批：选择器与上传

| # | 文档 | 组件 | 功能 | `text` fence 位置 |
|---|------|------|------|-------------------|
| 3.1 | 输入与表单 | Select | `.option_groups(vec![...])` — 选项分组 | L113-118 |
| 3.2 | 输入与表单 | Select | `.loading(true)` — 下拉加载旋转器 | L113-118 |
| 3.3 | 输入与表单 | Select | `.render_option(\|opt\| view)` — 自定义选项渲染 | L113-118 |
| 3.4 | 输入与表单 | Slider | `.marks(vec![...])` — 轨道刻度标记 | L202-207 |
| 3.5 | 输入与表单 | Slider | `.tooltip(TooltipPlacement)` — 拖动提示 | L202-207 |
| 3.6 | 输入与表单 | Slider | `Slider::range(...).start(&s).end(&e)` — 双滑块 | L202-207 |
| 3.7 | 输入与表单 | Cascader | `.loading_child(key, true)` — 子级加载 | L341-345 |
| 3.8 | 输入与表单 | Cascader | `.searchable(true)` — 跨层级过滤 | L341-345 |
| 3.9 | 输入与表单 | InputNumber | `.formatter(\|v\| ...)` — 自定义显示格式 | L185-189 |
| 3.10 | 输入与表单 | InputNumber | `.keyboard(false)` — 仅步进按钮 | L185-189 |
...
> 虚线 = 弱依赖（可先写目标用法再落地校验）；实线 = 强依赖（父组件新增 API 被子组件 form binding 使用）。
