...
> **接口**：声明 ui System 的内置通用组件库及组件分类边界。组件运行时、布局、事件、主题、浮层、表单和虚拟化能力均由 ui System 通过公开/私有契约编排，widgets 不直接持有兄弟 Module 实例。导出：`uix_app::ui` 与 prelude 中的组件类型。
>
...
- `Dropdown`：`src/ui/widgets/navigation/dropdown/` 同目录保存 Rust 与 UIX；UIX 拥有兼容触发器、弹层、菜单行、分隔线、层级缩进、图标、箭头、排版、描边、圆角、阴影、浮层层级及主题角色，Rust 只保留稳定 key、递归可见项、展开与选择状态、组合 trigger 子树、触发方式、键盘与指针输入、动画推进、语义事件、命中和绘制执行；原分类根单文件已迁入既有组件目录，触发器与弹层同帧共享一次主题解析。
...
- `Steps`：`src/ui/widgets/navigation/steps/` 同目录保存 Rust 与 UIX；UIX 拥有横向与纵向尺寸、步骤标记、连接线、圆点、完成图标、标题、说明、默认方向与交互开关、排版、描边、圆角及主题角色，Rust 只保留 current 双向状态、显式与推导状态、方向键和指针输入、回调、语义事件、命中和绘制执行；原分类根单文件已迁入组件目录，两种方向同帧只解析一次主题。
- `Tabs`：`src/ui/widgets/navigation/tabs/` 同目录保存 Rust 与 UIX；UIX 拥有四向标签栏、标签容量、图标、关闭与新增入口、滚动步长、分隔线、活动指示、内容留白、默认开关、排版、描边、圆角及主题角色，Rust 只保留稳定 key、typed 值绑定、活动面板可见性、滚动偏移与可见性算法、编辑回调、键盘与指针及滚轮输入、语义事件、布局失效和绘制执行；原分类根单文件已迁入组件目录，全部区域同帧只解析一次主题。
...
当前没有只透传 `kernel`/`children`、仍待回填静态视觉声明的已迁移 widget；Rust 完整默认视觉表与 UIX 巨型位置参数壳两类债务也已清零。公开视觉 widget 清单当前为 84 个，现已完成 84/84，显式债务集合为空；该清单是架构迁移记录，不构成测试覆盖。已迁移的简单叶组件、分组视觉组件以及由 Tooltip 与 Slider 共享的 `TooltipBubble` 均完成 `<Visual>` 单源闭环；共享视觉资源允许通过 `uix_items!` 只声明模块级 `Visual`，不需要伪造可实例化视图根。
