...
> 历史组件迁移记录（当前源码使用 widget_runtime）。
>
...
`WidgetRender::children_transform(frame)` 可声明父坐标系中的直接子树变换；默认 `None` 不增加变换，`Some(identity)` 仍声明动态合成边界，防止静态 Picture 缓存冻结动画采样。`children_opacity()` 默认 `1.0`，只在直接父子合成边界相乘，后代继承，不逐层重复施加同一父透明度。两者不改变终态布局，绘制、视觉语义、裁剪和命中必须消费同一视觉变换。

`WidgetTextInput::text_input_cursor_rect()` 返回布局绘制坐标；窗口层再叠加祖先视觉变换和滚动，形成平台所需的表面光标矩形。控件不自行重复叠加该矩阵。
