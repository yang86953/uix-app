...
| `LayoutChild` | struct | 子身份、border-box 测量尺寸、独立 flex_basis、最小盒尺寸、margin、flex/grid 参数 |
...
- `Container` / `Grid` 的声明宽高采用 border-box，不在声明尺寸之外再加 border。自动尺寸通过 `BoxModel::border_box_size` 从内容加上 border 与 padding；空内容同样保留内边距。`min_size` 保存不可压缩的盒内边界，空间不足时允许可观测溢出，不把边框与内边距收缩为零。
- 测量尺寸与弹性基值是不同事实：`measure` 返回首选 border-box，`LayoutChild::flex_basis` 仅覆盖父主轴分配。父级方向由布局组件传入，不能以子组件内部方向推断。`Container` / `Grid` 的 grow 自动主轴采用零内容基值，但保留 padding 与 border；交叉轴仍由自然内容决定。
- 委托容器布局的包装组件（如窗口拖动/缩放/控制区域）须一并转发 `measure_natural`、`flex_basis`、`minimum_size` 和 `flex_layout_axes`，不能只转发 grow/shrink 而丢失弹性基值与盒模型下限。
...
- 标准、溢出与 wrapped 路径共享同一求解器；溢出模式禁用 grow/shrink 并忽略零基值，不另建位置算法。固有交叉轴的自然尺寸下限对这些路径一致有效。
- 横排父级的非 Stretch 子项高度由内容决定；其内部 Column 不能因自身 `flex_grow > 0` 就把上一轮高度当成已分配的纵向主轴，从而把新换行压回旧高度。通过 `WidgetLayout::flex_layout_axes` 的窄契约读取父轴事实，不递归重测整棵子树。
