...
- Flex 实际分行 overflow/wrap/固有轴中负 margin/gap 的联合矩阵、轴转置、正反向镜像与弹性冻结矩阵，及表格其他合并组合的 measure/paint/hit-test 一致性已有独立门禁；可变行高缓存已完成缓存、滚动重锚与 212/212 库测试验证，当前剩余审计为真窗视觉矩阵。
- #715 真窗视觉矩阵首轮证据准备于 2026-08-08 执行：D3D11 硬件上下文（AMD Radeon 780M）与 Agent bridge 已启动，但 `visual-first-present` 等待 30 秒超时，`presented_revision` 保持 0，未生成截图。Windows console 会话为 Active、DWM 与驱动状态正常；该项暂记为 DXGI 首帧 Present/occlusion 阻塞，等待主人在可呈现物理桌面上复验，不视为布局通过。
