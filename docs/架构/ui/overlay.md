...
> **接口**：声明 ui System 的窗口内浮层栈、层叠、焦点陷阱和进退场。与[组件运行时框架](widget_runtime.md)、[event](event.md)和[animation](animation.md)的协作由 ui System 编排，Module 间不直接持有实例。导出：所有内置与自定义浮层共享的栈契约；本页维护框架的浮层栈机制，具体弹窗与提示组件由其实现库维护。
>
