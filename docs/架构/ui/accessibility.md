...
>
> **当前实现线索**：相关实现暂位于 `src/ui/accessibility_override.rs`、`component_snapshot/`、`semantic_snapshot.rs` 等位置；重构后由本模块统一派生语义树。
...
WidgetTree 当前状态
  ...
  → TestApp / Agent Bridge / 未来平台桥
```

快照只在事件、due work 或帧轮次末尾按需更新；内容变化才推进 revision，不创建轮询线程、timer 或额外 frame。
