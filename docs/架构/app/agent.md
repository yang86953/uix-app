...
>
> **当前实现线索**：Agent Module 位于 `src/app/agent/`（`agent_bridge.rs`、`agent_control.rs`、`agent_policy.rs`、`agent_protocol/`、`agent_transport.rs`），app System 私有命令/状态边界位于 `src/app/queues/agent_command_queue.rs`、`src/app/queues/window_agent_state.rs` 与 `src/app/window_semantics.rs`，OS IPC 传输实现位于 `src/platform/adapters/transport/`，窗口动作的平台操作适配位于 `src/app/window/window_agent_ops.rs`。
...
优化分层计量：任务规划/模型往返、传输、UI owner 排队、动作与 settle、语义/呈现验证分别记录，不能把 socket 微基准当作业务完成或人工速度。控制连接、wait 连接和 media 连接各自持有已鉴权会话；同一实例内分流可减少队头阻塞，不改变 UI 队列 FIFO、策略、确认或逐动作 settle。

- 稳定目标：复用 instance/window generation 和认证连接，按 automation_id 重新取得当前节点；版本、代际或目标条件变化时复核，不按同名应用改绑。Hub 只负责发现，不代理 UI 状态。
- 顺序执行：目标已明确的操作由调用端组织有界成果批次，不让每个按键都返回模型。当前 UIX 协议仍逐请求返回真实 revision；没有事务型批量提交或跨进程 exactly-once，不能为了提速把多条 mutation 全部预先入队并忽略中间终态。
- 状态等待：直接使用现有 wait 的 after_revision 或 presented_revision（严格二选一），同代际状态变化后再读取必要快照。普通客户端 wait_for_snapshot 是通用谓词轮询助手，当前仍有 50 ms 间隔；不能无条件用仅监听语义 revision 替代任意呈现/外部条件谓词。专用事件驱动助手和工具包的 sequence/transition 适配作为后续显式接口工作。
- 观察预算：优先必要的语义回读，成果阶段末才取离屏截图；媒体通道的大响应按帧预算处理，不影响普通控制连接。需要小字/像素验证时保留完整图像，不将预览成功当作完整视觉验收。
- 失败停止：关联错误、半帧、超限或传输失败不能继续发动作；已经开始执行的 outcome_unknown 保留原信封并封闭该客户端，需在重新核对的同一实例上另建只读观察，不自动重派。用户取消、策略撤销、generation 变化不属于应被优化掉的中断。

### 当前客户端实现与后续边界

官方 Rust AgentBridgeClient 将请求 JSON 与换行合并写出，以 BufRead 的连续块解析响应，不再逐字节调用 read；换行计入协商的响应上限。任何开始传输后的分帧/I/O/身份错误，以及可信 outcome_unknown，都会将当前客户端标为不可继续派发，后续请求在本地返回 connection_unusable。参数/请求长度的本地拒绝与普通可信业务拒绝不会自动耗尽连接，不新增授权。

客户端协议检查与真实后台工作面的人工验收是不同证据。第三方电脑控制工具使用真实前台桌面路线，
不从 UIX 的后台 Agent 改动自动获得能力；任何外部消费者都需按自己采用的依赖版本和权限边界验证。
不能绕过背景隔离检查或回退到用户窗口。
