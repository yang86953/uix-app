...
> **GPU recipe 原子门禁**：GPU recipe 在进入 backend 前必须暴露单一 `GpuRecipeContext`，thin RHI 与 resize owner 不再能形成分裂状态；缺少完整视图的 context 会在构造失败路径 checked shutdown。这样首个窗口 resize 不再承担发现错误 adapter 注册的职责。
...
> **静态 capability 快照**：`GraphicsRecipeOwner` 在 native factory 出口只读取一次 `GraphicsContextCaps`，再把同一快照交给 GPU 或 PixelUpload 专用 owner 门禁；专用 owner 不重复调用兼容 context 的 `caps()`。该快照在 owner 生命周期内固定 backend/raster/present/coherency/occlusion 事实。thread-bound wrapper 同样只在创建线程捕获一次静态快照，GPU 与 PixelUpload resize 成功后只刷新完整 `PresentSurface`。运行期 recipe 身份与 owner-loss 诊断不再依赖兼容 context 的重复查询。
...
>
> **resize 边界**：生产 `GpuBackend` 只接收 native factory 已准备并通过 probe 的 GPU-only thin RHI context。初始化只同步 drawable 元数据；后续 resize 释放旧代 retained/overlay 资源后借用同一 `GpuRecipeContext` 调用唯一 RHI surface resize，任何 typed failure 都直接进入恢复层。`IGraphicsContext` 不再声明通用 `resize`、RHI resize 或独立 RHI 借用，只返回单一 recipe 视图；thread-bound wrapper 先转发 owner-thread resize，成功后才整体刷新 `PresentSurface`。CPU × PixelUpload recipe 在构造时必须暴露独立 `PixelUploadSurface`，Vulkan/Metal 只通过该契约重建上传 surface，GPU adapter 不实现它。
>
