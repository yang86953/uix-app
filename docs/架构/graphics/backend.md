...
> **设计状态**：🔄 迁移中。通用 FramePlan、retained RHI、D3D11 与 OpenGL ES 执行路径已经落地；overlay backdrop 的 typed UI 策略、Theme/显式半径、区域聚合与 clean/effect 双纹理失效已接线。剩余工作集中在普通树变化期间的单-present 快照重建与真实环境矩阵。
...
- overlay backdrop 已接通全部 OverlayKind 的显式 typed opt-in、Theme 默认 8.0、独立逻辑区域、区域并集/最大半径聚合、能力查询、mask-only 降级、旧/新区域 damage 与 clean→effect 重派生。剩余差距是普通树、主题或窗口尺寸变化期间，在不重复 acquire/present 的前提下完成正常树提交→新 clean snapshot→blur→overlay，以及 Picture/offscreen blur 与 overlay backdrop 的跨 DPI、真实 GPU 和操作系统矩阵。
...
- overlay backdrop recording 测试覆盖 retained→clean 快照与 effect/clean→retained 恢复的方向、全幅物理 extent、唯一 device submit、无 acquire/present，以及创建失败无半成品和 submit 失败检查式销毁；blur 测试另覆盖逻辑区域只应用一次主 surface DPR、extent 裁剪、effect→scratch→effect 顺序、单次 submit、无 acquire/present 与 scratch 检查式销毁。UI 聚合测试覆盖全部 OverlayKind、Theme/显式半径、mask/独立区域、区域并集与最大半径；场景管线测试覆盖 snapshot/blur DeviceLost、restore SurfaceLost 与 release OOM 的 typed 分类传播，并证明失败后不继续 begin/end/present。
