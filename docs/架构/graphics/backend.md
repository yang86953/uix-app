...
- `IGraphicsContext` 与 `NativeRasterCaps` 仍保留迁移期的逐 UI 操作；overlay backdrop 的 snapshot/restore/release/has 四个高层入口已移除，目标是继续把几何、batch、atlas、offscreen 与 effect 调度收回通用 GPU Renderer。
