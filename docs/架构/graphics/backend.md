...
- 剩余差距包括 RHI Picture 已提交后的 destination-dependent soft 混合、更多仿射图元，以及跨 DPI、GPU 和操作系统的测试覆盖。
