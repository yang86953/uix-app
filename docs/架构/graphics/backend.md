...
> **坐标与行序契约**：通用逻辑层只使用左上原点逻辑坐标，thin RHI 的 viewport、scissor、顶点、UV 输入与 `PresentDamage` 均保持左上原点语义，跨平台一致（`RhiScissor` 保持左上原点约定）。帧缓冲行序差异只发生在呈现层，由各 adapter 内部消化，不得泄漏到通用层：WGL 的 DIB 为 bottom-up 行序，Wayland EGL 的 wl_buffer 与 D3D11 backbuffer 均为 top-left 行序。顶点 Y 翻转按平台机理处理——WGL 与 D3D11 的顶点阶段固定保留 `ndc.y = -ndc.y`（逻辑 top-down 到 NDC y-up 的标准转换）；EGL 通过 `flip_y` 标志在 shader 编译期去掉该翻转，其呈现行序反向与 GL 坐标反向恰好双重抵消，scissor 亦可直接使用 RHI 坐标；WGL 因呈现同向必须把 scissor 换算为 GL 左下原点（height − y − h）；D3D11 scissor 原生 top-left 直接使用。`read_surface_pixels` 的 readback 输出统一为 top-left 行序（row 0 = 窗口顶部）：D3D11 staging 拷贝与 EGL 的 glReadPixels（row 0 = GL y=0 行 = 窗口顶部）天然满足，bottom-up 平台（WGL）的 adapter 必须在返回前完成垂直翻转。像素搬运路径分两类：机械拷贝（GPU 快照 copy、滚动 memmove）逐 texel 对位、与行序无关，天然自洽；blur 的 NDC quad 采样依赖与呈现行序的耦合——WGL 上两垂直 pass 的翻转恰好互相抵消（scratch 中间结果颠倒、第二 pass 恢复），属脆弱结构，新增单 pass 效果前必须显式处理，不得在通用层叠加翻转。CPU 软渲染/upload 的像素数据统一为 top-left 行序，纹理上传保持数据直通、UV 跟随顶点，无需行序处理。当前 `flip_y` 已覆盖 OpenGL 绘制路径；readback 行序在 WGL 实现上尚未收敛，属于待办契约缺口。
