...
> **recipe 静态事实**：具体 adapter 创建模块拥有唯一的 `GraphicsContextCaps` 构造函数，并把新 context 与该快照封装为 `GraphicsContextCandidate`；registry 只消费 candidate、校验精确 registry row，再把同一已验证快照交给 `GraphicsRecipeOwner` 及 `GpuRecipeOwner` 或 `PixelUploadRecipeOwner` 的专用门禁。`IGraphicsContext::caps` 已物理移除，thread-bound wrapper 也不再接收或缓存静态 capability，只负责线程亲和与 live `PresentSurface` 生命周期。运行期 capability 投影与 owner-loss 诊断都使用 owner 内不可漂移的构造期快照。
