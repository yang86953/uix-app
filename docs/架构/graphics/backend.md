...
- `IGraphicsContext` 的逐 UI draw/clear/offscreen/upload 定义与 adapter wrapper 已移除，`NativeRasterCaps` 只保留 retained framebuffer 和 RHI Additive 事实；D3D11/OpenGL ES 无消费者的旧 rect/glyph/gradient/mesh/shadow/image pipeline 资源已经删除，D3D12 的测试期逐 UI raster pipeline 也已退出。draw backend 已无 direct swapchain legacy consumer；其余迁移工作是继续收窄生命周期/present 兼容门面，并把未闭合 effect 调度收回通用 GPU Renderer。
...
- 剩余差距包括 backdrop blur 等尚未闭合的多阶段效果、`IGraphicsContext` 生命周期/present 兼容门面收窄，以及 Picture/offscreen blur 与 overlay backdrop 跨 DPI、真实 GPU 和操作系统的运行时矩阵覆盖。
