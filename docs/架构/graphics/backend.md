...
>
> **当前实现线索**：通用执行位于 `src/draw/backend/`；thin RHI、类型化 recipe、registry、线程绑定与原生 Adapter 分布于 `src/native/presentation/contracts/`、`src/native/factory/` 和 `src/native/presentation/graphics/`。路径只用于定位迁移，不构成公开 API。
