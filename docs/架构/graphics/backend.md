...
- 剩余差距包括 backdrop blur 等尚未闭合的多阶段效果、`IGraphicsContext` 剩余 recipe/lifecycle 与最终呈现入口收窄，以及 Picture/offscreen blur 与 overlay backdrop 跨 DPI、真实 GPU 和操作系统的运行时矩阵覆盖。
