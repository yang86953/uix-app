...
> **设计状态**：📋 目标设计，当前未排期。现有 `RenderBackend` / `GpuBackend` 已形成部分通用执行层，但原生 `IGraphicsContext` 仍混合 device、surface 与逐 UI 操作的高层光栅接口，尚未收敛为本文定义的薄 RHI。

> **当前实现线索**：通用部分主要位于 `src/draw/backend/`；平台接口位于 `src/native/present/`，原生实现位于 `src/native/presentation/graphics/`。
...
- 📋 目标是把原生实现中的 UI 几何、batch、atlas、offscreen 与 effect 调度收回通用 GPU Renderer，仅保留本文的最小 RHI。该工作当前未排期，交付状态由[交付方向](../../进度/交付方向.md)持有。
