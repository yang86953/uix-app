...
- 剩余差距包括 backdrop blur 等尚未闭合的多阶段效果、CPU PixelUpload 与共享生命周期对 `IGraphicsContext` 的剩余依赖，以及 Picture/offscreen blur 与 overlay backdrop 跨 DPI、真实 GPU 和操作系统的运行时矩阵覆盖。
