...
Picture 绘制事务在 draw 侧只公开 `try_begin_offscreen_paint`、`try_flush_offscreen_paint`、`try_end_offscreen_paint` 与 `try_blit_offscreen_src` 四个 `Result` 边界；`RenderBackend`、`RenderTarget`、`Renderer` 与 `RecoveryDriver` 不再保留 bool/void 兼容入口。未实现的默认 begin/flush/end/blit 返回 typed `NotImplemented`，不能以无操作或漏绘报告成功；GPU 最终 present 若发现 active Picture，也必须先通过 checked end，失败时停止主 surface 提交。create/destroy owner 契约不在本轮改变。
