...
- `IGraphicsContext` 的逐 UI draw/clear/offscreen/upload 定义、adapter wrapper、二阶段 `initialize`、`make_current`、`swap_buffers`、readback 及默认 `present` 回退已移除；native factory 的构造成功即表示 context 已绑定 surface 并可用，RenderSession 每帧通过语义型 `prepare_frame` 进入 thin RHI 设备维护。生产 `GpuBackend` 只接受 GPU-only retained RHI baseline；initialize 不再二次 resize，显式 resize 也不再回退兼容 context。同步 surface readback 现在是 `GraphicsCapabilities::surface_readback` 声明的可选 `GraphicsSurface` 操作，仅 D3D11/OpenGL ES 生产 adapter 启用；Vulkan GFX-R5 与 D3D12 测试期内部诊断不构成通用能力。`NativeRasterCaps` 只保留 retained framebuffer 和 RHI Additive 事实；D3D11/OpenGL ES 无消费者的旧 rect/glyph/gradient/mesh/shadow/image pipeline 资源已经删除，D3D12 的测试期逐 UI raster pipeline 也已退出。draw backend 已无 direct swapchain legacy consumer；其余迁移工作是继续收窄 `IGraphicsContext::resize` 本身，并把未闭合 effect 调度收回通用 GPU Renderer。
