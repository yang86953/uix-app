...
> **recipe 静态事实**：具体 adapter 创建层把新 context 与一次组装的 `GraphicsContextCaps` 封装为 `GraphicsContextCandidate`；registry 只消费 candidate、校验精确 registry row，并把同一已验证快照依次交给 thread-bound wrapper、`GraphicsRecipeOwner` 以及 `GpuRecipeOwner` 或 `PixelUploadRecipeOwner` 的专用门禁，不再通过兼容 context 重查 capability。运行期 capability 投影与 owner-loss 诊断都使用该不可漂移快照。drawable extent、DPR、transform 与 generation 仍只从 live `PresentSurface` 原子读取。
