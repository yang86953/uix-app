...
| <a id="d168"></a>168 | 可组合组件模型 | **架构原则**（统领六域）：每层按 **正交能力组件** 划分，经 trait / registry **自由组装**；engine 类是轴组合的组装结果，**不得**把 bundled 枚举当作架构 mental model。**draw 渲染轴**：`RasterMode`（CPU `CpuBackend` / GPU `RenderBackend`）× `PresentMode`（swap / pixel upload / `IPresenter`）× `GraphicsBackend`（`IGraphicsContext` peer）。旧 `RenderPipelineProfile` **已拒绝**（[#169](#d169)）。**参照实现**：`ui` 组件 capability trait（Layout / Render / Event / Lifecycle / Animation 按需 impl）；`layout` measure·arrange 与纯函数引擎；`event` 平台·系统·语义三层；`native` `Platform` 聚合子 trait。**禁止**：文档或 API 把「仅 X API 能 Y」写成永久约束（应写「组件 A + 组件 B」）；上层感知 OS/API 细节；保留第二套并行分派。**文档**：[graphics-backend-pluggable · 可组合渲染轴](areas/systems/graphics-backend-pluggable.md#可组合渲染轴) · [component · 能力](areas/systems/component.md#能力) · [demand-driven · 分域要求](areas/systems/demand-driven.md#分域要求) |
| <a id="d169"></a>169 | 可组合渲染轴落地 | **[#168](#d168) follow-up 确认**（三选一合并）：**(1) 分派模型**：engine 分派 **仅** `RasterMode` × `PresentMode`（× `GraphicsBackend`）；旧 `RenderPipelineProfile` **breaking 删除**；**禁止**别名或第二套并行分派。**(2) 代码优先（[#167](#d167)）**：**Windows 先行** — **D3D11 GPU native raster**（扩展 `RenderBackendRegistry` + `GraphicsContextCaps` 声明 `RasterMode::GpuNative`）；Metal / D3D12 native raster **随后**。**(3) BackendKind 统一**：`BackendKind`（`Cpu` / `Gpu` / `Auto` / `Null`）纳入可组合 refactor，与 `RasterMode` / `PresentMode` / `GraphicsBackend` **对齐**；factory / engine **表驱动正交装配**（registry 行 = API + 光栅 + present 组件组合）。**文档**：[graphics-backend-pluggable · 可组合渲染轴](areas/systems/graphics-backend-pluggable.md#可组合渲染轴) · [implementation · P6.8](areas/implementation.md#p68-可组合渲染轴) |
