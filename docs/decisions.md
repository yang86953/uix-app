...
> **唯一决策台账**（#1–#100 定稿；#101–#169 术语收敛、核心理念与主要实现边界；#170–#175 收敛 Result-only、直接依赖/cfg、图形合法组合、零闲置证据、布局两阶段输入与运行时全局主题命令；新决策 #176+）。锚点 `#d{N}`。系统正文 → [`areas/systems/`](areas/systems/) · 完整索引 → [areas/architecture.md](areas/architecture.md)
...
| <a id="d113"></a>113 | 零闲置豁免 | 无法框架托管的定时/轮询 → **`decisions.md` #176+** 公开条目 + 测试证明；默认不豁免；当前无豁免见 [#158](#d158) |
...
| <a id="d158"></a>158 | 零闲置豁免台账 | 当前 **无豁免**；新增豁免必须追加公开条目（当前从 **#176+** 起），并写明触发源、wake 频率、允许工作范围、无法 register 的理由与测试边界 |
...
| <a id="d162"></a>162 | 多图形 API 抽象与选型 | Vulkan / D3D11·12 / Metal / OpenGL ES /（远期 WebGPU）经 `native::IGraphicsContext` + `draw::GraphicsEngine` 统一；**框架内部上层实现**（`app`/`ui`/`draw` engine path）禁止按具体 API 分支，具体 `GraphicsBackend` identity 仅允许出现在用户配置、诊断与 native factory registry（[#172](#d172)）。**选型**：`draw::bootstrap_graphics_engine` **唯一** probe 循环（#163）；`native::try_create_context` 单条目创建；失败链式回退至下一 API，终态 `SoftwareEngine`（#59）。**分层**：`BackendKind::Gpu` = GPU 管线；具体 API 用 `GraphicsBackend` 枚举配置/诊断；实现封装在 `native/graphics/<api>/`（#164）。**渲染组装**（[#168](#d168) · [#169](#d169)）：`RasterMode` × `PresentMode` × `GraphicsBackend` 是受 caps 约束的正交描述；旧 `RenderPipelineProfile` bundled 分派 **已拒绝并删除**（P6.8）。**Auto 链**（registry 表驱动，#163）：Windows D3D11→GL ES(WGL)（D3D12 Planned）；Linux Vulkan→GL ES(EGL)；macOS Metal→Software。**零闲置**：禁止每帧 probe 或热切换 API。详见 [rendering · 多图形 API](areas/systems/rendering.md#多图形-api) · [graphics-backend-pluggable](areas/systems/graphics-backend-pluggable.md) · [implementation · P6](areas/implementation.md#p6-生产级框架) |
| <a id="d163"></a>163 | 可插拔图形后端 registry 架构 | **对等实现**：Vulkan / D3D / GL ES / Metal 均为 `IGraphicsContext` peer；`GraphicsBackend` = **API 身份**（非 OS）；**源码落点**见 [#164](#d164)；OS **仅**决定编译候选与 `Auto` probe 顺序；`draw`/`ui`/`app` 的 engine 实现只按 `IGraphicsContext` / `GraphicsEngine` / caps 工作，API identity 的配置与诊断例外见 [#172](#d172)。**范围**：「任意更换」= 跨平台配置 + 初始化时从**已编译**候选 probe + 清晰扩展路径；**不含**运行中热切换、`dlopen` 动态插件、同进程多窗口异构 API（不在范围）。**Registry**：`native/factory` 静态 `GraphicsBackendEntry` 表驱动；`native::try_create_context` / `try_create_gpu_context` **单条目**创建（**无** probe 循环）；`create_gpu_context*` 亦单条目，`Auto` 须走 bootstrap。**Probe 唯一性**：**仅** `draw::bootstrap_graphics_engine` 迭代 `gpu_probe_candidates`。**Engine 分派**（设计，[#169](#d169)）：**仅** caps 允许的 `RasterMode` × `PresentMode`；`GpuNative` 的 `RenderBackend` 由 `draw::RenderBackendRegistry` 按 `caps.backend` 配对；`Cpu` × `PixelUpload` → `PresentUploadEngine` + `CpuBackend`。非法组合记录诊断并继续 fallback。旧 `RenderPipelineProfile` **已拒绝并删除**（P6.8）。**Backlog**：见 [#169](#d169)（D3D11 GPU native raster 优先；随后 Metal / D3D12）。**能力**：`GraphicsContextCaps`（native）与既有 `GraphicsCapabilities`（draw）分层推导。**Bootstrap**：选项 B — GPU 路径归 draw；`SoftwareEngine` + `IPresenter` CPU 回退留 app；engine 失败 **必须** `ctx.shutdown()`。**Present**：`IGraphicsContext::present(PresentFrame)` 统一契约。详见 [graphics-backend-pluggable](areas/systems/graphics-backend-pluggable.md) |
...
| <a id="d168"></a>168 | 可组合组件模型 | **架构原则**（统领六域）：每层按 **正交能力组件** 划分，经 trait / registry 做 **受 capability 约束的组合**；engine 类是合法轴组合的装配结果，**不得**把 bundled 枚举当作架构 mental model。**draw 渲染轴**：`RasterMode`（CPU `CpuBackend` / GPU `RenderBackend`）× `PresentMode`（swap / pixel upload / `IPresenter`）× `GraphicsBackend`（`IGraphicsContext` peer）；“×”表示独立描述维度，**不承诺完整笛卡尔积**，合法性由 caps + registry 判定（[#172](#d172)）。旧 `RenderPipelineProfile` **已拒绝**（[#169](#d169)）。**参照实现**：`ui` 组件 capability trait（Layout / Render / Event / Lifecycle / Animation 按需 impl）；`layout` measure·arrange 与纯函数引擎；`event` 平台·系统·语义三层；`native` `Platform` 聚合子 trait。**禁止**：文档或 API 把「仅 X API 能 Y」写成永久约束（应写「组件 A + 组件 B」）；框架内部上层按 OS/API 分支；保留第二套并行分派。**文档**：[graphics-backend-pluggable · 可组合渲染轴](areas/systems/graphics-backend-pluggable.md#可组合渲染轴) · [component · 能力](areas/systems/component.md#能力) · [demand-driven · 分域要求](areas/systems/demand-driven.md#分域要求) |
| <a id="d169"></a>169 | 可组合渲染轴落地 | **[#168](#d168) follow-up 确认**（三选一合并）：**(1) 分派模型**：engine 分派 **仅**按 caps 已确认的 `RasterMode` × `PresentMode`（× `GraphicsBackend` identity）选择合法组合；旧 `RenderPipelineProfile` **breaking 删除**；**禁止**别名或第二套并行分派。**(2) 代码优先（[#167](#d167)）**：**Windows 先行** — **D3D11 GPU native raster**（扩展 `RenderBackendRegistry` + `GraphicsContextCaps` 声明 `RasterMode::GpuNative`）；Metal / D3D12 native raster **随后**。**(3) BackendKind 统一**：`BackendKind`（`Cpu` / `Gpu` / `Auto` / `Null`）纳入可组合 refactor，与 `RasterMode` / `PresentMode` / `GraphicsBackend` **对齐**；factory / engine **表驱动正交装配**（registry 行 = API + 光栅 + present 组件组合）。非法组合不得猜测装配，须返回诊断并继续 bootstrap fallback（[#172](#d172)）。**文档**：[graphics-backend-pluggable · 可组合渲染轴](areas/systems/graphics-backend-pluggable.md#可组合渲染轴) · [implementation · P6.8](areas/implementation.md#p68-可组合渲染轴) |
| <a id="d170"></a>170 | 平台可选窗口能力 Result-only | `WindowOps` 与公开 `PlatformWindow` / `IWindowProperties` 的可失败能力均以 `Result` 暴露；不再保留仅记录日志的 void 兼容面。`Errc::NotImplemented` 是能力缺失的 canonical 表达；调用方不得用状态缓存猜测成功。该 breaking 收敛以 [platform · 窗口可选能力](areas/systems/platform.md#窗口可选能力) 为准。 |
...
| <a id="d175"></a>175 | 运行时全局主题命令 | 启动主题仍由 `App::theme(Theme)` builder 指定；运行中由任意存活窗口的 `AppHandle::set_theme(Theme) -> Result<()>` 替换 **App 全局**主题。连续未消费请求合并为最终值且只制造一次 event-loop wake；消费时主窗与全部存活副窗各派发一次 `ThemeChanged`，仅 palette 节点 Paint invalidate，禁止 Layout / reconcile 与调用方逐窗标脏。切换后新建窗口直接读取当前主题，不补发历史事件；关闭 handle 返回 `Errc::InvalidState`。与 OS 跟随同时启用时，显式主题保持到下一次真实 `ThemeChanged`，届时重新采用系统模式；全程无后台 poll。详见 [theme-style](areas/systems/theme-style.md) · [application](areas/systems/application.md#appstate--多窗--settings)。 |
