...
> **AI 何时打开**：产品愿景 / Windows 优先 / 编号需求。硬约束 → [`AGENTS.md`](../AGENTS.md)；执行 → [`进度.md`](进度.md)。
...
证据级别：`designed` → `coded` → `automated` → `compiled:<platform>` → `hardware:<platform>` → `production`。细节与验收检查 → [`进度.md`](进度.md)。
