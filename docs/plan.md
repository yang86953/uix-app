...
1. `cargo test --all-targets` 零失败；当前 lib 1096/1096、demo 19/19，历史两项红测、D3D11 完整 fill topology 与完整 `StrokeOptions` cap/join 原生 tessellation、D3D12 context 同步/row-pitch readback/resize/shutdown及 rounded solid + premultiplied soft-blit、API-neutral `NativeGpuBackend` 的逐操作 capability/soft fallback 路由、原生录制顺序、单次 offset、context `Result` 到 `RenderBackend` 边界的错误传播/重试重放与幂等 shutdown、统一 backend registry 路由及 factory backend identity 守卫、CPU fill 的 clipped 斜边采样/开放子路径闭合、颜色通道、GDI damage 越界、首帧 LayerTree/Picture 回退、DisplayList glyph/clip、WGL core loader/caps、Win32 多窗状态串用、embedded build 子树删除与 UTF-16 代理对错误已用代码与守卫修复，未使用豁免。
