...
| 文档合格化与契约收敛 | 结构、追踪、依赖/cfg、Result-only、wake/L1、caps 组合、证据等级、布局两阶段输入与全局主题命令已在 2026-07-10 审计修复 | [requirements.md](requirements.md) · [design.md](design.md) · [#170–#175](decisions.md#d170) |
| 验证基线 | lib 1097/1097、demo 19/19、doc 3/3；三目标 compile、Clippy 与严格文档检查通过 | [implementation · 当前验证基线](areas/implementation.md#当前验证基线-2026-07-10) |
...
1. `cargo test --all-targets` 零失败；当前 lib 1097/1097、demo 19/19，历史两项红测、D3D11 完整 fill topology 与完整 `StrokeOptions` cap/join 原生 tessellation、D3D11/D3D12 `PresentFrame::Swapchain` 错误传播、D3D12 context 同步/row-pitch readback/resize/shutdown、Present 故障锁定及 rounded solid + premultiplied soft-blit、API-neutral `NativeGpuBackend` 的逐操作 capability/soft fallback 路由、原生录制顺序、单次 offset、context `Result` 到 `RenderBackend` 边界的错误传播/重试重放与幂等 shutdown、统一 backend registry 路由及 factory backend identity 守卫、CPU fill 的 clipped 斜边采样/开放子路径闭合、颜色通道、GDI damage 越界、首帧 LayerTree/Picture 回退、DisplayList glyph/clip、WGL core loader/caps、Win32 多窗状态串用、embedded build 子树删除与 UTF-16 代理对错误已用代码与守卫修复，未使用豁免。
...
- **P6 代码优先（[#169](decisions.md#d169)）**：可组合渲染轴落地 — 1) D3D11 `GpuNative` × `Swapchain` 垂直切片、完整 fill topology，以及遵循 `StrokeOptions` cap/join 的 identity path mesh ✅；2) 非 GL 原生光栅已收敛为 API-neutral `NativeGpuBackend` + `NativeRasterCaps` 逐操作路由，未支持操作确定性 soft fallback，普通 pipeline 不再下转具体 API 类型 ✅；3) D3D12 device/queue/swapchain/context 生命周期 + rounded solid + premultiplied soft-blit 垂直切片 ✅，以低优先级 `D3D11 → OpenGL ES → D3D12` 进入 Auto 链；4) `RasterMode` / `PresentMode` 类型与 caps + 表驱动装配、`BackendKind` 统一 ✅。下一 native raster 代码优先为 Metal `GpuNative`；D3D12 stroke/glyph/gradient/path/shadow 等仍按 capability 确定性走 soft。详见 [implementation · P6.8](areas/implementation.md#p68-可组合渲染轴) · [graphics-backend-pluggable · 可组合渲染轴](areas/systems/graphics-backend-pluggable.md#可组合渲染轴)。
...
- D3D11 identity fill/stroke path 子项已闭合；D3D12 bounded `GpuNative` 垂直切片已接入共享 `NativeGpuBackend`，原生 clear/rounded solid，其余操作 soft；下一 native raster 切片为 Metal `GpuNative`。
- 已废弃 bundled 管线枚举（`RenderPipelineProfile`）已随 P6.8 正交轴 refactor **删除**（[#169](decisions.md#d169)）；D3D11 完整能力与 D3D12 bounded 能力均按同一 registry/caps 模式运行，D3D12 能力扩展与 Metal `GpuNative` 仍 backlog。
