...
| 验证基线 | lib 1076/1076、demo 19/19、doc 3/3；三目标 compile、Clippy 与严格文档检查通过 | [implementation · 当前验证基线](areas/implementation.md#当前验证基线-2026-07-10) |
...
- **P6 代码优先（[#169](decisions.md#d169)）**：可组合渲染轴落地 — 1) D3D11 `GpuNative` × `Swapchain` 垂直切片、完整 fill topology，以及遵循 `StrokeOptions` cap/join 的 identity path mesh ✅；数值异常、预算超限、不支持 transform/blend 等保留确定性 soft fallback；2) `RasterMode` / `PresentMode` 类型与 caps + 表驱动装配 ✅；3) `BackendKind` 统一 ✅。下一原生 raster backlog 为 D3D12 `GpuNative`（Windows 优先），随后 Metal `GpuNative`。详见 [implementation · P6.8](areas/implementation.md#p68-可组合渲染轴) · [graphics-backend-pluggable · 可组合渲染轴](areas/systems/graphics-backend-pluggable.md#可组合渲染轴)。
