...
| 验证基线 | lib 1088/1088、demo 19/19、doc 3/3；三目标 compile、Clippy 与严格文档检查通过 | [implementation · 当前验证基线](areas/implementation.md#当前验证基线-2026-07-10) |
...
1. `cargo test --all-targets` 零失败；当前 lib 1088/1088、demo 19/19，历史两项红测、D3D11 完整 fill topology 与完整 `StrokeOptions` cap/join 原生 tessellation、D3D12 context 同步/row-pitch readback/resize/shutdown、API-neutral `NativeGpuBackend` 的逐操作 capability/soft fallback 路由、原生录制顺序、单次 offset、context `Result` 到 `RenderBackend` 边界的错误传播/重试重放与幂等 shutdown、统一 backend registry 路由及 factory backend identity 守卫、CPU fill 的 clipped 斜边采样/开放子路径闭合、颜色通道、GDI damage 越界、首帧 LayerTree/Picture 回退、DisplayList glyph/clip、WGL core loader/caps、Win32 多窗状态串用、embedded build 子树删除与 UTF-16 代理对错误已用代码与守卫修复，未使用豁免。
...
- **P6 代码优先（[#169](decisions.md#d169)）**：可组合渲染轴落地 — 1) D3D11 `GpuNative` × `Swapchain` 垂直切片、完整 fill topology，以及遵循 `StrokeOptions` cap/join 的 identity path mesh ✅；2) 非 GL 原生光栅已收敛为 API-neutral `NativeGpuBackend` + `NativeRasterCaps` 逐操作路由，未支持操作确定性 soft fallback，普通 pipeline 不再下转 D3D11 具体类型 ✅；3) D3D12 device/queue/flip-discard swapchain/RTV/allocator-list/fence/readback/resize/shutdown context 基础 ✅，但仍为 `Planned`，尚未达到 clear + soft-blit hybrid 基线；4) `RasterMode` / `PresentMode` 类型与 caps + 表驱动装配、`BackendKind` 统一 ✅。下一代码切片为 D3D12 rounded solid + soft-blit 后接入 `NativeGpuBackend`，随后 Metal `GpuNative`。详见 [implementation · P6.8](areas/implementation.md#p68-可组合渲染轴) · [graphics-backend-pluggable · 可组合渲染轴](areas/systems/graphics-backend-pluggable.md#可组合渲染轴)。
...
- D3D11 identity fill/stroke path 子项已闭合；D3D12 context 正常与可 drain 故障路径的生命周期基础已闭合，持续同步失败时锁死并保守保活；下一 native raster 切片为 rounded solid + soft-blit + `NativeGpuBackend` 接入，随后 Metal `GpuNative`。
- 已废弃 bundled 管线枚举（`RenderPipelineProfile`）已随 P6.8 正交轴 refactor **删除**（[#169](decisions.md#d169)）；D3D11 `GpuNative` × `Swapchain` 与 identity fill/stroke path mesh ✅；D3D12 context foundation ✅ 但 registry 仍为 `Planned`，D3D12 draw/Metal `GpuNative` 仍 backlog。
