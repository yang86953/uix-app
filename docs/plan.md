...
| 验证基线 | lib 1085/1085、demo 19/19、doc 3/3；三目标 compile、Clippy 与严格文档检查通过 | [implementation · 当前验证基线](areas/implementation.md#当前验证基线-2026-07-10) |
...
- **P6 代码优先（[#169](decisions.md#d169)）**：可组合渲染轴落地 — 1) D3D11 `GpuNative` × `Swapchain` 垂直切片、完整 fill topology，以及遵循 `StrokeOptions` cap/join 的 identity path mesh ✅；2) 非 GL 原生光栅已收敛为 API-neutral `NativeGpuBackend` + `NativeRasterCaps` 逐操作路由，未支持操作确定性 soft fallback，普通 pipeline 不再下转 D3D11 具体类型 ✅；3) `RasterMode` / `PresentMode` 类型与 caps + 表驱动装配、`BackendKind` 统一 ✅。下一原生 raster backlog 为 D3D12 `GpuNative`（Windows 优先），随后 Metal `GpuNative`。详见 [implementation · P6.8](areas/implementation.md#p68-可组合渲染轴) · [graphics-backend-pluggable · 可组合渲染轴](areas/systems/graphics-backend-pluggable.md#可组合渲染轴)。
...
- D3D11 identity fill/stroke path 子项已闭合；下一 native raster 切片按 Windows 优先推进 D3D12 `GpuNative`，随后 Metal `GpuNative`。
- 已废弃 bundled 管线枚举（`RenderPipelineProfile`）已随 P6.8 正交轴 refactor **删除**（[#169](decisions.md#d169)）；D3D11 `GpuNative` × `Swapchain` 与 identity fill/stroke path mesh ✅；D3D12 / Metal `GpuNative` 仍 backlog。
