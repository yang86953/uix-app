...
|-------------|:-----:|:---------:|:--------:|:--------------:|:----------:|
| Windows D3D11 `GpuNative × Swapchain` | 是 | 单元/集成与 real-window factory + adapter identity + BGRA staging readback 测试通过 | default/no-default/all-features | RTX 4070 Ti SUPER hardware context；主副窗 light↔dark backbuffer GUI 通过；待前台 capture、输入/IME 与多 GPU/驱动矩阵 | 否 |
...
| 项 | 说明 |
|----|------|
| **D3D11 原生路径扩展** | 复杂 `fill_path`（嵌套/相交轮廓、孔洞、自交已有 soft fallback 拓扑守卫）与精确 `stroke_path` cap/join；简单 fill_path/stroke_path + fill+stroke+identity glyph atlas+linear/radial gradient+box/ambient shadow 已落地 |
