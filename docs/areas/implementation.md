...
| 能力 | 要点 | 文档 |
|------|------|------|
| GPU 引擎 | OpenGL ES / D3D11 / bounded D3D12 `GpuNative × Swapchain` → `GpuEngine`；Vulkan/Metal `Cpu × PixelUpload` → `PresentUploadEngine`；失败回退 SoftwareEngine | [rendering · 引擎](systems/rendering.md#引擎) · [#59](../decisions.md#d59) · [#172](../decisions.md#d172) |
...
| 焦点环 | `LayerTree` 绘制时将 focused `node_frame` 经 `cumulative_scroll` 映射到 viewport，并按祖先 `children_clip` 裁剪 | [rendering](systems/rendering.md) |
...
| 事件 | HandlerTable（`ComponentId` key）；PointerMove 窄路径；同父级跨节点文字拖选与复制聚合 | [event](systems/event.md) |
...
| Windows D3D12 bounded `GpuNative × Swapchain` | 是（`feature=d3d12`；registry `Active`、priority 5） | WARP real HWND 强制覆盖 context 生命周期、rounded/scissor/straight-alpha、BGRA row-pitch、多次 soft upload、premultiplied alpha，并由 `GpuEngine` 装配至共享 `NativeGpuBackend` 后手工编排 mixed submit/readback/present；Hardware 可用时在 context 层同测 rounded+soft | default、`--features d3d12`、`--no-default-features --features d3d12`；纯 `--no-default-features` 走 disabled stub | 当前主机 RTX 4070 Ti SUPER Hardware 与 Microsoft Basic Render Driver WARP 自动化通过；无完整 engine frame/present、前台/GUI/多驱动矩阵；Present、持续 device-loss/同步失败均锁死新工作并保守保活 | 否 |
...
| P0 | 全量测试恢复零失败 | `automated` 已完成 | lib 1097/1097、demo 19/19；历史布局回归、扫描假阳性、CPU fill 采样、D3D11 复杂拓扑与精确描边、D3D11/D3D12 `PresentFrame::Swapchain` 错误、D3D12 context + rounded/soft mixed 路径、native GPU capability/顺序/offset/RenderBackend 边界错误传播与重试/生命周期/registry/factory identity 守卫、颜色通道、GDI damage 越界、首帧 LayerTree/Picture 回退、DisplayList glyph/clip、WGL core loader/caps、Win32 多窗状态串用、embedded build 子树删除与 UTF-16 代理对错误均闭合 |
...
| P2 | D3D12 / Metal GPU 光栅 | 部分 ✅ | **增强**；D3D12 bounded clear/rounded solid + soft-blit 垂直切片 `Active`，其余能力按 caps 走 soft；随后 Metal |
...
| P6.8 可组合渲染轴 | `RasterMode` × `PresentMode` 类型与表驱动装配；`BackendKind` 统一；共享 `NativeGpuBackend` + 逐操作 `NativeRasterCaps`；D3D11 完整与 D3D12 bounded GpuNative | 部分 ✅ — D3D11 identity fill/stroke path、D3D12 clear/rounded solid + soft-blit/Active registry ✅；D3D12 扩展能力与 Metal GpuNative backlog — [#169](../decisions.md#d169) |
...
| P2 | D3D12 / Metal GPU 光栅 | 部分 ✅ | D3D12 bounded GpuNative 垂直切片已落地；下一步 Metal，D3D12 额外能力按后续需求扩展 |
...
| `src/native/graphics/*` | [platform](systems/platform.md) · [graphics-backend-pluggable](systems/graphics-backend-pluggable.md) | **#164** IGraphicsContext 对等 API 实现（vulkan / opengl / d3d11 / metal / d3d12 bounded native raster） |
