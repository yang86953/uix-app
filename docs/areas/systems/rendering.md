...
|--------|------------------|----------|--------------|------|
| **GpuEngine** | `GpuNative` × `Swapchain` | `RenderBackendRegistry` → `GpuBackend` / `NativeGpuBackend` | `swap_buffers` / swapchain | OpenGL ES ✅；D3D11 完整能力 ✅；D3D12 bounded clear/rounded solid + soft-blit ✅ |
...
|------|------|------|----------------|----------|
| **Windows** | Direct3D 11 | OpenGL ES（WGL） | ✅ D3D11 完整 `GpuNative` + OpenGL ES `GpuNative`；D3D12 bounded `GpuNative`（`feature=d3d12`，低优先级 Active） | ✅ SoftwareEngine（GDI） |
...
选型在 **窗口 / 引擎初始化时一次性完成**；**禁止**每帧探测或切换 API（#105）。probe 流程、Auto 链、配置优先级 → [graphics-backend-pluggable · 核心抽象](graphics-backend-pluggable.md#核心抽象) · [配置入口](#配置入口-p65-已落地)。D3D12 **native raster pipeline** 与 Metal **native raster** → [implementation · P6](../implementation.md#p6-生产级框架)。

### draw 域约束

- `draw` **不** `use native::backends::*` 或 `native::graphics::*`；仅 `IGraphicsContext` trait object。
- `GpuBackend` / `canvas_2d` 通过 `get_proc_address` 加载 GL 函数；Windows WGL 用隐藏 bootstrap context 加载 `wglChoosePixelFormatARB` / `wglCreateContextAttribsARB`，为真实 HWND 选择并校验 displayable 双缓冲格式，对 OpenGL 1.1 core export 回退 `opengl32.dll`，并只接受渲染器所需的 ES 3.0 context。OpenGL soft fallback 将 little-endian `AARRGGBB` 的 BGRA 内存字节按 GLES core RGBA 上传，再由 blit shader 交换 R/B；不得依赖可选 BGRA 扩展。`GpuBackend` 的 partial redraw 能力服从 native context 的 `partial_present`，不支持时提升为全帧，soft fallback 与 present 错误按 `Result` 向上传播。非 GL **GPU 光栅** backend 经 `RenderBackendRegistry` 注册（D3D11 完整能力 ✅；D3D12 bounded 能力 ✅）。当前 D3D11 swapchain 使用 `DXGI_SWAP_EFFECT_DISCARD`，present 后不保证 backbuffer 内容，因此 `NativeGpuBackend` 明确声明 `partial_redraw = false`，仅在既有 Active 帧内提升为全帧，不引入闲置绘制；真实窗口自动化通过 staging texture、row-pitch aware BGRA readback 验证首帧内容后再 present。D3D11 context 先请求 Hardware，失败时带原错误记录再尝试 WARP；adapter 诊断通过 `IDXGIDevice` / `IDXGIAdapter` 记录 driver kind、名称、vendor/device ID 与专用显存，禁止把 WARP 静默计入硬件证据；real-window 测试直接覆盖 Hardware 与 WARP 两种 driver。D3D11 identity `fill_path` 现于既有 flatten tolerance、数值容差与顶点/事件/三角/累计工作量预算内，以 strict contour-forest/earcut fast path + fill-rule-aware continuous y-band decomposition 支持严格 forest、相交轮廓、边界接触、自交与重合边，并保持 `EvenOdd` / `NonZero` 语义；identity `stroke_path` 完整覆盖 `StrokeOptions` cap/join并复用同一 tessellator。两类 path 均生成原生、triangle interior 不重复覆盖的 mesh，数值歧义、预算超限、不支持 transform/blend 或 tessellation 失败时确定性转 soft。真实 HWND 的完整 `Path → NativeGpuBackend → GpuSolidMesh` 像素回读现覆盖双孔/四层嵌套、重叠、自交、整边接触 fill 与半透明 90° Miter stroke，并验证完整像素数和单次 alpha 后 present。Vulkan/Metal 当前 caps 为 `Cpu` × `PixelUpload`；**下一 native raster 代码优先**是 Metal `GpuNative`。**非**「这些 API 只能 CPU 光栅」（#168）。
- D3D12 bounded `GpuNative` 在 `feature=d3d12` 下实现 context foundation、`SV_VertexID` rounded-solid PSO 与 BGRA8 premultiplied soft-blit，精确声明 `clear_target + solid_rects + soft_blit`，通过共享 `NativeGpuBackend`/`GpuEngine` 装配；未声明的 stroke/glyph/gradient/path/mesh/shadow/ellipse 等走 CPU soft 后全帧透明 overlay，`partial_present=false`。Windows Auto 数值优先级为 `D3D11(20) → OpenGL ES(10) → D3D12(5)`，避免 WARP 抢占既有硬件路径；关闭 feature 时公开 create 走 disabled stub。WARP real HWND 强制证据先由 `GpuEngine` 装配共享 backend，再由测试手工编排 mixed submit/readback/present；当前主机 Hardware 证据在 context 层完成同级 rounded+soft 回读与 Present。两者均不是完整 engine frame/present、前台/GUI 或多驱动矩阵。Present、持续 device-loss/同步失败均锁死新工作并保守保活，不是恢复。
- 非 GL `GpuNative` draw adapter 统一为 `NativeGpuBackend`：初始化时快照 `NativeRasterCaps`，每种 Canvas2D 操作在入队前裁决 native/soft；原生命令保持 Canvas 录制顺序，仅合并相邻同类/同 scissor batch，首次 soft 后同帧后续操作保持 soft，offset 只应用一次。构造器只接受 `GpuNative × Swapchain` 且具备 clear + soft-blit hybrid 基线的 context；context `Result` 错误传播到 `RenderBackend` 边界，native/soft 录制仅在最终 present 成功后提交清理，失败帧保留内容供下一次 present attempt 以 full-clear 完整重放，shutdown/Drop 幂等。该守卫不承诺 engine 自动调度恢复帧；现有 D3D11 legacy `swap_buffers` 的 DXGI Present 失败仍仅记录日志。`make_current` 与 DPR 经 `RenderBackend` 访问，所有 `BackendKind::Gpu` 创建统一经过 `RenderBackendRegistry`，普通 pipeline 不再 downcast D3D11 具体类型。factory 还会校验 registry row 与 live context 的 backend/raster/present identity，不匹配即 shutdown 并继续回退链。
