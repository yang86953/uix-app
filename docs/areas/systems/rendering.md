...
- 非 GL `GpuNative` draw adapter 统一为 `NativeGpuBackend`：初始化时快照 `NativeRasterCaps`，每种 Canvas2D 操作在入队前裁决 native/soft；原生命令保持 Canvas 录制顺序，仅合并相邻同类/同 scissor batch，首次 soft 后同帧后续操作保持 soft，offset 只应用一次。构造器只接受 `GpuNative × Swapchain` 且具备 clear + soft-blit hybrid 基线的 context；context `Result` 错误传播到 `RenderBackend` 边界，D3D11 / D3D12 的 `PresentFrame::Swapchain` 失败也会沿此路径返回，native/soft 录制仅在最终 present 成功后提交清理，失败帧保留内容供下一次 present attempt 以 full-clear 完整重放，shutdown/Drop 幂等。该守卫不承诺 engine 自动调度恢复帧。`make_current` 与 DPR 经 `RenderBackend` 访问，所有 `BackendKind::Gpu` 创建统一经过 `RenderBackendRegistry`，普通 pipeline 不再 downcast D3D11 具体类型。factory 还会校验 registry row 与 live context 的 backend/raster/present identity，不匹配即 shutdown 并继续回退链。
