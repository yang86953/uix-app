...
|--------|------------------|----------|--------------|------|
| **GpuEngine** | `GpuNative` × `Swapchain` | `RenderBackendRegistry` → `GpuBackend` / `NativeGpuBackend` | `swap_buffers` / swapchain | OpenGL ES ✅；D3D11 ✅（`NativeRasterCaps` 全能力 + soft blit） |
