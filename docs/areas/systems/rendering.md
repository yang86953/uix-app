...
|--------|------------------|----------|--------------|------|
| **GpuEngine** | `GpuNative` × `Swapchain` | `RenderBackendRegistry` → `GpuBackend` / `NativeGpuBackend` | `swap_buffers` / swapchain | OpenGL ES ✅；D3D11 ✅（`NativeRasterCaps` 全能力 + soft blit）；D3D12 仅 context foundation，尚未装配 engine |
...
|------|------|------|----------------|----------|
| **Windows** | Direct3D 11 | OpenGL ES（WGL） | ✅ D3D11（`GpuNative` × `Swapchain`，原生 fill/stroke/glyph）+ OpenGL ES（`GpuNative` × `Swapchain`，WGL）；D3D12 context foundation（`feature=d3d12`）✅、draw/registry 规划中 | ✅ SoftwareEngine（GDI） |
