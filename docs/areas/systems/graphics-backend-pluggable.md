...
> **实现状态**：P6.7 registry / probe / 统一 present ✅ · P6.8 正交轴类型与表驱动装配 ✅ · D3D11 `GpuNative` × `Swapchain` ✅（`fill_rect`/`fill_circle`/`stroke_rect`/`stroke_circle` + 轴对齐 `draw_line` + identity solid `blit_glyph` 字形 atlas；path/gradient/斜线等仍 soft + alpha blit）（[#169](../../decisions.md#d169)）→ [P6.8](../implementation.md#p68-可组合渲染轴)。非 P6 任务不必通读。
...
¹ D3D11：device/swapchain/RTV + 原生 solid/rounded fill + stroke（VS/PS SDF）+ identity solid glyph atlas（CPU coverage → R8 atlas → textured quads）+ soft 回退 alpha blit + present；path/gradient/斜线/非 identity 文本等仍 soft。  
² D3D11 优先路径已切到 `GpuNative` × `Swapchain`；`present_pixels` / `upload_surface_pixels` 仍保留作低层测试 helper，主路径不再全帧 upload。

**下一代码优先**（[#167](../../decisions.md#d167) [#169](../../decisions.md#d169)）：D3D11 原生路径扩展（gradient；path）；随后 Metal / D3D12 `GpuNative`。
...
| D3D11 `GpuNative` × `Swapchain` | ✅ 原生 fill/stroke rect + circle + 轴对齐 line + identity solid glyph atlas + soft blit；path/gradient/非 identity 文本仍 soft |
