...
| open_window | 副窗 + 新 AppHandle | `WindowConfig` / `AppHandle::open_window` / `.on_window_start` 已导出；运行时可分配新 `window_id`、独立 AppTimer/MainThreadQueue/AppHandle 并暂存副窗创建请求；GUI loop 会在首窗启动后与活动轮次中 drain 请求，创建 native 窗与独立 `WindowSession` bootstrap；副窗事件路由/帧循环消费待接 | [#144](decisions.md#d144) [#148](decisions.md#d148) |
...
| update_view | AppHandle 按 session reconcile | `AppHandle::update_view` / `set_root` 已导出，经 `AppRuntime` 按 `window_id` 写入目标 MainThreadQueue；单窗 loop 已消费，副窗 session bootstrap 已接但副窗帧循环消费待接 | [#149](decisions.md#d149) |
...
| App Timer API | run_after / run_interval | `App` / `AppHandle` 的 `run_after` / `run_interval` / `TimerHandle` 已导出；`AppHandle` 经 `AppRuntime` 按 `window_id` 路由到所属 session Timer；副窗 session bootstrap 已接但副窗帧循环消费待接 | [#132](decisions.md#d132) |
| post_to_ui | App / AppHandle 主线程投递 | `App::post_to_ui` + `AppHandle::post_to_ui` + MainThreadQueue 已接；`AppHandle` 经 `AppRuntime` 按 `window_id` 路由；阻塞等待 wake 与副窗帧循环消费待接 | [#133](decisions.md#d133) |
| MainThreadQueue | FIFO + 帧内 drain 顺序 | 每 WindowSession 队列已接；drain 顺序为 UiEvent → due work → post_to_ui；`AppHandle` 多窗队列路由与副窗 bootstrap 已接，副窗 loop 消费待接 | [#137](decisions.md#d137) |
