...
| Handle emit / invalidate / getter | dispatch_semantic + 窄 Paint + 只读配置 | `ComponentHandle::emit` 已导出并走 `WidgetTree::dispatch_semantic`；`invalidate()` 已接 `WidgetTree::invalidate_paint` 窄 Paint；`snapshot()` / `text()` / `placeholder()` / `disabled()` 已接；App 默认持有同一 `AppState` 并注入主窗与副窗 `WindowSession`；`AppState::get_handle` 可查 snapshot handle；lookup handle 的 live tree 绑定待接 | [#119](decisions.md#d119) [#147](decisions.md#d147) |
