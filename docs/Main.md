...
| AppState / ComponentHandle | mount 自动 register | `AppState` snapshot registry 已接入 `WidgetTree::set_app_state` mount/unmount，主窗与副窗共享同一 AppState；lookup handle 可读 snapshot、窄 Paint invalidate，并通过 AppState semantic queue 触发 emit | [application · AppState](systems/application.md#appstate--多窗--settings) · [#145](decisions.md#d145) |
