...
| 能力 | 设计 | 当前 | 文档 |
|------|------|------|------|
| ComponentId | Generational 稳定 ID | `core::ComponentId` 已落地并从 `ui::ComponentId` 与 `prelude` 重导出；WidgetTree 内部 `WidgetId` 与 draw 内部 `NodeId` 为源码别名；`SemanticEvent`、`EventHandler::semantic_event`、`HandlerTable` 存储/API、Reconciler 热路径、State paint binding、Widget trait 边界、managers 与 layout implementor 已按 `ComponentId` 命名并覆盖 generation key 测试 | [component · WidgetTree](systems/component.md#widgettree) · [#10](decisions.md#d10) [#101](decisions.md#d101) |
