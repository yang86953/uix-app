...
| 网络、同步与凭据管理不在当前 `data` 范围，扩展前需要新的产品边界决策。 | 高 | [plan.md · 开放问题](plan.md#开放问题)；新增 #176+ 决策。 |
...
当前证据（2026-07-10，基于当前工作树）：`cargo test --all-targets` 为 lib **1070/1070**、demo **19/19**；Windows default/no-default/all-features 与 Linux/macOS default/all-features `cargo check` 全部通过；`cargo clippy --all-targets`、严格文档检查通过。FrameRenderer 的版本 `0` 首帧与无 offscreen Picture 直绘回退、embedded component build 子树的 reconcile 保留、DisplayList glyph/clip 与未完整录制回退均有自动化守卫，并经 SoftwareEngine GUI 首帧、按钮 `0→1`、Input placeholder、`abc` 提交、运行时全局 light/dark 及真实双窗口双向主题联动复验；WGL real-window 自动化已覆盖 ES 3 engine 初始化、红色像素绘制/readback 与 present 调用。Windows D3D11 与无 GPU feature 的 SoftwareEngine 已完成首帧、颜色、按钮点击、最大化/恢复和关闭基础 GUI smoke；D3D11 真实窗口自动化还覆盖 BGRA staging readback，discard swapchain 已按全帧契约运行。Win32 双真实窗口的独立状态、resize 事件路由与非全局退出，以及 native IME composition/UTF-16/candidate rect、Input 焦点会话和预编辑/缓存像素已有自动化守卫。Microsoft Pinyin 真机已验证候选窗跟随 caret，但其 IMM32 `GCS_COMPSTR` 在当前系统只提供空白占位，应用内 phonetic preedit 仍需 TSF/UI-less text store。详见 [implementation · 当前验证基线](areas/implementation.md#当前验证基线-2026-07-10)。自动化/编译 gate 已恢复，但 OpenGL ES 屏幕呈现、TSF phonetic preedit、D3D11 主题/多窗联合 GUI、硬件矩阵与 P0 生产阻塞清零仍未完成，因此 P6 尚未达到 `production`。
...
| 4. P7 移动端 spike 与切片 | 验证 traits 对移动生命周期、surface 和输入足够 | iOS/Android 最小窗口+事件+present 原型；缺口决策 #176+ |
...
| 网络/同步属于 `data` 扩展还是新域？ | 会改变依赖图、安全边界和公开 API | 当前不纳入；形成独立需求与 #176+ 决策后再设计 | 主人 |
...
| 日期 | 变更 | 原因 | 后续 |
|------|------|------|------|
| 2026-07-10 | 新增项目级总设计、需求追踪、替代方案、质量属性、rollout、风险与开放问题。 | 原文档已有完整系统细节，但缺少跨系统 `design.md` 与独立需求 → 设计 → 验证链。 | 以 [plan.md](plan.md) 推进 P6；新事实同步 requirements/design/implementation，持久取舍写入 #176+。 |
