...
| Active / 动画中 | `tree.update` 返回 true → Registry 登记下一帧 deadline | Active 帧运行 `update`，且仅在 Effect pending 时运行 `tick_effects`；`Spin` / `ProgressBar` indeterminate / Tooltip fade / Modal / Drawer 已作为内置 Animation 源接入，其他过渡动画源待接 |
| 首帧 | 单次 `poll_event` | 同左 |
