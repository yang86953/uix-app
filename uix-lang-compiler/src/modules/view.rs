...
impl Checker<'_> {
    ...
    fn node(
        ...
        let (kind, allowed, defaults): (ViewKind, &[&str], &[(&str, Value)]) =
            match element.name.as_str() {
                "Column" => (
                    ViewKind::Column,
                    &["key", "gap", "padding"],
                    &[("gap", Value::Float(8.0)), ("padding", Value::Float(12.0))],
                ),
                "Row" => (
                    ViewKind::Row,
                    &["key", "gap", "padding"],
                    &[("gap", Value::Float(8.0)), ("padding", Value::Float(0.0))],
                ),
                "Text" => (
                    ViewKind::Text,
                    &["key", "text"],
                ...
                "Input" => (
                    ViewKind::Input,
                    &["key", "value", "placeholder", "onChange"],
                    &[
                ...
                _ => {
                    return Err(fail(
                        element.span,
                        format!("模块 View 暂不支持控件 {}", element.name),
                    ));
                }
            };
        ...
        let event = if kind == ViewKind::Input {
            "onChange"
        } else {
            "onClick"
        };
        ...
        if !matches!(kind, ViewKind::Column | ViewKind::Row) && !children.is_empty() {
