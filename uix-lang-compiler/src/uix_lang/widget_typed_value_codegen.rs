...
impl WidgetExpander {
    ...
    pub(super) fn complex_typed_initial_tokens(
        ...
        let initial = match value_type {
            ...
            WidgetValueType::CascaderValue | WidgetValueType::Record(_) => {
                // 生成结构体字面量。
    ...
    fn typed_value_tokens(
        ...
        match value_type {
            ...
            | WidgetValueType::VecOfUploadFile
            | WidgetValueType::CascaderValue => self
                // 生成复合初始值并丢弃重复类型令牌。
    ...
    fn struct_target_fields(
        ...
        match value_type {
            ...
            WidgetValueType::CascaderValue => Ok((
                ...
                vec![
                    // 标签路径字段。
                    ("labels".to_string(), WidgetValueType::VecOfString),
                    // 值路径字段。
                    ("values".to_string(), WidgetValueType::VecOfString),
                ],
            )),
            // record 使用文档声明的字段集合。
