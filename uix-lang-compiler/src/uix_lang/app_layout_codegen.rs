...
pub(crate) fn generate_app_layout(element: &Element) -> Result<TokenStream, Diagnostic> {
    ...
    let (widget, consumed) = match element.name.as_str() {
        ...
        // Header 使用文档锚点默认高度。
        "Header" => (quote! { ::uix_app::prelude::Header::default() }, &[][..]),
        // Content 默认占用纵向 Layout 的剩余空间。
        "Content" => (quote! { ::uix_app::prelude::Content::new() }, &[][..]),
        // Footer 使用与 Header 对称的默认高度。
        "Footer" => (quote! { ::uix_app::prelude::Footer::default() }, &[][..]),
...
fn generate_layout(element: &Element) -> Result<TokenStream, Diagnostic> {
    // 从公开默认纵向布局壳开始。
    let mut layout = quote! { ::uix_app::prelude::Layout::new() };
    // 可选方向必须在编译期决定布局主轴。
    if let Some(attribute) = find_attribute(element, "direction") {
        ...
        layout = match direction.as_str() {
            ...
            "row" => quote! {
                // 调用公开 Layout 方向构建器。
                (#layout).direction(::uix_app::prelude::FlexDirection::Row)
            },
            // column 显式保持 Header / Content / Footer 的纵向结构。
            "column" => quote! {
                // 调用公开 Layout 方向构建器。
                (#layout).direction(::uix_app::prelude::FlexDirection::Column)
            },
            // 未登记方向不得静默近似。
            _ => {
                // 返回来源精确的方向诊断。
...
fn generate_sider(element: &Element) -> Result<TokenStream, Diagnostic> {
    // 从二百逻辑像素文档默认侧栏开始。
    let mut sider = quote! { ::uix_app::prelude::Sider::default() };
