...
// 判断文本内容是否包含至少一个动态插值。
pub(super) fn has_interpolation(children: &[Node]) -> bool {
    // 只检查直接文本内容节点；嵌套元素由共享文本生成器拒绝。
    children
        // 遍历全部直接子节点。
        .iter()
        // 任意插值都需要 DynamicLabel。
        .any(|child| matches!(child, Node::Interpolation(_)))
}

// 生成拥有捕获且在运行时延迟求值的 DynamicLabel。
pub(super) fn generate_dynamic_label(
    // 接收源码顺序中的文本与插值。
    children: &[Node],
    // 接收所属 Text 元素跨度。
    span: SourceSpan,
