// ============================================================================
// platform/core/window.rs — 窗口实现共享层
//
// WindowOps trait：平台只需实现 6 个必须方法，其余 20 个有合理默认实现。
//
// 必须实现（6 个）：
//   os_show, os_hide, os_close, os_set_title, os_set_size, native_handle
//
...
/// ## 必须实现（6 个）
///
/// 这些是任何 GUI 窗口都需要的核心操作——无默认实现，编译期强制要求：
/// - `os_show` — 显示窗口
/// - `os_hide` — 隐藏窗口
/// - `os_close` — 关闭窗口
/// - `os_set_title` — 设置窗口标题
/// - `os_set_size` — 设置窗口尺寸
/// - `native_handle` — 返回原生窗口句柄
///
/// ## 可选实现（运行时报错）
///
/// 其余方法有默认实现，但调用时会输出 ERROR 级别日志。
/// 新平台可先跑起来再逐步覆盖。
pub trait WindowOps {
    // ── 必须实现（无默认，编译期强制）───────────────────
...
impl<O: WindowOps> PlatformWindow for PlatformWindowCore<O> {
    ...
    fn resize_notify(&mut self, width: i32, height: i32) {
        ...
        // 通知呈现器尺寸变化（GdiPresenter 需立即重建 DIB，
        // WaylandPresenter 的惰性 resize 在 present() 中执行）。
        if let Err(e) = self.presenter.resize(width, height) {
            log::warn!("resize_notify: presenter.resize({}, {}) failed: {}", width, height, e.short_what());
        }
    }
