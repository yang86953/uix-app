...
impl WaylandBackend {
    ...
    fn generate_key_repeats(&mut self) {
        let (_linux_key, code, mods, first_press) = match self.held_key_info.lock()
        ...
        let should_fire = match *self.last_repeat_time.lock().unwrap_or_else(|e| e.into_inner()) {
            Some(last) => now.duration_since(last) >= interval,
            None => now >= first_press + delay,
        };
        if !should_fire { return; }

        *self.last_repeat_time.lock().unwrap_or_else(|e| e.into_inner()) = Some(now);
