...
class GraphicsRhiLayeringContractTests(unittest.TestCase):
    ...
    def test_submission_sequence_is_shared_by_both_adapters(self) -> None:
        ...
        rhi = (ROOT / "src/native/present/rhi.rs").read_text(encoding="utf-8")
        # 读取 API 无关提交序列契约。
        submission = (ROOT / "src/native/present/rhi/submission.rs").read_text(encoding="utf-8")
        ...
        d3d11_submit = (ROOT / "src/native/presentation/graphics/d3d11/platform/context/rhi_device_submit.rs").read_text(encoding="utf-8")
    ...
    def test_device_and_surface_capabilities_are_orthogonal(self) -> None:
        ...
        adapter_factories = "\n".join((ROOT / path).read_text(encoding="utf-8") for path in ("src/native/presentation/graphics/d3d11/platform/mod.rs", "src/native/presentation/graphics/opengl/platform/mod.rs"))
    ...
    def test_gradient_uniform_and_alpha_semantics_are_shared(self) -> None:
        # 读取共享 Gradient 值对象。
        gradient = (ROOT / "src/native/present/rhi/gradient.rs").read_text(encoding="utf-8")
        ...
        d3d11_shader = (ROOT / "src/native/presentation/graphics/d3d11/platform/pipeline/mod.rs").read_text(encoding="utf-8")
    ...
    def test_shape_shader_mask_has_one_owner_in_each_adapter(self) -> None:
        ...
        d3d11 = (ROOT / "src/native/presentation/graphics/d3d11/platform/pipeline/mod.rs").read_text(encoding="utf-8")
    ...
    def test_shadow_uniform_and_resources_are_shared_and_typed(self) -> None:
        # 读取共享 Shadow 值对象。
        shadow = (ROOT / "src/native/present/rhi/shadow.rs").read_text(encoding="utf-8")
    ...
    def test_blur_uniform_is_shared_typed_and_complete(self) -> None:
        # 读取共享 Blur 值对象。
        blur = (ROOT / "src/native/present/rhi/blur.rs").read_text(encoding="utf-8")
    ...
    def test_msdf_uniform_is_shared_typed_and_api_neutral(self) -> None:
        # 读取共享 MSDF 值对象。
        msdf = (ROOT / "src/native/present/rhi/msdf.rs").read_text(encoding="utf-8")
        ...
        d3d11_shader = (ROOT / "src/native/presentation/graphics/d3d11/platform/pipeline/msdf_shader.rs").read_text(encoding="utf-8")
        # 共享层必须拥有完整三十二字节值对象。
    ...
    def test_basic_primitive_uniforms_are_shared_and_typed(self) -> None:
        # 读取基础图元共享值对象。
        primitive = (ROOT / "src/native/present/rhi/primitive.rs").read_text(encoding="utf-8")
    ...
    def test_pipeline_binding_closes_frame_plan_layout_contract(self) -> None:
        ...
        draw_packet = (ROOT / "src/native/present/rhi/draw_packet.rs").read_text(encoding="utf-8")
    ...
    def test_texture_transfer_contract_is_shared_and_top_left(self) -> None:
        ...
        rhi = (ROOT / "src/native/present/rhi.rs").read_text(encoding="utf-8")
        # 读取共享纹理传输契约。
        transfer = (ROOT / "src/native/present/rhi/transfer.rs").read_text(encoding="utf-8")
        # 读取 OpenGL 普通复制实现。
    ...
    def test_color_transfer_and_blend_domain_are_shared(self) -> None:
        # 读取共享颜色契约。
        color = (ROOT / "src/native/present/rhi/color.rs").read_text(encoding="utf-8")
        ...
        wgl = (ROOT / "src/native/presentation/graphics/opengl/platform/wgl.rs").read_text(encoding="utf-8")
        # 读取 D3D11 texture 格式映射。
        d3d11_texture = (ROOT / "src/native/presentation/graphics/d3d11/platform/context/rhi_device.rs").read_text(encoding="utf-8")
        # 读取 D3D11 swapchain 格式映射。
        d3d11_surface = (ROOT / "src/native/presentation/graphics/d3d11/platform/swapchain.rs").read_text(encoding="utf-8")
    ...
    def test_clear_rect_reuses_shared_target_geometry_contract(self) -> None:
        ...
        # 读取 D3D11 的局部清屏翻译。
        d3d11 = (ROOT / "src/native/presentation/graphics/d3d11/platform/context/rhi_device_clear.rs").read_text(encoding="utf-8")
        # 共享状态机必须拥有唯一目标边界判定。
    ...
    def test_clear_color_premultiplication_has_one_shared_owner(self) -> None:
        ...
        d3d11 = (
            # 合并完整 pass 清理实现。
            (ROOT / "src/native/presentation/graphics/d3d11/platform/context/rhi_device.rs").read_text(encoding="utf-8")
            # 合并局部清理实现。
            + (ROOT / "src/native/presentation/graphics/d3d11/platform/context/rhi_device_clear.rs").read_text(encoding="utf-8")
        )
