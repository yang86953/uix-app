...
fn aot_and_dynamic_text_processing_have_identical_results_state_and_effect_order() {
    ...
    for module in [native, dynamic] {
        ...
        let mut instance = Instance::new(
            module.clone(),
            ports(&module, log.clone()),
            Limits::default(),
        )
        .expect("实例");
        let id = Value::String("doc-1".into());
        assert_eq!(
            instance.call("read", &[id.clone()]).expect("读取"),
            Value::String("原始 文档".into())
        );
        let mut results = Vec::new();
        for input in ["", "   ", "hello", " hello  世界 \nnext "] {
            results.push(
                instance
                    .call("process", &[Value::String(input.into())])
                    .expect("处理"),
            );
        }
        assert_eq!(
            ...
            Value::Record(BTreeMap::from([
                ("characters".into(), Value::Int(0)),
                ("words".into(), Value::Int(0)),
                ("normalized".into(), Value::String(String::new()))
            ]))
        );
        assert_eq!(
            instance.call("current", &[]).expect("状态"),
            Value::String("hello 世界 next".into())
        );
        assert_eq!(
            instance.call("commit", &[id]).expect("提交"),
            Value::Bool(true)
        );
        observed.push((
            results,
...
#[test]
fn async_boundaries_reject_implicit_or_synchronous_waiting_in_both_entries() {
    for declaration in [
        r#"<Port name="read" returns="String" effect="query" async="sometimes" />"#,
        r#"<Port name="read" returns="String" effect="query" async="true" />
            <Function name="run" returns="String" body="read()" />"#,
        r#"<Port name="read" returns="String" effect="query" async="true" />
            <Query name="run" returns="String" body="read()" />"#,
        ...
        let source = format!(
            r#"<Module name="AsyncBoundary" version="1" schema="1">{declaration}</Module>"#
        );
        let dynamic = load_module(&source, "async-boundary.uix").expect_err("必须显式使用异步合同");
        let aot = uix_lang_compiler::modules::compile_inline(&source, "async-boundary.uix")
            .expect_err("AOT 不得把异步入口降级为同步");
...
fn generated_aot_and_dynamic_signatures_keep_the_synchronous_contract() {
    for module in [
        ...
        assert!(
            module
                .functions
                .iter()
                .all(|function| !function.signature.asynchronous)
        );
    ...
    let ports = BTreeMap::from([(
        ...
        HostPort {
            signature,
            callback: Arc::new(|_| Ok(Value::Int(42))),
        },
    )]);
...
fn failures_rollback_state_and_keep_checked_arithmetic_and_short_circuit() {
    ...
    assert_eq!(
        instance.call("divide", &[Value::Int(4)]).expect("成功"),
        Value::Int(25)
    );
    assert_eq!(instance.state()["count"], Value::Int(1));
    assert_eq!(
        instance
            .call("overflow", &[Value::Int(i64::MAX)])
            .expect_err("溢出")
            .kind,
        ErrorKind::Arithmetic
    );
    assert_eq!(
        instance.call("short_circuit", &[]).expect("短路"),
        Value::Bool(false)
    );
    assert_eq!(
        instance
...
fn replacement_revocation_and_close_have_observable_terminal_results() {
    ...
    let mut instance = Instance::new(
        module.clone(),
        ports(&module, Arc::new(Mutex::new(Vec::new()))),
        Limits::default(),
    )
    .expect("实例");
    instance
        .call("process", &[Value::String("old draft".into())])
        .expect("初始业务");
    let generation = instance.generation();
    let next_source = TEXT
        .replace("version=\"1.0.0\"", "version=\"2.0.0\"")
        .replace("text.words().join(' ')", "text.words().join('-')");
    let next = load_module(&next_source, "bench-v2.uix").expect("新代");
    assert_eq!(
        instance
            .replace(next.clone(), generation + 1)
            .expect_err("陈旧代")
            .kind,
        ErrorKind::Conflict
    );
    ...
    instance
        .call("process", &[Value::String("new text".into())])
        .expect("升级算法");
    assert_eq!(instance.state()["draft"], Value::String("new-text".into()));
    let incompatible = load_module(
        &next_source.replace("schema=\"1\"", "schema=\"2\""),
        "incompatible.uix",
    )
    .expect("候选");
    assert_eq!(
        instance
            .replace(incompatible, instance.generation())
            .expect_err("schema 不符")
            .kind,
        ErrorKind::Conflict
    );
    instance.revoke();
    assert_eq!(
        instance.call("current", &[]).expect_err("撤权").kind,
        ErrorKind::CapabilityDenied
    );
    instance.close();
    assert!(instance.state().is_empty());
    assert_eq!(
        instance.call("current", &[]).expect_err("关闭").kind,
        ErrorKind::Closed
    );
}
...
fn cancellation_quotas_arguments_and_instance_isolation_are_enforced() {
    let module = load_module(TEXT, "bench.uix").expect("装载");
    assert!(matches!(
        Instance::new(module.clone(), BTreeMap::new(), Limits::default()),
        Err(RuntimeError {
    ...
    let mut first =
        Instance::new(module.clone(), ports.clone(), Limits::default()).expect("实例一");
    let second = Instance::new(module.clone(), ports.clone(), Limits::default()).expect("实例二");
    let cancellation = Cancellation::default();
    cancellation.cancel();
    assert_eq!(
        first
            .call_with_cancellation("process", &[Value::String("text".into())], &cancellation)
            .expect_err("取消")
            .kind,
        ErrorKind::Cancelled
    );
    ...
    assert_eq!(
        limited
            .call("process", &[Value::String("text".into())])
            .expect_err("配额")
            .kind,
        ErrorKind::Quota
    );
