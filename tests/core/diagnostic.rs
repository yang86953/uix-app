...
fn collector_sequential() {
    ...
    assert!(c.total_collected() >= before + 4);
...
fn fatal_sequential() {
    ...
    assert!(
        !found.is_empty(),
        "collect_or_abort should have collected the error with expected message"
    );

    // install_fatal_handler 会安装全局 panic hook 并 abort 进程，
    // 与 try_invoke 等 catch_unwind 测试不兼容，故不在集成测试套件中调用。
}
...
// fatal_collect_or_abort 已合并至 fatal_sequential，避免 Collector 单例并行污染。
