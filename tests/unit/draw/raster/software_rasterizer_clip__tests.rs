...
fn fixed_surface_clip_starts_without_stack_allocation() {
    let rasterizer =
        SoftwareRasterizer::new_with_surface_clip(32, 24, Rect::new(2.0, 3.0, 12.0, 10.0));
...
fn reset_for_extent_releases_oversized_transient_stack() {
    let mut rasterizer = SoftwareRasterizer::new(16, 16);
    let excessive_depth = MAX_RETAINED_TRANSIENT_STACK_BYTES / std::mem::size_of::<Rect>() + 1;
