...
#[test]
fn radio_uses_colocated_uix_visual() {
    // 构建前不得复制第二份默认视觉表。
    let radio = Radio::default();
    assert!(ptr::eq(radio.visual, RADIO_VISUAL_REF));
    // UIX 注入后的叶内核仍指向同一静态记录。
    let node = View::build(radio);
    let radio = node
        .widget
        .as_any()
        .downcast_ref::<Radio>()
        .expect("UIX 根应保留 Radio 内核");
    assert!(ptr::eq(radio.visual, RADIO_VISUAL_REF));
}
...
fn fractional_position_records_concentric_indicator_circles() {
    ...
    // 与真实渲染入口一致，每帧只解析一次主题角色。
    let visual = radio.visual.resolve(tokens.as_ref());
    ...
    let circle_centers = list
        ...
        .take(3)
        .map(|op| match op {
            PaintOp::StrokeCircle { cx, cy, .. } | PaintOp::FillCircle { cx, cy, .. } => (*cx, *cy),
