...
class ResolvedGraphTests(unittest.TestCase):
    ...
    def test_rich_text_scenarios_bind_feature_and_public_api_guards(self) -> None:
        ...
        self.assertEqual(
            disabled.forbidden_uix_features,
            ("charts", "feedback", "navigation", "rich-text", "table", "tree-widgets"),
        )
        # 负向入口必须以公开导入失败结束。
        self.assertTrue(disabled.expected_compile_failure)
        # 诊断必须同时覆盖组件类型和解析辅助函数。
        ...
        # 每个场景都必须对无专属 package 的 rich-text feature 作正向或负向断言。
    ...
    # 确认图表正反场景同时覆盖纯源码 feature 与基础/高级公开面收缩。
    def test_charts_scenarios_bind_feature_and_public_api_guards(self) -> None:
        # 读取当前仓库的全部场景定义。
        scenarios = measure_usage_build.scenario_specs(measure_usage_build.project_root())
        # 按稳定名称索引场景。
        by_name = {scenario.name: scenario for scenario in scenarios}
        # 读取只启用图表能力的正向入口。
        enabled = by_name["charts"]
        # 图表没有专属第三方 package，正向证据必须绑定根 feature。
        self.assertEqual(enabled.required_packages, ())
        # metadata 必须证明只选择 charts 源码 capability。
        ...
        self.assertEqual(
            disabled.forbidden_uix_features,
            ("charts", "feedback", "navigation", "table", "rich-text", "tree-widgets"),
        )
        # 负向入口必须以公开导入失败结束。
        self.assertTrue(disabled.expected_compile_failure)
        # 诊断必须同时覆盖基础图表和高级图表类型。
        self.assertEqual(
            disabled.expected_error_fragments,
            ("unresolved import", "BarChart", "Gauge"),
        )
        # 每个场景都必须对无专属 package 的 charts feature 作正向或负向断言。
        for scenario in scenarios:
            # 防止新增场景遗漏纯源码 capability 的解析图边界。
            self.assertIn(
                "charts",
    ...
    # 确认导航正反场景同时覆盖纯源码 feature 与基础/泛型公开面收缩。
    def test_navigation_scenarios_bind_feature_and_public_api_guards(self) -> None:
        # 读取当前仓库的全部场景定义。
        scenarios = measure_usage_build.scenario_specs(measure_usage_build.project_root())
        # 按稳定名称索引场景。
        by_name = {scenario.name: scenario for scenario in scenarios}
        # 读取只启用导航能力的正向入口。
        enabled = by_name["navigation"]
        # 导航没有专属第三方 package，正向证据必须绑定根 feature。
        self.assertEqual(enabled.required_packages, ())
        # metadata 必须证明只选择 navigation 源码 capability。
        self.assertEqual(enabled.required_uix_features, ("navigation",))
        # 单导航入口不得合并图表源码能力。
        self.assertIn("charts", enabled.forbidden_uix_features)
        # 单导航入口不得合并表格源码能力。
        self.assertIn("table", enabled.forbidden_uix_features)
        # 读取关闭导航能力的负向入口。
        disabled = by_name["navigation-disabled"]
        # 负向入口必须禁止 navigation feature 意外进入解析图。
        self.assertEqual(
            disabled.forbidden_uix_features,
            ("charts", "feedback", "navigation", "table", "rich-text", "tree-widgets"),
        )
        # 负向入口必须以公开导入失败结束。
        self.assertTrue(disabled.expected_compile_failure)
        # 诊断必须同时覆盖基础面包屑和泛型导航容器类型。
        self.assertEqual(
            disabled.expected_error_fragments,
            ("unresolved import", "Breadcrumb", "Navigation"),
        )
        # 每个场景都必须对无专属 package 的 navigation feature 作正向或负向断言。
        for scenario in scenarios:
            # 防止新增场景遗漏纯源码 capability 的解析图边界。
            self.assertIn(
                "navigation",
    ...
    # 确认反馈正反场景同时覆盖纯源码 feature、组件族与全局门面收缩。
    def test_feedback_scenarios_bind_feature_and_public_api_guards(self) -> None:
        # 读取当前仓库的全部场景定义。
        scenarios = measure_usage_build.scenario_specs(measure_usage_build.project_root())
        # 按稳定名称索引场景。
        by_name = {scenario.name: scenario for scenario in scenarios}
        # 读取只启用反馈能力的正向入口。
        enabled = by_name["feedback"]
        # 反馈没有专属第三方 package，正向证据必须绑定根 feature。
        self.assertEqual(enabled.required_packages, ())
        # metadata 必须证明只选择 feedback 源码 capability。
        self.assertEqual(enabled.required_uix_features, ("feedback",))
        # 单反馈入口不得合并图表源码能力。
        self.assertIn("charts", enabled.forbidden_uix_features)
        # 单反馈入口不得合并导航源码能力。
        self.assertIn("navigation", enabled.forbidden_uix_features)
        # 读取关闭反馈能力的负向入口。
        disabled = by_name["feedback-disabled"]
        # 负向入口必须禁止 feedback feature 意外进入解析图。
        self.assertEqual(
            disabled.forbidden_uix_features,
            ("charts", "feedback", "navigation", "table", "rich-text", "tree-widgets"),
        )
        # 负向入口必须以公开导入失败结束。
        self.assertTrue(disabled.expected_compile_failure)
        # 诊断必须同时覆盖基础组件、弹层组件与两个全局门面函数。
        self.assertEqual(
            disabled.expected_error_fragments,
            ("unresolved import", "Alert", "Modal", "message", "notify"),
        )
        # 每个场景都必须对无专属 package 的 feedback feature 作正向或负向断言。
        for scenario in scenarios:
            # 防止新增场景遗漏纯源码 capability 的解析图边界。
            self.assertIn(
                "feedback",
    ...
    # 确认树组件正反场景同时覆盖纯源码 feature、组件族与快照公开面收缩。
    def test_tree_widget_scenarios_bind_feature_and_public_api_guards(self) -> None:
        # 读取当前仓库的全部场景定义。
        scenarios = measure_usage_build.scenario_specs(measure_usage_build.project_root())
        # 按稳定名称索引场景。
        by_name = {scenario.name: scenario for scenario in scenarios}
        # 读取只启用树组件能力的正向入口。
        enabled = by_name["tree-widgets"]
        # 树组件没有专属第三方 package，正向证据必须绑定根 feature。
        self.assertEqual(enabled.required_packages, ())
        # metadata 必须证明只选择 tree-widgets 源码 capability。
        self.assertEqual(enabled.required_uix_features, ("tree-widgets",))
        # 单树组件入口不得合并既有纯源码能力。
        self.assertTrue(
            {"charts", "feedback", "navigation", "rich-text", "table"}.issubset(
                enabled.forbidden_uix_features
            )
        )
        # 读取关闭树组件能力的负向入口。
        disabled = by_name["tree-widgets-disabled"]
        # 负向入口必须禁止全部纯源码 capability 意外进入解析图。
        self.assertEqual(
            disabled.forbidden_uix_features,
            ("charts", "feedback", "navigation", "rich-text", "table", "tree-widgets"),
        )
        # 负向入口必须以公开导入失败结束。
        self.assertTrue(disabled.expected_compile_failure)
        # 诊断必须同时覆盖树、树节点、树选择器、拖放位置与快照节点。
        self.assertEqual(
            disabled.expected_error_fragments,
            ("unresolved import", "DropPosition", "SnapshotTreeNode", "Tree", "TreeSelect"),
        )
        # 每个场景都必须对无专属 package 的 tree-widgets feature 作正向或负向断言。
        for scenario in scenarios:
            # 防止新增场景遗漏树组件源码 capability 的解析图边界。
            self.assertIn(
                "tree-widgets",
                (*scenario.required_uix_features, *scenario.forbidden_uix_features),
                scenario.name,
