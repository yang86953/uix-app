...
class ResolvedGraphTests(unittest.TestCase):
    ...
    def test_demo_logging_scenarios_bind_root_binary_and_feature_graph(self) -> None:
        ...
        # 文档声明的 schema v6 矩阵必须保持三十三个独立执行场景。
        ...
        self.assertEqual(
            ...
            ("d3d11", "charts", "navigation", "rich-text", "table"),
        )
        # 读取只启用演示日志的根二进制场景。
    ...
    def test_graphics_backend_scenarios_bind_dependency_and_public_api_guards(self) -> None:
        ...
        self.assertEqual(
            ...
            (*measure_usage_build.GRAPHICS_BACKEND_FEATURES, "charts", "navigation", "table"),
        )
    ...
    def test_rich_text_scenarios_bind_feature_and_public_api_guards(self) -> None:
        ...
        self.assertEqual(
            disabled.forbidden_uix_features,
            ("charts", "navigation", "rich-text", "table"),
        )
        # 负向入口必须以公开导入失败结束。
        self.assertTrue(disabled.expected_compile_failure)
        # 诊断必须同时覆盖组件类型和解析辅助函数。
    ...
    def test_charts_scenarios_bind_feature_and_public_api_guards(self) -> None:
        ...
        self.assertEqual(disabled.forbidden_uix_features, ("charts", "navigation", "table"))
        # 负向入口必须以公开导入失败结束。
        self.assertTrue(disabled.expected_compile_failure)
        # 诊断必须同时覆盖基础图表和高级图表类型。
        self.assertEqual(
    ...
    def test_agent_control_scenarios_bind_shared_json_and_public_api_guards(self) -> None:
        ...
        self.assertEqual(
            disabled.forbidden_uix_features,
            ("agent-control", "charts", "navigation", "table"),
        )
