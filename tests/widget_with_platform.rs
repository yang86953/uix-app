...
fn inject_clipboard_handler(pf: &mut FakePlatform) {
    ...
    uix::clipboard::set_clipboard_parts(parts.0, parts.1);
}
...
fn clipboard_with_fake_platform() {
    ...
    uix::clipboard::copy_to_clipboard("hello from ui");
...
fn clipboard_multiple_writes() {
    ...
    uix::clipboard::copy_to_clipboard("first");
    assert_eq!(pf.clipboard.text(), "first");

    uix::clipboard::copy_to_clipboard("second");
...
fn clipboard_has_text_after_copy() {
    ...
    uix::clipboard::copy_to_clipboard("now has text");
    assert!(pf.clipboard.has_text());
}
...
fn clipboard_handler_replacement() {
    ...
    uix::clipboard::copy_to_clipboard("before re-inject");
    assert_eq!(pf.clipboard.text(), "before re-inject");

    // 重新注入（模拟窗口重建等场景）
    inject_clipboard_handler(&mut pf);

    uix::clipboard::copy_to_clipboard("after re-inject");
