...
// 构造读取外部行集合并生成 keyed Component 实例的根工厂。
#[allow(non_snake_case)]
// UIX Component 与 camelCase prop 会进入卫生局部名，测试只消费语言规范名称。
fn row_root(rows: State<Vec<RowSpec>>) -> impl Fn() -> ViewNode {
    // 每次 reconcile 都从同一响应式集合取得当前拥有型快照。
    move || {
        // 读取集合同时登记根 View 的结构性响应式依赖。
        let items = rows.get();
        // 让真实 UIX Component、For、私有状态和事件进入公开宏路径。
        uix!(
            r#"
            <Component name="StatefulRow" props="valueId: String, buttonId: String" state="count: 0">
...
#[test]
fn keyed_for_component_state_follows_business_identity_and_lifecycle() {
    // 创建最初按 A、B 排列的业务行集合。
