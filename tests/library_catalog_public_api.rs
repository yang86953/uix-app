...
use uix_app::lang::{compiler::{components::ComponentCatalog, CompileTarget, CompilerSystem, QueryEntry, QueryKind}, demand};
use serde_json::json;
...
impl Project {
    fn new() -> Self {
        ...
        let root = std::env::temp_dir().join(format!("uix-catalog-{}-{}", std::process::id(), NEXT.fetch_add(1, std::sync::atomic::Ordering::Relaxed)));
        ...
        fs::write(root.join("library/uix-library.json"), serde_json::to_vec_pretty(&descriptor).unwrap()).unwrap();
    ...
    fn source(&self, source: &str) { fs::write(self.0.join("main.uix"), source).unwrap(); }
    fn catalog(&self) -> Arc<ComponentCatalog> { Arc::new(ComponentCatalog::for_project(&self.0).unwrap()) }
}
...
fn third_party_components_and_data_report_only_emitted_units() {
    ...
    project.source("<Widget name=\"Unused\"><Expensive /></Widget><Pill value={Flavor('mint')}>Hello</Pill>");
    let output = CompilerSystem::new().compile_file_auto(&project.0.join("main.uix")).unwrap();
    assert_eq!(output.required_units, ["sample-ui/flavor".into(), "sample-ui/pill".into()].into());
...
fn catalog_scope_does_not_leak_to_other_compilations() {
    ...
    project.catalog().with(|| {
        CompilerSystem::new().check_inline("<Pill>ok</Pill>", "inline", CompileTarget::View).unwrap();
    });
    assert!(CompilerSystem::new().check_inline("<Pill>ok</Pill>", "inline", CompileTarget::View).is_err());
}
...
fn declared_integer_bounds_reject_negative_and_fractional_values() {
    let project = Project::new();
    project.catalog().with(|| {
        for source in ["<Pill count=\"-1\" />", "<Pill count=\"6\" />", "<Pill count={2.5} />"] {
            assert!(CompilerSystem::new().check_inline(source, "invalid", CompileTarget::View).is_err(), "{source}");
        }
        CompilerSystem::new().check_inline("<Pill count=\"5\" />", "valid", CompileTarget::View).unwrap();
    });
}
...
fn queries_include_project_library_components_and_data() {
    ...
    for (kind, expected) in [(QueryKind::Components, "Pill"), (QueryKind::Data, "Flavor")] {
        let output = CompilerSystem::new().query_for_project(&project.0, kind).unwrap();
        assert!(output.entries.iter().any(|entry| matches!(entry, QueryEntry::Library { name, .. } if name == expected)));
    }
    assert!(!CompilerSystem::new().query(QueryKind::Components).entries.iter().any(|entry| matches!(entry, QueryEntry::Library { .. })));
}
...
fn build_view_prunes_unused_dependency_and_copies_selected_resources() {
    ...
    assert_eq!(plan.packages["sample-ui"].selected, ["flavor".into(), "pill".into()].into());
    ...
    assert_eq!(manifest["dependencies"]["heavy"]["optional"].as_bool(), Some(true));
    assert!(!manifest["features"]["__uix_selected"].as_array().unwrap().iter().any(|v| v.as_str() == Some("dep:heavy")));
    assert_eq!(fs::read_to_string(view.join("resources/sample-ui/shade.bin")).unwrap(), "palette");
    let descriptor: serde_json::Value = serde_json::from_slice(&fs::read(view.join("packages/sample-ui/uix-library.json")).unwrap()).unwrap();
    assert!(descriptor["components"].get("Expensive").is_none());
    assert!(descriptor["data_constructors"].get("Flavor").is_some());
    assert!(fs::read_to_string(project.0.join("library/Cargo.toml")).unwrap().contains("dep:heavy"));
}

#[test]
fn portable_modules_accept_library_defined_view_contracts() {
    use uix_app::lang::{compiler::modules, runtime::{Instance, HostPorts, Limits, Value}};
    let project = Project::new();
    let path = project.0.join("library/uix-library.json");
    let mut descriptor: serde_json::Value = serde_json::from_slice(&fs::read(&path).unwrap()).unwrap();
...
fn app_source_selects_application_while_a_view_does_not() {
    ...
    let app = CompilerSystem::new().compile_file_auto(&project.0.join("main.uix")).unwrap();
    assert!(app.required_units.contains("uix-app/application"));
    project.source("<Pill>Hello</Pill>");
    let view = CompilerSystem::new().compile_file_auto(&project.0.join("main.uix")).unwrap();
