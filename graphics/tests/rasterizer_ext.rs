...
use uix_graphics::rasterizer::fill;
use uix_graphics::rasterizer::gradient;
use uix_graphics::rasterizer::glyph;
use uix_graphics::rasterizer::image;
use uix_graphics::rasterizer::stroke;
use uix_graphics::types::{GradientDirection, Radius};
use uix_graphics::path::{PathBuilder, FillRule};
...
fn fill_linear_gradient_horizontal() {
    ...
    assert!(left < right, "左侧应比右侧暗，left={left:#X}, right={right:#X}");
}
...
fn fill_radial_gradient_basic() {
    ...
    assert!(center > corner, "圆心应比角落亮，center={center:#X}, corner={corner:#X}");
}
...
fn blit_image_identity() {
    ...
    image::blit_image(
        &mut pixels, 10, 10, clip, 1.0,
        &src, 5, Rect::new(0.0, 0.0, 5.0, 5.0),
...
fn blit_image_scaled() {
    ...
    image::blit_image(
        &mut pixels, 20, 20, clip, 1.0,
        &src, 3, Rect::new(0.0, 0.0, 3.0, 3.0),
...
fn fill_rect_module() {
    ...
    fill::fill_rect(
        &mut pixels, 10, 10, clip, 1.0,
        Rect::new(0.0, 0.0, 5.0, 5.0),
        Color::from_rgba(0, 255, 0, 255), None,
    );
...
fn fill_rect_module_opacity() {
    ...
    fill::fill_rect(
        &mut pixels, 10, 10, clip, 0.5,
        Rect::new(0.0, 0.0, 5.0, 5.0),
        Color::from_rgba(255, 0, 0, 255), None,
    );
...
fn fill_circle_module() {
    ...
    fill::fill_circle(
        &mut pixels, 20, 20, clip, 1.0,
        10.0, 10.0, 5.0, Color::from_rgba(255, 0, 0, 255),
    );
...
fn fill_path_module() {
    ...
    fill::fill_path(
        &mut pixels, 20, 20, clip, 1.0,
        &path, Color::from_rgba(255, 255, 255, 255), FillRule::NonZero,
    );
    // 三角形内部应有白色像素
...
fn fill_sector_module_basic() {
    ...
    fill::fill_sector(
        &mut pixels, 20, 20, clip, 1.0,
        10.0, 10.0, 5.0, 0.0, std::f32::consts::PI,
        Color::from_rgba(255, 0, 0, 255),
    );
    // 圆心（下半部分）应在扇形范围内
    let pixel = pixels[10 * 20 + 10];
    ...
    assert_eq!(pixels[6 * 20 + 10], 0x00000000, "扇形外（圆心上方）应为透明");
}
...
fn fill_sector_module_zero_radius_noop() {
    ...
    fill::fill_sector(
        &mut pixels, 10, 10, clip, 1.0,
        5.0, 5.0, 0.0, 0.0, std::f32::consts::PI,
        Color::from_rgba(255, 0, 0, 255),
    );
    // 半径为 0，不应修改任何像素
    assert_eq!(pixels[0], 0xFFFFFFFF);
...
fn fill_sector_module_three_quarter_circle() {
    ...
    fill::fill_sector(
        &mut pixels, 20, 20, clip, 1.0,
        10.0, 10.0, 5.0, 0.0, three_quarter,
        Color::from_rgba(0, 255, 0, 255),
    );
    // 圆心（在 0~3π/2 内）应为绿色
    let pixel = pixels[10 * 20 + 10];
...
fn draw_box_shadow_module_basic() {
    ...
    uix_graphics::rasterizer::shadow::draw_box_shadow(
        &mut pixels, 20, 20, clip, 1.0,
        Rect::new(5.0, 5.0, 10.0, 10.0), 2.0, 3.0, 3.0,
...
fn draw_box_shadow_module_zero_blur() {
    ...
    uix_graphics::rasterizer::shadow::draw_box_shadow(
        &mut pixels, 20, 20, clip, 1.0,
        Rect::new(5.0, 5.0, 10.0, 10.0), 0.0, 3.0, 3.0,
        Color::from_rgba(0, 0, 0, 255), None,
    );
    // 偏移后的阴影区域应有像素
...
fn draw_box_shadow_ambient_module_basic() {
    let mut pixels = vec![0u32; 400]; // 20x20
    let clip = Rect::new(0.0, 0.0, 20.0, 20.0);
    uix_graphics::rasterizer::shadow::draw_box_shadow_ambient(
        &mut pixels, 20, 20, clip, 1.0,
        Rect::new(5.0, 5.0, 10.0, 10.0), 3.0, 0.0, 0.0,
        Color::from_rgba(0, 0, 0, 128), None,
    );
...
fn draw_box_shadow_transparent_color_noop() {
    ...
    uix_graphics::rasterizer::shadow::draw_box_shadow(
        &mut pixels, 10, 10, clip, 1.0,
        Rect::new(0.0, 0.0, 5.0, 5.0), 2.0, 1.0, 1.0,
        Color::from_rgba(0, 0, 0, 0), None,
    );
    // 颜色 alpha=0，不应修改任何像素
    assert_eq!(pixels[0], 0xFFFFFFFF);
}
...
fn stroke_rect_module() {
    ...
    stroke::stroke_rect(
        &mut pixels, 10, 10, clip, 1.0,
        Rect::new(0.0, 0.0, 10.0, 10.0),
        Color::from_rgba(255, 255, 255, 255), 1.0, None,
    );
    // 边框应有白色
    assert_eq!(pixels[0], 0xFFFFFFFF);
    // 内部应为透明（1px 边框）
...
fn stroke_circle_module() {
    ...
    stroke::stroke_circle(
        &mut pixels, 20, 20, clip, 1.0,
        10.0, 10.0, 5.0, Color::from_rgba(255, 0, 0, 255), 1.0,
    );
    // 圆环上应有红色
...
fn draw_line_module() {
    ...
    stroke::draw_line(
        &mut pixels, 10, 10, clip, 1.0,
        0.0, 5.0, 9.0, 5.0, Color::from_rgba(255, 255, 255, 255), 1.0,
    );
    ...
    assert!(upper == 0xFFFFFFFF || lower == 0xFFFFFFFF, "水平线上应至少有一个白色像素");
}
