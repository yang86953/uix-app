...
impl RasterRenderer {
    ...
    pub fn set_offset(&mut self, dx: f32, dy: f32) { self.offset_x = dx; self.offset_y = dy; }
    ...
    fn premul(c: Color) -> u32 { rast::premul(c.to_rgba()) }
    fn apply_opa(&self, c: u32) -> u32 { rast::apply_opacity(c, self.opacity) }

    // ═══ 像素操作 ═══

    fn put_pixel_raw(&self, pixels: &mut [u32], w: i32, h: i32, x: i32, y: i32, color: u32) {
        let (cx0, cy0, cx1, cy1) = self.clip_int;
        if x < cx0.max(0) || y < cy0.max(0) || x >= cx1.min(w) || y >= cy1.min(h) { return; }
        ...
        if src_a == 0xFF && dst_a == 0 { pixels[idx] = color; return; }
    ...
    fn put_pixel_aa(&self, pixels: &mut [u32], w: i32, h: i32, x: i32, y: i32, premul_color: u32, coverage: f32) {
        let (cx0, cy0, cx1, cy1) = self.clip_int;
        if x < cx0 || y < cy0 || x >= cx1 || y >= cy1 { return; }
        if coverage >= 1.0 - 1e-6 { self.put_pixel_raw(pixels, w, h, x, y, premul_color); return; }
    ...
    fn fill_span(&self, pixels: &mut [u32], w: i32, h: i32, x: i32, y: i32, span_w: i32, color: u32) {
        if (color >> 24) == 0xFF {
            let (cx0, cy0, cx1, cy1) = self.clip_int;
        ...
        for dx in 0..span_w { self.put_pixel_raw(pixels, w, h, x + dx, y, color); }
    }

    fn fill_rect_raw(&self, pixels: &mut [u32], w: i32, h: i32, x: i32, y: i32, rw: i32, rh: i32, color: u32) {
        for dy in 0..rh { self.fill_span(pixels, w, h, x, y + dy, rw, color); }
    }
    ...
    fn sdf_to_coverage(sd: f32) -> f32 { rast::sdf_to_coverage(sd) }

    // ═══════════════════════════════════════════
    // 矢量填充
    // ═══════════════════════════════════════════

    pub fn fill_rect(&self, pixels: &mut [u32], surface_w: i32, surface_h: i32, rect: Rect, color: Color, radius: Option<Radius>) {
                ...
                for py in y0..y1 { for px in x0..x1 {
                    if let Some((ux, uy)) = self.apply_inverse(px as f32 + 0.5, py as f32 + 0.5) {
                        let coverage = match radius {
                            None => if ux >= rect.x && ux <= rect.x + rect.w && uy >= rect.y && uy <= rect.y + rect.h { 1.0 } else { 0.0 },
                            Some(ref rad) => Self::sdf_to_coverage(Self::rounded_rect_sdf(ux, uy, &rect, rad)),
                        };
                        if coverage > 0.0 { self.put_pixel_aa(pixels, surface_w, surface_h, px, py, c, coverage); }
                    }
                }}
            }
            return;
        ...
        if is_sharp {
            ...
            if cw > 0 && ch > 0 { self.fill_rect_raw(pixels, surface_w, surface_h, x0, y0, cw, ch, c); }
            return;
        }
        let expand = 1.0;
    ...
    pub fn fill_circle(&self, pixels: &mut [u32], surface_w: i32, surface_h: i32, cx: f32, cy: f32, r: f32, color: Color) {
        let (ox, oy) = (self.offset_x, self.offset_y);
        ...
        if Self::is_identity(&self.transform) {
            ...
            for py in y0..y1 { for px in x0..x1 {
                ...
                if dist2 <= inner2 { self.put_pixel_aa(pixels, surface_w, surface_h, px, py, c, 1.0); continue; }
            ...
            if let Some(cr) = self.intersect_clip(&bounds) {
                for py in (cr.y as i32)..((cr.y + cr.h) as i32) { for px in (cr.x as i32)..((cr.x + cr.w) as i32) {
                    if let Some((ux, uy)) = self.apply_inverse(px as f32 + 0.5, py as f32 + 0.5) {
    ...
    pub fn fill_ellipse(&self, pixels: &mut [u32], surface_w: i32, surface_h: i32, rect: Rect, color: Color) {
        let (ox, oy) = (self.offset_x, self.offset_y);
        let rect = if ox != 0.0 || oy != 0.0 { Rect::new(rect.x + ox, rect.y + oy, rect.w, rect.h) } else { rect };
        let c = self.apply_opa(Self::premul(color));
        ...
        if let Some(cr) = self.intersect_clip(&expanded) {
            ...
            for py in y0..y1 { for px in x0..x1 {
                ...
                if v >= 1.15 { continue; }
                if v <= 0.85 { self.put_pixel_aa(pixels, surface_w, surface_h, px, py, c, 1.0); continue; }
                let grad_mag = 2.0 * (dx * dx * inv_rx2 * inv_rx2 + dy * dy * inv_ry2 * inv_ry2).sqrt();
    ...
    pub fn fill_sector(&self, pixels: &mut [u32], surface_w: i32, surface_h: i32, cx: f32, cy: f32, r: f32, start_angle: f32, end_angle: f32, color: Color) {
        let (ox, oy) = (self.offset_x, self.offset_y);
        let (cx, cy) = if ox != 0.0 || oy != 0.0 { (cx + ox, cy + oy) } else { (cx, cy) };
        let c = self.apply_opa(Self::premul(color));
        let expand = r + 1.0;
        let norm = |a: f32| a.rem_euclid(std::f32::consts::TAU);
        let sa = norm(start_angle); let ea = norm(end_angle);
        let in_sector = |angle: f32| -> bool { let a = norm(angle); if sa <= ea { a >= sa && a <= ea } else { a >= sa || a <= ea } };
        let outer2 = (r + 1.0) * (r + 1.0);
        if Self::is_identity(&self.transform) {
            let x0 = (cx - expand).max(self.clip_rect.x) as i32;
            let y0 = (cy - expand).max(self.clip_rect.y) as i32;
            ...
            for py in y0..y1 { for px in x0..x1 {
                ...
                let dist2 = dx * dx + dy * dy;
                if dist2 >= outer2 { continue; }
                if !in_sector(dy.atan2(dx)) { continue; }
                let coverage = Self::sdf_to_coverage(dist2.sqrt() - r);
                if coverage > 0.0 { self.put_pixel_aa(pixels, surface_w, surface_h, px, py, c, coverage); }
            }}
        } else {
            ...
            if let Some(cr) = self.intersect_clip(&bounds) {
                for py in (cr.y as i32)..((cr.y + cr.h) as i32) { for px in (cr.x as i32)..((cr.x + cr.w) as i32) {
                    if let Some((ux, uy)) = self.apply_inverse(px as f32 + 0.5, py as f32 + 0.5) {
                        let dist = ((ux - cx).powi(2) + (uy - cy).powi(2)).sqrt();
                        if dist >= r { continue; }
                        if !in_sector((uy - cy).atan2(ux - cx)) { continue; }
                        let coverage = Self::sdf_to_coverage(dist - r);
                        if coverage > 0.0 { self.put_pixel_aa(pixels, surface_w, surface_h, px, py, c, coverage); }
                    }
                }}
    ...
    pub fn fill_path(&self, pixels: &mut [u32], surface_w: i32, surface_h: i32, path: &Path, color: Color, fill_rule: FillRule) {
        let (ox, oy) = (self.offset_x, self.offset_y);
        ...
        crate::rasterizer::polygon::fill_polygons(
            &polys, pixels, surface_w, surface_h, clip, c, fill_rule,
    ...
    pub fn stroke_rect(&self, pixels: &mut [u32], surface_w: i32, surface_h: i32, rect: Rect, color: Color, line_width: f32, radius: Option<Radius>) {
        let (ox, oy) = (self.offset_x, self.offset_y);
        let rect = if ox != 0.0 || oy != 0.0 { Rect::new(rect.x + ox, rect.y + oy, rect.w, rect.h) } else { rect };
            ...
            && lw == 1.0 && lw.fract() == 0.0
        {
            ...
            let iw = lw as i32;
            if iw * 2 >= w || iw * 2 >= h { self.fill_rect_raw(pixels, surface_w, surface_h, x0, y0, w, h, c); return; }
            let inner_h = h - iw * 2;
            ...
            self.fill_rect_raw(pixels, surface_w, surface_h, x0 + w - iw, y0 + iw, iw, inner_h, c);
            return;
        }
        let h = lw * 0.5;
        let expand = h + 1.0;
    ...
    pub fn stroke_circle(&self, pixels: &mut [u32], surface_w: i32, surface_h: i32, cx: f32, cy: f32, r: f32, color: Color, line_width: f32) {
        let (ox, oy) = (self.offset_x, self.offset_y);
        let (cx, cy) = if ox != 0.0 || oy != 0.0 { (cx + ox, cy + oy) } else { (cx, cy) };
        let lw = line_width.max(0.0);
        let c = self.apply_opa(Self::premul(color));
    ...
    pub fn stroke_path(&self, pixels: &mut [u32], surface_w: i32, surface_h: i32, path: &Path, color: Color, opts: &StrokeOptions) {
        let (ox, oy) = (self.offset_x, self.offset_y);
        let path = if ox != 0.0 || oy != 0.0 { &path.translated(ox, oy) } else { path };
        let c = self.apply_opa(Self::premul(color));
        ...
        let clip = self.clip_rect;
        crate::rasterizer::polygon::fill_polygons(
            &polys, pixels, surface_w, surface_h, clip, c, FillRule::NonZero,
            &mut global_edges, &mut active_edges,
        );
    }

    pub fn draw_line(&self, pixels: &mut [u32], surface_w: i32, surface_h: i32, mut x1: f32, mut y1: f32, mut x2: f32, mut y2: f32, color: Color, width: f32) {
        let (ox, oy) = (self.offset_x, self.offset_y);
        ...
        if (x1 - x2).abs() < 1e-6 {
            let x = x1 - half_lw; let y = y1.min(y2);
            self.fill_rect_raw(pixels, surface_w, surface_h, x as i32, y as i32, width as i32, (y1 - y2).abs() as i32, c);
            return;
        }
        if (y1 - y2).abs() < 1e-6 {
            let x = x1.min(x2); let y = y1 - half_lw;
            self.fill_rect_raw(pixels, surface_w, surface_h, x as i32, y as i32, (x1 - x2).abs() as i32, width as i32, c);
            return;
        }

        let expand = half_lw + 1.0;
        let bb = Rect::new(x1.min(x2) - expand, y1.min(y2) - expand, (x1 - x2).abs() + expand * 2.0, (y1 - y2).abs() + expand * 2.0);
        if let Some(cr) = self.intersect_clip(&bb) {
            let x0 = cr.x as i32; let y0 = cr.y as i32;
            let x1b = (cr.x + cr.w) as i32; let y1b = (cr.y + cr.h) as i32;
            for py in y0..y1b { for px in x0..x1b {
                let ux = px as f32 + 0.5; let uy = py as f32 + 0.5;
