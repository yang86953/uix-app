...
impl RenderTarget {
    ...
    pub fn pixel_bytes(&self) -> &[u8] {
        crate::software_engine::core::slice_u32_as_u8(&self.pixels)
    }
...
impl RenderTarget {
    ...
    /// 用指定颜色清除全部像素（用于第一帧或全帧 dirty）。
    pub fn clear_all_with(&mut self, color: u32) {
    ...
    /// 用指定颜色清除指定区域。
    pub fn clear_region_with(&mut self, x: i32, y: i32, w: i32, h: i32, color: u32) {
        if (color >> 24) == 0xFF {
            // 完全不透明：直接 fill
            let tw = self.width;
            for row in 0..h {
            ...
            // 半透明或透明：逐像素混合（与原有 clear_region 兼容）
            let tw = self.width;
            for row in 0..h {
                let start = ((y + row) * tw + x) as usize;
                for px in self.pixels[start..start + w as usize].iter_mut() {
                    *px = color;
                }
            }
        }
