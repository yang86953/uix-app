...
fn frame_graph_execute() {
    ...
    let _ = <NullEngine as GraphicsEngine>::initialize(&mut engine, 100, 100, &TestSystemInfo);
...
fn frame_graph_execute_with_callback() {
    ...
    let mut engine = NullEngine::new();
    let _ = <NullEngine as GraphicsEngine>::initialize(&mut engine, 50, 50, &TestSystemInfo);
