...
use uix_platform::{Error, IGraphicsContext};
...
use crate::traits::{Canvas2D, GraphicsCapabilities, UpdateStrategy};
...
impl RenderSession {
    ...
    pub fn resize(&mut self, width: i32, height: i32) {
        ...
        if self.backend.resize(width, height).is_err() {
            uix_platform::log::error_fn("RenderSession::resize 失败");
        }
    }
    ...
    pub fn begin_frame(&mut self, strategy: UpdateStrategy) -> RenderOutcome {
        if self.force_full_frame {
            ...
            return crate::pipeline::frame::begin_frame(
        ...
        let caps = self.backend.capabilities();
        crate::pipeline::frame::begin_frame(
            strategy,
            self.backend.surface(),
            self.width,
            self.height,
    ...
    pub fn end_frame(&mut self) -> RenderOutcome {
        crate::pipeline::frame::end_frame(self.backend.surface())
    }
