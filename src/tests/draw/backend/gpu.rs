use crate::draw::backend::contract::{BackendKind, DrawSurface, RenderBackend};
use crate::draw::backend::gpu::*;
#[cfg(feature = "d3d11")]
use crate::draw::command::EncodedPictureExecution;
use crate::draw::command::{EncodedFrameExecution, FrameCommand, FrameRasterOp};
#[cfg(feature = "d3d11")]
...
fn gpu_only_transformed_path_queues_solid_mesh() {
    ...
    let mut builder = crate::draw::geometry::path::PathBuilder::new();
    builder
        .move_to(0.0, 0.0)
        .line_to(4.0, 0.0)
        .line_to(4.0, 3.0)
...
#[test]
fn gpu_only_ellipse_queues_solid_mesh_without_soft_fallback() {
    let mut canvas = NativeGpuCanvas2D::new_gpu_only(48, 32, NativeRasterCaps::wgpu_full());
    canvas.fill_ellipse(
        Rect::new(6.0, 8.0, 24.0, 12.0),
    ...
    let Some(PendingNativeOp::SolidMesh(op)) = canvas.pending_native.first() else {
        panic!("GPU-only ellipse must route through the shared solid-mesh path");
    };
...
fn destination_dependent_failure(stage: DestinationFailureStage, code: Errc) -> Error {
    let mut backend = GpuBackend::new(Box::new(DestinationFailureContext { stage, code }))
    ...
    encoder.native(FrameRasterOp::FillRectAdditive {
        rect: crate::draw::command::FrameRect::new(0, 0, 1, 1),
        color: Color::white(),
    });
    backend
...
fn recording_gpu_only_backend(fail: FailStage) -> RecordingFixture {
    ...
    let backend = GpuBackend::new_gpu_only(Box::new(recording_context(
        &fail_stage,
        &stages,
        &solid_rects,
        &solid_scissors,
...
fn recording_backend_with_caps(fail: FailStage, native_caps: NativeRasterCaps) -> RecordingFixture {
    ...
    let shutdown_calls = Rc::new(Cell::new(0));
    let make_current_calls = Rc::new(Cell::new(0));
    let backend = GpuBackend::new(Box::new(recording_context(
        &fail_stage,
        &stages,
        &solid_rects,
        &solid_scissors,
...
fn native_gpu_resize_aligns_canvas_to_actual_gpu_client_size() {
    let mut backend = GpuBackend::new(Box::new(ClientRectLargerContext {
        width: 800,
        height: 600,
        bias_w: 120,
        bias_h: 80,
...
fn render_session_resize_follows_adopted_native_gpu_surface() {
    ...
    let backend = GpuBackend::new(Box::new(ClientRectLargerContext {
        width: 800,
        height: 600,
        bias_w: 120,
        bias_h: 80,
...
fn add_test_rect(
    builder: &mut crate::draw::geometry::path::PathBuilder,
    x0: f32,
    y0: f32,
    x1: f32,
    y1: f32,
...
#[test]
fn gpu_backend_offscreen_follows_native_caps() {
    ...
    let mut backend = GpuBackend::new(Box::new(fake_ctx(
        &clear_calls,
        &clear_rect_calls,
        &draw_calls,
        &stroke_calls,
...
fn gpu_only_picture_blit_with_opacity_uses_texture_blit() {
    use crate::draw::command::{FrameEncoder, FrameImage, FrameOpacity, FrameRect};
    ...
    encoder.blit_picture_with_opacity(
        ...
        crate::draw::command::FrameSampledRect::from_integer(FrameRect::new(7, 9, 2, 1)),
...
fn gpu_only_blur_reaches_engine_via_compositor_helper() {
    ...
    use crate::draw::scene::blur_picture_region;
    ...
    impl RenderTarget for BlurEngine {
...
fn wgpu_retained_framebuffer_unlocks_draw_side_partial_redraw_caps() {
    ...
    let caps = NativeRasterCaps::wgpu_full();
    assert!(caps.retained_framebuffer);
    // 与 GpuBackend::capabilities 同一路由：保留色缓冲 ⇒ 绘制侧 partial。
...
fn retained_backend_snapshots_and_restores_overlay_backdrop_without_cpu_readback() {
    ...
    let mut backend = GpuBackend::new_gpu_only(Box::new(BackdropFake {
        width: 64,
        height: 48,
...
#[test]
fn gpu_backend_offscreen_create_bind_blit_when_caps_prove_support() {
    ...
    let mut backend = GpuBackend::new(Box::new(fake)).expect("backend");
    ...
    // ScenePipeline reaches end_frame/final present. The old void method
...
#[test]
fn gpu_backend_rejects_context_without_hybrid_baseline() {
    struct GlCaps;
    ...
    let err = match GpuBackend::new(Box::new(GlCaps)) {
    ...
    let backend = GpuBackend::new(Box::new(LimitedD3d12))
...
fn native_gpu_canvas_stops_native_recording_after_first_soft_operation() {
    ...
    let mut path = crate::draw::geometry::path::PathBuilder::new();
...
#[test]
fn gpu_backend_propagates_each_present_stage_error() {
    let RecordingFixture {
        mut backend,
        fail_stage,
        stages,
...
fn main_frame_encoder_executes_each_command_at_its_recorded_boundary() {
    use crate::draw::command::{FrameImage, FrameRasterOp, FrameRect};
...
fn retained_partial_frame_encoder_preserves_clean_pixels() {
    use crate::draw::command::FrameRect;
...
fn picture_blit_group_opacity_is_quantized_before_the_bounded_soft_upload() {
    use crate::draw::command::{FrameImage, FrameOpacity, FrameRect};
    ...
    assert_eq!(
        ...
        source
            .into_iter()
            .map(|pixel| crate::draw::backend::cpu::rasterizer::apply_opacity(pixel, opacity))
            .collect::<Vec<_>>()
    );
}
...
fn zero_opacity_picture_blit_does_not_issue_an_empty_soft_upload() {
    ...
    encoder.blit_picture_with_opacity(
        ...
        FrameRect::new(0, 0, 2, 2),
        crate::draw::command::FrameSampledRect::from_integer(FrameRect::new(7, 9, 2, 2)),
...
fn main_frame_encoder_forwards_rounded_rect_geometry_to_native_gpu() {
    use crate::draw::command::{FrameRadius, FrameRect};
...
fn main_frame_encoder_forwards_stroke_rect_ir_when_capability_is_available() {
    use crate::draw::command::{FrameRadius, FrameRect, FrameStrokeRect, FrameStrokeWidth};
...
fn main_frame_glyph_ir_forwards_one_native_batch_with_integral_scissor() {
    use crate::draw::command::{FrameGlyphBlit, FrameRect};
...
fn recorded_picture_glyph_frame_encoder(
    width: i32,
    height: i32,
) -> (crate::draw::command::FrameEncoder, Arc<[u8]>) {
...
fn glyph_incapable_backend_uses_sparse_bounded_soft_clusters_in_order() {
    ...
    let glyph = |x| {
        crate::draw::command::FrameGlyphBlit::new(
            x,
            20,
            Arc::clone(&coverage),
            4,
...
#[test]
fn gpu_backend_shutdown_is_idempotent_across_drop() {
    let RecordingFixture {
        mut backend,
        shutdown_calls,
        make_current_calls,
...
#[test]
fn gpu_backend_retries_a_checked_context_shutdown_failure() {
    let RecordingFixture {
        mut backend,
        fail_stage,
        shutdown_calls,
...
#[test]
fn gpu_backend_surfaces_swapchain_restore_failure_after_offscreen_clear_fails() {
    let RecordingFixture {
        mut backend,
        fail_stage,
        ..
...
fn d3d11_backend_routes_supported_and_unsupported_paths_by_topology() {
    ...
    {
        let canvas = backend.surface().canvas();
        let mut fill = crate::draw::geometry::path::PathBuilder::new();
        ...
        let mut stroke = crate::draw::geometry::path::PathBuilder::new();
        stroke
            .move_to(24.0, 24.0)
            .line_to(72.0, 24.0)
            .line_to(72.0, 72.0);
        canvas.stroke_path(
            ...
            &StrokeOptions {
                width: 16.0,
                cap: crate::draw::geometry::path::LineCap::Butt,
                join: crate::draw::geometry::path::LineJoin::Miter,
                miter_limit: 2.0,
            },
        );
        ...
        for (y, cap, join) in [
            (
                12.0,
                crate::draw::geometry::path::LineCap::Square,
                crate::draw::geometry::path::LineJoin::Bevel,
            ),
            (
                44.0,
                crate::draw::geometry::path::LineCap::Round,
                crate::draw::geometry::path::LineJoin::Round,
            ),
        ] {
            let mut variant = crate::draw::geometry::path::PathBuilder::new();
            variant
                .move_to(84.0, y)
                .line_to(108.0, y)
                .line_to(116.0, y + 8.0);
        ...
        let mut empty = crate::draw::geometry::path::PathBuilder::new();
        ...
        canvas.stroke_path(
            ...
            &StrokeOptions {
                width: 8.0,
                cap: crate::draw::geometry::path::LineCap::Round,
                join: crate::draw::geometry::path::LineJoin::Round,
                miter_limit: 4.0,
            },
        );
    }
    ...
    {
        let canvas = backend.surface().canvas();
        let mut two_holes = crate::draw::geometry::path::PathBuilder::new();
        ...
        let mut deep = crate::draw::geometry::path::PathBuilder::new();
        ...
        let mut islands = crate::draw::geometry::path::PathBuilder::new();
    ...
    {
        let canvas = backend.surface().canvas();
        let mut intersecting = crate::draw::geometry::path::PathBuilder::new();
        ...
        let mut self_intersecting = crate::draw::geometry::path::PathBuilder::new();
        self_intersecting
            .move_to(8.0, 8.0)
            .line_to(56.0, 8.0)
            .line_to(56.0, 40.0)
        ...
        let mut touching = crate::draw::geometry::path::PathBuilder::new();
        ...
        let mut cancelled = crate::draw::geometry::path::PathBuilder::new();
    ...
    {
        let canvas = backend.surface().canvas();
        let mut oversized = crate::draw::geometry::path::PathBuilder::new();
        for index in 0..513 {
...
fn assert_contour_forest_on_real_window() {
    ...
    let mut backend = GpuBackend::new(context).expect("D3D11 backend");
    ...
    let mut overlapping = crate::draw::geometry::path::PathBuilder::new();
...
fn d3d12_warp_real_context_flows_through_gpu_renderer_with_mixed_native_and_soft() {
    ...
    let mut engine = Renderer::from_context(context).expect("D3D12 Renderer");
    RenderTarget::initialize(&mut engine, width, height).expect("initialize D3D12 engine");
    {
        let backend = engine
            .session_mut()
            .backend_mut()
            ...
            .expect("D3D12 must use shared GpuBackend");
        assert_eq!(backend.kind(), BackendKind::Gpu);
        assert_eq!(
            backend.capabilities(),
            crate::draw::backend::contract::BackendCapabilities::gpu_full_redraw()
        );
        backend
            .gpu_ctx
            .clear_render_target(0.0, 0.0, 0.0, 1.0)
...
#[cfg(any(feature = "d3d11", feature = "d3d12"))]
fn verify_clipped_rounded_native_backend(native: &mut GpuBackend, label: &str) {
...
#[cfg(any(feature = "d3d11", feature = "d3d12"))]
fn verify_picture_glyph_native_backend(native: &mut GpuBackend, label: &str) {
    let (encoder, _) =
...
fn d3d11_warp_clipped_rounded_opacity_ir_matches_reference() {
    ...
    let mut native = GpuBackend::new(context).expect("native backend");
...
fn d3d11_warp_executes_encoded_picture_in_its_bound_offscreen_target() {
    ...
    let context =
        crate::native::factory::create_d3d11_warp_test_context(window.native_surface_ptr(), 96, 64)
            .expect("D3D11 WARP context");
    let mut native = GpuBackend::new(context).expect("native WARP backend");
...
fn d3d11_warp_executes_main_frame_encoder_before_its_only_present() {
    use crate::draw::command::{FrameImage, FrameRadius, FrameRasterOp, FrameRect};
...
fn d3d11_warp_additive_and_scroll_frame_ops_match_reference_executor() {
    use crate::draw::command::{FrameRasterOp, FrameRect};
...
fn destination_dependent_frame_ops_use_readback_apply_upload_on_pixel_context() {
    ...
    let mut backend = GpuBackend::new(Box::new(PixelContext {
        width: 1,
        height: 1,
...
fn native_common_hybrid_clip_pixels(
    ...
    let mut backend = GpuBackend::new(context).expect("native WARP backend");
