...
use crate::draw::scene::layer_tree::*;
use crate::draw::scene::ScenePaint;
use crate::draw::{BlendMode, GradientDirection, Radius, Transform};
...
impl crate::draw::api::IColorTokens for TestTokens {
    test_color_tokens!(
        color_primary,
        color_primary_hover,
...
impl crate::draw::api::ITypographyTokens for TestTokens {
    fn font_family(&self) -> &str {
        "sans"
    }
}

impl crate::draw::api::IBoxShadowTokens for TestTokens {
...
impl crate::draw::api::ISpacingTokens for TestTokens {}
impl crate::draw::api::ThemeTokens for TestTokens {}
...
struct LeakyClipCanvas {
    inner: SharedRasterizer,
}

impl LeakyClipCanvas {
    fn new(width: i32, height: i32) -> Self {
        Self {
            inner: SharedRasterizer::new(PixelSurface::new(width, height)),
        }
    }
}

impl crate::draw::Canvas2D for LeakyClipCanvas {
    ...
    fn set_offset(&mut self, dx: f32, dy: f32) {
        self.inner.set_offset(dx, dy);
    }

    fn fill_rect(&mut self, rect: Rect, color: Color, radius: Option<Radius>) {
        self.inner.fill_rect(rect, color, radius);
    }
    ...
    fn fill_ellipse(&mut self, rect: Rect, color: Color) {
        self.inner.fill_ellipse(rect, color);
    }
    ...
    fn fill_linear_gradient(
        ...
        self.inner
            .fill_linear_gradient(rect, color_a, color_b, direction);
    }
    ...
    fn draw_box_shadow_ambient(
        ...
        self.inner.draw_box_shadow_ambient(
            rect,
            blur_radius,
            offset_x,
            offset_y,
    ...
    fn blit_image(&mut self, src: &[u32], src_w: i32, src_rect: Rect, dst_rect: Rect) {
        self.inner.blit_image(src, src_w, src_rect, dst_rect);
    }
    ...
    fn blit_glyph(
        ...
        self.inner.blit_glyph(x, y, coverage, width, height, color);
    }
    ...
    fn surface_size(&self) -> crate::core::Size {
        self.inner.surface_size()
    }

    fn current_clip(&self) -> Rect {
        self.inner.current_clip()
    }

    fn scroll_region(&mut self, viewport: Rect, dx: f32, dy: f32) {
        self.inner.scroll_region(viewport, dx, dy);
    }
}
...
impl RenderTarget for LeakyClipEngine {
    ...
    fn begin_frame(
        &mut self,
        _: crate::draw::UpdateStrategy,
    ) -> crate::draw::renderer::RenderOutcome {
        crate::draw::renderer::RenderOutcome::FrameReady(crate::draw::backend::DamageRegion::full())
    }
    ...
    fn end_frame(
        ...
        crate::draw::renderer::RenderOutcome::Present(damage.clone())
    }
...
fn direct_layer_applies_scene_transform_to_widget_paint() {
    ...
    engine.initialize(20, 16).expect("CPU renderer init");
    let tokens = TestTokens;
    let theme = ThemeSnapshot::new(&tokens);
    let fonts = FontService::new();
    let images = ImageService::new();
...
fn direct_layers_multiply_parent_and_child_opacity() {
    ...
    tree.build(&scene, false);
    let mut engine = Renderer::cpu();
    engine.initialize(8, 8).expect("CPU renderer init");
    let tokens = TestTokens;
    let theme = ThemeSnapshot::new(&tokens);
    let fonts = FontService::new();
    let images = ImageService::new();
...
fn picture_partial_dirty_rerasterize_repaints_all_direct_children() {
    ...
    struct Tok;
    impl crate::draw::api::IColorTokens for Tok {
    ...
    impl crate::draw::api::ITypographyTokens for Tok {
        fn font_family(&self) -> &str {
            "sans"
        }
    }
    impl crate::draw::api::IBoxShadowTokens for Tok {
        fn box_shadow(&self) -> crate::draw::api::ShadowToken {
            crate::draw::api::ShadowToken::none()
        }
        fn box_shadow_secondary(&self) -> crate::draw::api::ShadowToken {
    ...
    impl crate::draw::api::ISpacingTokens for Tok {}
    impl crate::draw::api::ThemeTokens for Tok {}
    ...
    engine.begin_frame(crate::draw::UpdateStrategy::FullRedraw);
    tree.render(
        &mut engine,
        &scene,
        &region,
