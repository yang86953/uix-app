...
use crate::draw::{GraphicsCapabilities, RenderTarget, UpdateStrategy};
...
fn present_upload_engine_uses_factory_initialized_context_without_second_initialize() {
    ...
    let mut engine = Renderer::from_context(Box::new(context)).unwrap();
...
fn present_upload_engine_logical_extent_follows_adopted_gpu_client_rect() {
    ...
    let mut engine = Renderer::from_context(Box::new(AdoptedClientContext {
        width: 800,
        height: 600,
        logical_width: 800,
        logical_height: 600,
...
fn present_upload_engine_submits_cpu_pixels_to_context() {
    ...
    assert_eq!(
        engine.capabilities(),
        GraphicsCapabilities::backend_managed_retained_pixels(),
