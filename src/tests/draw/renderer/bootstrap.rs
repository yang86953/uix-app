...
use crate::draw::renderer::bootstrap::ProbeStage;
...
#[test]
fn create_renderer_shuts_down_context_for_unsupported_cpu_presenter_recipe() {
    ...
    let err = match create_renderer(Box::new(context)) {
        Ok(_) => panic!("cpu_presenter context recipe must be rejected"),
        Err(err) => err,
    };

    assert!(err.message().contains("cpu × cpu_presenter"));
...
#[cfg(feature = "d3d11")]
#[test]
fn bootstrap_shuts_down_context_when_renderer_creation_fails() {
    let shutdown_called = Rc::new(Cell::new(false));
    match bootstrap_renderer_with(
        null_surface(),
        640,
        480,
        ...
        Ok(_) => panic!("cpu_presenter must fail renderer creation"),
        Err(report) => {
            assert!(shutdown_called.get());
            let failure = report.failures.first().expect("renderer failure");
            assert_eq!(failure.backend, GraphicsBackend::D3d12);
            assert!(failure.message.contains("stage=renderer_create"));
            assert!(failure
                .message
                .contains("selected=backend=d3d12; raster=cpu; present=cpu_presenter"));
            assert!(failure.message.contains("cpu × cpu_presenter"));
        }
    }
}
...
#[cfg(feature = "d3d11")]
#[test]
fn bootstrap_shuts_down_context_when_renderer_initialize_fails() {
    let shutdown_called = Rc::new(Cell::new(false));
    ...
    match bootstrap_renderer_with(
        ...
        Ok(_) => panic!("renderer initialization failure must reject bootstrap"),
        Err(report) => {
            assert!(shutdown_called.get());
            let failure = report.failures.first().expect("init failure");
            assert_eq!(failure.backend, GraphicsBackend::D3d12);
            assert!(failure.message.contains("stage=renderer_initialize"));
            assert!(failure
                .message
...
fn bootstrap_preserves_initialize_failure_when_checked_teardown_fails() {
    ...
    let report = match bootstrap_renderer_with(
        null_surface(),
        640,
        480,
        GraphicsBackend::D3d12,
...
fn auto_falls_through_established_backends_to_low_priority_d3d12() {
    ...
    let mut bootstrap = bootstrap_renderer_with(
        null_surface(),
        64,
        48,
    ...
    bootstrap.renderer.try_shutdown().expect("checked shutdown");
}
...
fn same_api_recipe_probe_falls_through_to_later_recipe() {
    ...
    let mut bootstrap = bootstrap_renderer_with_candidates(
        64,
        48,
        GraphicsBackend::D3d11,
