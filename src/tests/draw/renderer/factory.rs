...
use crate::draw::renderer::factory::*;
...
#[test]
fn gpu_native_swapchain_assembles_gpu_renderer_for_d3d11() {
    let mut engine = create_renderer(Box::new(FakeGpuNative(GraphicsBackend::D3d11)))
        .expect("Renderer for D3D11");
    engine.initialize(64, 48).expect("initialize");
    engine.try_shutdown().expect("checked shutdown");
}

#[test]
fn gpu_native_swapchain_assembles_gpu_renderer_for_d3d12() {
    let mut engine = create_renderer(Box::new(FakeGpuNative(GraphicsBackend::D3d12)))
        .expect("Renderer for D3D12");
    engine.initialize(64, 48).expect("initialize");
    engine.try_shutdown().expect("checked shutdown");
}
