...
use crate::draw::renderer::session::*;
...
#[test]
fn set_backend_to_test_forces_full_frame_once() {
    let mut session = RenderSession::new(BackendKind::Cpu).expect("Cpu 会话");
    session.initialize(10, 10).expect("init");
    session.set_backend(BackendKind::Test).expect("switch");
    assert_eq!(session.backend_kind(), BackendKind::Test);
...
#[test]
fn cpu_or_test_switch_keeps_staged_context_until_session_shutdown() {
    ...
    session
        .set_backend(BackendKind::Test)
        .expect("switch to test backend");
