...
impl IGraphicsContext for FactoryInitializedNativeContext {
    ...
    fn initialize(
        ...
        panic!("factory-initialized context must not be initialized by Renderer")
    }
...
fn gpu_native_engine_uses_factory_drawable_without_initial_resize() {
    ...
    let mut engine = Renderer::from_context(Box::new(context)).expect("GpuNative engine");
...
#[test]
fn gpu_renderer_rejects_cpu_raster_without_allocating_soft_resources() {
    let resize_calls = Rc::new(Cell::new(0));
    let context = FactoryInitializedNativeContext {
        resize_calls,
        width: 64,
    ...
    assert!(engine.session().gpu_backend().is_some_and(|backend| backend
        .surface
        .canvas
        .soft_fallback
        .is_none()));
