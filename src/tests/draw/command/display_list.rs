use crate::draw::command::display_list::*;
use crate::draw::geometry::path::PathBuilder;
use crate::draw::geometry::spatial::Orientation;
use crate::draw::scene::picture::encode_cached_picture;
...
fn complete_picture_encoder_preserves_local_painter_order() {
    ...
    assert_eq!(
        frame.pixel(2, 1),
        Some(crate::draw::backend::cpu::rasterizer::core::blend_srcover(
...
fn display_list_clone_shares_all_operation_storage_until_mutated() {
    let layout = crate::draw::resources::font::text_backend::TextLayout {
...
#[test]
fn display_list_replay_matches_public_geometry_helpers_and_state() {
    use crate::draw::{BlendMode, Radius};

    fn paint_public_geometry(ctx: &mut PaintContext<'_>) {
        let triangle = [
            Point::new(8.0, 3.0),
            Point::new(20.0, 3.0),
            Point::new(14.0, 13.0),
        ...
        ctx.draw_point(Point::new(4.0, 4.0), Color::red(), 4.0);
        ctx.fill_polygon(
            &triangle,
            Color::from_rgba(20, 180, 80, 190),
            FillRule::EvenOdd,
        );
        ctx.stroke_polyline(&polyline, Color::blue(), &stroke);
        ctx.stroke_polygon(&triangle, Color::black(), &stroke);
        ctx.stroke_ellipse(
            Rect::new(4.0, 18.0, 14.0, 8.0),
            Color::from_rgba(220, 80, 30, 210),
            &stroke,
        );
        ctx.fill_rounded_rect(
            Rect::new(22.0, 18.0, 14.0, 8.0),
            Color::from_rgba(40, 120, 230, 180),
            Radius::uniform(3.0),
        );
        ctx.stroke_rounded_rect(
            Rect::new(4.0, 32.0, 14.0, 10.0),
            Color::white(),
            2.0,
            Radius::uniform(3.0),
        );
        ...
        ctx.save();
        ctx.set_transform(crate::draw::Transform::translate(1.0, 0.0));
        ctx.concat_transform(crate::draw::Transform::scale(1.0, 1.0));
        ctx.push_clip_path(&clip_path);
        ctx.translate(2.0, 1.0);
        ctx.set_blend_mode(BlendMode::Additive);
        ctx.set_opacity(0.8);
        ctx.with_opacity(0.5, |ctx| {
            ctx.fill_rounded_rect(
                Rect::new(22.0, 32.0, 14.0, 10.0),
                Color::from_rgba(180, 40, 190, 220),
                Radius::uniform(2.0),
            );
        });
        ctx.draw_point(Point::new(30.0, 37.0), Color::white(), 3.0);
        ctx.pop_clip();
        ctx.restore();
    }
    ...
    let mut recorded = Renderer::cpu();
    recorded.initialize(48, 48).expect("recorded init");
    let _ = recorded.begin_frame(UpdateStrategy::FullRedraw);
    let list = {
        let mut list = DisplayList::new();
        let mut ctx = PaintContext::new_for_test(
        ...
        ctx.with_recorder(&mut list, paint_public_geometry);
        assert!(ctx.recording_complete());
        list
    };
    let _ = recorded.end_frame(&crate::draw::DamageRegion::full());
    ...
    let mut replayed = Renderer::cpu();
    replayed.initialize(48, 48).expect("replayed init");
    let _ = replayed.begin_frame(UpdateStrategy::FullRedraw);
    {
        let mut ctx = PaintContext::new_for_test(
            replayed.canvas_2d(),
    ...
    assert_eq!(
        actual, expected,
        "public geometry helpers and state must replay with identical pixels"
    );
}
