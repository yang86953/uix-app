use crate::draw::command::recorder::*;
...
use crate::draw::geometry::types::{BlendMode, Radius, Transform};
...
fn glyph_opacity_stays_native_and_matches_cpu_before_coverage_quantization() {
    ...
    for opacity in [f32::NAN, 0.0, 1.0 / 255.0, 0.37, 0.5, 0.999_998, 1.0] {
        ...
        let encoder = engine.finish_recording().expect("finish recorder");
        let expected_color =
            crate::draw::backend::cpu::rasterizer::color_with_glyph_opacity(glyph_color, opacity);
...
fn integral_picture_origin_splices_opacity_adjusted_shared_glyph_ir_into_the_main_frame() {
    ...
    assert_eq!(
        glyphs[0].color(),
        crate::draw::backend::cpu::rasterizer::color_with_glyph_opacity(glyph_color, 0.37)
    );
    assert!(Arc::ptr_eq(glyphs[0].coverage(), &coverage));
    assert!(!encoder.commands().iter().any(|command| matches!(
        command,
...
fn fill_opacity_stays_native_and_preserves_exact_cpu_premultiplied_sources() {
    ...
    for opacity in [0.0, 0.37, 0.5, 0.999_998] {
        ...
        let expected_sharp = crate::draw::backend::cpu::rasterizer::apply_opacity(
            sharp_color.premultiplied(),
            opacity,
        );
        let expected_rounded = crate::draw::backend::cpu::rasterizer::apply_opacity(
            rounded_color.premultiplied(),
            opacity,
        );
        assert!(matches!(
            encoder.commands(),
...
fn integral_circle_bounds_keep_offset_clip_and_opacity_native_with_cpu_parity() {
    ...
    let expected_color =
        crate::draw::backend::cpu::rasterizer::apply_opacity(color.premultiplied(), opacity);
    assert!(encoder.commands().iter().any(|command| matches!(
        command,
        FrameCommand::Native {
            operation: FrameRasterOp::FillRoundedRectClipped {
...
#[test]
fn integral_stroke_rect_keeps_offset_clip_and_opacity_in_native_ir() {
    use crate::draw::backend::cpu::pixel_surface::PixelSurface;
    use crate::draw::backend::cpu::shared_rasterizer::SharedRasterizer;
    ...
    let color = Color::from_rgba(211, 47, 129, 149);
    let opacity = 0.37;
    let rect = Rect::new(3.0, 3.0, 12.0, 8.0);
    let radius = Radius::uniform(3.0);
    let line_width = 2.5;
    let offset = (2.0, 1.0);
    let local_clip = Rect::new(4.0, 3.0, 10.0, 8.0);
    ...
    engine
        .canvas_2d()
        .stroke_rect(rect, color, line_width, Some(radius));
    let encoder = engine.finish_recording().expect("finish recorder");
    ...
    assert!(encoder.commands().iter().any(|command| matches!(
        ...
        FrameCommand::Native {
            ...
            if stroke.rect() == FrameRect::new(5, 4, 12, 8)
                && stroke.color().premultiplied() == expected_color
                && stroke.radius() == FrameRadius::new(radius).expect("valid radius")
                && stroke.line_width().value() == line_width)
            && *clip == FrameRect::new(6, 4, 10, 8)
    )));
    assert!(!encoder
        .commands()
        .iter()
    ...
    cpu.push_clip(local_clip);
    cpu.set_opacity(opacity);
    cpu.stroke_rect(rect, color, line_width, Some(radius));
    assert_eq!(encoder.render_reference().pixels(), cpu.surface().pixels());
}

#[test]
fn transparent_integral_stroke_rect_is_a_recording_noop() {
    let mut engine = CommandRecorder::new();
    engine.initialize(16, 12).expect("initialize recorder");
    engine.begin_recording(true).expect("begin recording");
    engine.canvas_2d().set_opacity(0.0);
    engine.canvas_2d().stroke_rect(
        Rect::new(2.0, 2.0, 10.0, 7.0),
        Color::from_rgb(40, 120, 220),
        2.0,
        Some(Radius::uniform(2.0)),
    );
...
#[test]
fn fractional_stroke_rect_geometry_keeps_the_exact_cpu_fallback() {
    let mut engine = CommandRecorder::new();
    engine.initialize(16, 12).expect("initialize recorder");
    engine.begin_recording(true).expect("begin recording");
    engine.canvas_2d().stroke_rect(
        Rect::new(2.25, 2.0, 10.0, 7.0),
        Color::from_rgba(40, 120, 220, 176),
        2.0,
        Some(Radius::uniform(2.0)),
    );
...
fn recording_canvas_emits_native_cpu_and_picture_commands_in_painter_order() {
    ...
    assert!(matches!(
        encoder.commands()[2],
        crate::draw::command::FrameCommand::CpuSegment { .. }
    ));
    assert!(matches!(
        encoder.commands()[3],
...
fn integral_image_blit_retains_only_the_visible_crop_with_offset_clip_and_opacity() {
    ...
    assert_eq!(
        *command.2,
        crate::draw::command::FrameSampledRect::from_integer(FrameRect::new(8, 4, 3, 2))
    );
...
fn recording_canvas_retains_compact_cpu_segment_tiles() {
    ...
    let cpu_segment = encoder
        ...
        .iter()
        .find_map(|command| match command {
            crate::draw::command::FrameCommand::CpuSegment { image, src, dst } => {
                Some((image, src, dst))
            }
            _ => None,
        })
...
fn additive_sampled_picture_blit_records_additive_flag() {
    use crate::draw::command::{FrameOpacity, FrameSampledRect};
