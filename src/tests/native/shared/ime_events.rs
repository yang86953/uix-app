...
#[test]
fn poisoned_event_queue_still_accepts_composition_events() {
    let events = Arc::new(Mutex::new(VecDeque::new()));
    let poisoned = Arc::clone(&events);
    let _ = std::thread::spawn(move || {
        let _queue = poisoned.lock().expect("lock queue before poisoning");
        panic!("poison IME queue for recovery test");
    })
    .join();
    let mut state = ImeCompositionState::default();
    ...
    assert!(state.active);
    let queue = queue(&events);
    assert_eq!(queue.len(), 2);
    assert_eq!(queue[0].type_, UiEventType::ImeCompositionStart);
    assert_eq!(queue[1].type_, UiEventType::ImeCompositionUpdate);
}
