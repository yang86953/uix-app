// ============================================================================
// platform/src/diagnostic/collector.rs — 错误收集器
//
...
use crate::platform::diagnostic::Timestamp;
use crate::platform::api::error::{Errc, Error, ErrorSeverity};
use crate::platform::log::{Level, Logger};
    ...
    pub fn collect(&self, err: Error) -> usize {
        ...
        if inner.config.auto_log || severity >= ErrorSeverity::Error {
            ...
            if severity.should_abort() {
                crate::platform::diagnostic::dump_crash_report();
                std::process::abort();
            }
        } else {
            drop(inner);
