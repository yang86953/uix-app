...
pub fn windows_diag(code: crate::platform::Errc, context: &str) -> Error {
...
pub fn system_default_font_paths() -> Vec<String> {
    unsafe {
        ...
        if let Some(ref name) = sys_font_name {
            crate::platform::log::info_fn(format!("System default UI font: {}", name));
        }
...
mod tests {
    ...
    fn windows_diag_produces_error() {
        let err = windows_diag(crate::platform::Errc::PlatformError, "test context");
