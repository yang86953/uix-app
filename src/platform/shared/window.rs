...
use crate::platform::api::window::{INativeHandle, IWindowProperties, PlatformWindow};
use crate::platform::shared::state::WindowState;
...
impl<O: WindowOps> PlatformWindow for PlatformWindowCore<O> {
    ...
    fn resize_notify(&mut self, width: i32, height: i32) {
        ...
        if let Err(e) = self.presenter.resize(width, height) {
            crate::platform::log::warn_fn(format!(
...
impl<O: WindowOps> IWindowProperties for PlatformWindowCore<O> {
    ...
    fn position(&self) -> crate::platform::api::geometry::Point {
        crate::platform::api::geometry::Point::new(
