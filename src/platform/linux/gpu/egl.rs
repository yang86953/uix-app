...
impl EglContext {
    ...
    pub fn new(native_surface: *mut c_void, width: i32, height: i32) -> Result<Self, Error> {
        ...
        crate::platform::log::info_fn("EglContext: EGL {major}.{minor}");
        ...
        let context = {
            ...
            match egl.create_context(display, config, None, &ctx3_attribs) {
                Ok(ctx) => {
                    crate::platform::log::info_fn("EglContext: GLES 3.0 上下文创建成功");
                    ctx
                }
                Err(_) => {
                    crate::platform::log::warn_fn("EglContext: GLES 3.0 不可用，降级到 2.0");
