...
use crate::platform::shared::unimpl;
use crate::platform::shared::WindowOps;
...
impl WaylandWindowOps {
    ...
    pub(crate) fn init(
        ...
        use crate::platform::api::event::{UiEventPayload, UiEventType};
        ...
        let got_kde = globals
            ...
            .map(|dm| {
                ...
                crate::platform::log::info_fn("[Wayland] KDE server-side decoration requested");
                true
            })
            .unwrap_or(false);
        if !got_kde {
            if let Ok(dm) = globals.instantiate_exact::<ZxdgDecorationManagerV1>(1) {
                ...
                crate::platform::log::info_fn("[Wayland] xdg-decoration ServerSide mode requested");
            } else {
                crate::platform::log::warn_fn("[Wayland] 无可用的窗口装饰协议，窗口可能无标题栏");
            }
        }
...
impl WindowOps for WaylandWindowOps {
    ...
    fn os_center_on_screen(&mut self) {
            ...
            .or_else(|| outputs.first())
        {
            ...
            crate::platform::log::info_fn("窗口居中: 未检测到显示器信息，由 compositor 自行放置");
        }
    }
    ...
    fn os_raise(&mut self) {
        ...
        if let (Some(xa), Some(surface)) = (self.xdg_activation.as_ref(), self.surface.as_ref()) {
            ...
            crate::platform::log::info_fn("已请求窗口激活 (xdg_activation_v1)");
        } else {
            crate::platform::log::info_fn("请求窗口提升: xdg_activation_v1 不可用，由 compositor 自行决定");
        }
    }
