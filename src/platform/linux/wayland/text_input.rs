...
impl ITextInput for WaylandBackend {
    fn start(&mut self) {
        let seat = match self.seat.as_ref() {
            Some(s) => s,
            None => {
                crate::platform::log::warn_fn("Wayland text_input: no seat available, IME not activated");
                return;
            }
        };
        ...
        let manager = match self.text_input_manager.as_ref() {
            Some(m) => m,
            None => {
                crate::platform::log::warn_fn("Wayland text_input: no zwp_text_input_manager_v3, compositor may not support IME");
                return;
            }
        };
