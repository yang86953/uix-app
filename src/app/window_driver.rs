...
fn update_scheduled_and_discovered_animations(
    tree: &mut WidgetTree,
...
fn protocol_failure(message: &str) -> GraphicsFailure {
...
fn sync_root_frame_exactly_to_engine(tree: &mut WidgetTree, engine: &mut dyn GraphicsEngine) {
    let (width, height) = {
        let canvas = engine.canvas_2d();
        (canvas.width() as f32, canvas.height() as f32)
    };
