...
use crate::ui::widgets::combinators::{button, column, input, label, row};
...
impl ModuleView {
    ...
    ) -> RuntimeResult<(ModuleWorker, Self)> {
        let changed = State::new(());
        let signal = changed.clone();
        let worker = ModuleWorker::spawn(instance, capacity, move || {
            signal.set(());
            wake();
        })?;
        let view = Self {
            ...
            changed,
            drafts: Arc::new(Mutex::new(Drafts::default())),
        };
        Ok((worker, view))
    }
    ...
    pub fn project(&self) -> RuntimeResult<ViewNode> {
        ...
        if !snapshot.active {
            ...
            return Err(snapshot
                .error
                .unwrap_or_else(|| RuntimeError::new(ErrorKind::Closed, "模块界面已关闭")));
        }
        let node = snapshot
            .view
            .as_ref()
            .ok_or_else(|| RuntimeError::new(ErrorKind::InvalidModule, "模块未声明 View"))?;
        if drafts.generation != snapshot.generation {
            drafts.inputs.clear();
            drafts.generation = snapshot.generation;
        }
        fn input_keys(node: &ViewSnapshot, keys: &mut BTreeSet<String>) {
            if node.kind == ViewKind::Input {
                keys.insert(node.key.clone());
            }
            for child in &node.children {
                input_keys(child, keys);
            }
        }
        let mut keys = BTreeSet::new();
        input_keys(node, &mut keys);
        drafts.inputs.retain(|key, _| keys.contains(key));
        let view = self.project_node(node, &snapshot, &mut drafts)?;
        // 业务错误保留可操作的旧界面；公开快照同时保留类型化失败。
        Ok(if let Some(error) = &snapshot.error {
            column(vec![
                view,
                label(error.to_string()).automation_id(format!("{}/error", snapshot.name)),
            ])
        } else {
            view
        })
    ...
    fn project_node(
        ...
        drafts: &mut Drafts,
    ) -> RuntimeResult<ViewNode> {
        let string = |name: &str| -> RuntimeResult<String> {
            node.properties
                .get(name)
                .ok_or_else(|| RuntimeError::new(ErrorKind::InvalidModule, "界面属性缺失"))?
                .as_str()
                .map(str::to_string)
        };
        let mut view = match node.kind {
            ViewKind::Column | ViewKind::Row => {
                let children = node
                    .children
                    .iter()
                    .map(|n| self.project_node(n, snapshot, drafts))
                    .collect::<RuntimeResult<Vec<_>>>()?;
                let mut view = if node.kind == ViewKind::Column {
                    column(children)
                } else {
                    row(children)
                };
                for (name, value) in &node.properties {
                    if let Value::Float(value) = value {
                        if name == "gap" {
                            view = view.gap(*value as f32);
                        }
                        if name == "padding" {
                            view = view.padding(*value as f32);
                        }
                    }
                }
                view
            ...
            ViewKind::Button => {
                let disabled = node
                    ...
                    .get("disabled")
                    .ok_or_else(|| {
                        RuntimeError::new(ErrorKind::InvalidModule, "按钮缺少 disabled")
                    })?
                    .as_bool()?;
            ...
            ViewKind::Input => {
                ...
                let draft = drafts
                    ...
                    .or_insert_with(|| Draft {
                        value: State::new(value.clone()),
                        pending: Arc::new(AtomicU64::new(0)),
                    });
                // 已确认序号之前的快照不得覆盖正在输入的更新草稿。
                let pending_sequence = draft.pending.load(Ordering::Acquire);
                if snapshot.sequence >= pending_sequence
                    && !snapshot.pending_sequences.contains(&pending_sequence)
                    && draft.value.get() != value
                {
