...
use super::FontData;
use crate::graphics::{Color, GradientDirection, Rect, Size, TextLayoutOptions, Point};
use crate::graphics::VAlign;

impl RenderTarget {
    pub fn fill_linear_gradient(
        &mut self, rect: Rect, color_a: Color, color_b: Color, dir: GradientDirection,
    ) {
        if let Some(cr) = self.intersect_clip(&rect) {
            ...
            for py in y0..y1 {
                for px in x0..x1 {
                    ...
                    let local_x = ux - rect.x; let local_y = uy - rect.y;
                    let t = match dir {
                    ...
                    let blended = Self::apply_opacity_with(
                        self.opacity, Self::premul(Color::from_rgba(r, g, b, a)),
                    );
    ...
    pub fn fill_radial_gradient(
        &mut self, cx: f32, cy: f32, inner_r: f32, outer_r: f32,
        inner_color: Color, outer_color: Color,
    ) {
        if outer_r <= 0.0 { return; }
    ...
    pub(crate) fn measure_ttf(font: &FontData, text: &str, opts: &TextLayoutOptions) -> Size {
        if text.is_empty() { return Size::new(0.0, 0.0); }
        let lh = if opts.line_height > 0.0 {
            opts.line_height
        } else {
            font.font.horizontal_line_metrics(font.size)
                .map(|m| m.new_line_size).unwrap_or(font.size * 1.3)
        };
        ...
        if opts.word_wrap {
            let mut line_w = 0.0f32; let mut max_line = 0.0f32; let mut lines = 1u32;
            for ch in text.chars() {
                if ch == '\n' { lines += 1; max_line = max_line.max(line_w); line_w = 0.0; continue; }
                let advance = font.char_advance(ch);
                if line_w + advance > max_w && line_w > 0.0 {
                    lines += 1; max_line = max_line.max(line_w); line_w = advance;
                } else { line_w += advance; }
            }
            max_line = max_line.max(line_w);
            Size::new(max_line.min(max_w), lines as f32 * lh)
        } else {
            let mut w = 0.0f32; let mut lines = 1u32;
            for ch in text.chars() {
                if ch == '\n' { lines += 1; } else { w += font.char_advance(ch); }
            }
    ...
    pub(crate) fn draw_ttf(
        &mut self, font: &FontData, text: &str, pos: Point, color: Color,
        ...
        struct GlyphPos { ch: char, x: f32, y: f32 }
        let mut glyphs: Vec<GlyphPos> = Vec::new();
        let mut cursor_x = 0.0f32; let mut cursor_y = 0.0f32;

        for ch in text.chars() {
            if ch == '\n' { cursor_x = 0.0; cursor_y += lh; continue; }
            let advance = font.char_advance(ch);
            if opts.word_wrap && cursor_x + advance > max_w && cursor_x > 0.0 {
                cursor_x = 0.0; cursor_y += lh;
            }
            glyphs.push(GlyphPos { ch, x: cursor_x, y: cursor_y });
            cursor_x += advance;
        }
        ...
        for gp in &glyphs {
            ...
            for row in 0..metrics.height {
                for col in 0..metrics.width {
                    ...
                    let blended_alpha = (alpha as u32 * cov_u32 / 255).min(255) as u32;
                    let r = ((c >> 0) & 0xFF) as u32 * cov_u32 / 255;
                    let g = ((c >> 8) & 0xFF) as u32 * cov_u32 / 255;
                    let b = ((c >> 16) & 0xFF) as u32 * cov_u32 / 255;
    ...
    pub fn draw_bitmap_text(&mut self, text: &str, pos: Point, color: Color) {
        let c = self.apply_opacity(Self::premul(color));
        let cw = 6i32; let lh = 9i32;
        let ox = pos.x as i32; let oy = pos.y as i32;
        let mut x = 0i32; let mut y = 0i32;
        for ch in text.chars() {
            if ch == '\n' { x = 0; y += lh; continue; }
            let idx = (ch as u8).wrapping_sub(32);
            if idx >= 95 { x += cw; continue; }
    ...
    pub(crate) fn apply_opacity_with(opacity: f32, color: u32) -> u32 {
        if (opacity - 1.0).abs() < 0.001 { return color; }
        let a = ((color >> 24) & 0xFF) as f32 * opacity;
        let r = ((color >> 0) & 0xFF) as f32 * opacity;
        let g = ((color >> 8) & 0xFF) as f32 * opacity;
        let b = ((color >> 16) & 0xFF) as f32 * opacity;
        ((a as u32).min(255) << 24) | ((b as u32).min(255) << 16)
            | ((g as u32).min(255) << 8) | (r as u32).min(255)
    }

    pub fn blit_image(
        &mut self, src_pixels: &[u32], src_w: i32, src_h: i32,
        src: &Rect, dst: &Rect, opacity: f32,
    ) {
        ...
        if sw <= 0 || sh <= 0 { return; }
        let dx = dst.x as i32; let dy = dst.y as i32;
        let dw = dst.w as i32; let dh = dst.h as i32;

        if dw == sw && dh == sh {
            for row in 0..sh {
                for col in 0..sw {
                    let src_idx = ((sy + row) * src_w + (sx + col)) as usize;
                    if src_idx >= src_pixels.len() { continue; }
