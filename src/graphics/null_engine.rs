...
use crate::diag::{info_fn, Error, Errc};
use crate::graphics::{
    BlendMode, Color, DirtyRegion, FontHandle, GradientDirection, GraphicsEngine,
    ImageHandle, Point, Radius, Rect, Size, TextLayoutOptions, Transform,
};
...
impl GraphicsEngine for NullEngine {
    fn initialize(&mut self, _hwnd: *mut std::ffi::c_void, w: i32, h: i32) -> Result<(), Error> {
        self.width = w;
        self.height = h;
    ...
    fn set_blend_mode(&mut self, m: BlendMode) { self.blend_mode = m; }
    ...
    fn load_font(&mut self, _p: &str, _s: f32) -> Result<&mut FontHandle, Error> {
    ...
    fn draw_text(&mut self, _f: &FontHandle, _t: &str, _p: Point, _c: Color, _o: &TextLayoutOptions) {}

    fn load_image(&mut self, _p: &str) -> Result<&mut ImageHandle, Error> {
        Err(Error::new(Errc::NotImplemented, "NullEngine"))
    }
