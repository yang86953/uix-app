...
impl FontService {
    ...
    pub fn new() -> Self {
        Self {
            text_backend: Box::new(
                crate::graphics::software_engine::ab_glyph_backend::AbGlyphBackend::new(),
            ),
            registry: Vec::new(),
    ...
    fn load_family_font(&mut self, family: &str, size: f32) -> Option<()> {
        #[cfg(target_os = "linux")]
        {
            ...
            let path =
                crate::platform::linux::system_info::probe_font_path_via_fc_match(&pattern).or_else(
                    || {
