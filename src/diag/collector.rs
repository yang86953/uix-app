...
impl Collector {
    ...
    pub fn collect(&self, err: Error) -> usize {
        ...
        if inner.config.auto_log || severity >= crate::diag::error::ErrorSeverity::Error {
            let level = match severity {
                crate::diag::error::ErrorSeverity::Info => Level::Info,
                crate::diag::error::ErrorSeverity::Warning => Level::Warn,
                crate::diag::error::ErrorSeverity::Error => Level::Error,
                crate::diag::error::ErrorSeverity::Fatal => Level::Fatal,
            };
            ...
            if severity.should_abort() {
                crate::diag::fatal::dump_crash_report();
                std::process::abort();
            }
        } else {
            drop(inner);
