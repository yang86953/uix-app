...
pub(crate) fn run_vulkan_wsi_production_chain_test() {
    ...
    let mut context = VulkanContext::new(native_surface, logical_width, logical_height)
        .expect("the production Vulkan WSI context must create a swapchain");
    let adapter = context.parity_adapter_diagnostic();
    ...
    let resized = context
        .surface()
        .resize(requested)
        .expect("real Vulkan swapchain resize must succeed");
    ...
    context
        .try_shutdown()
        .expect("the Vulkan WSI context must shut down before its native window");
    window
        .close()
        .expect("the native window must close after graphics shutdown");

    eprintln!(
        "Vulkan WSI production chain verified: {adapter}; path=UI Canvas::render -> Drawing PaintContext/Canvas2D -> shared FramePlan/RHI -> Vulkan Surface/Swapchain acquire/render/present; first={}x{}@{}; rejected=0x{}:{:?},token-unchanged; resized={}x{}@{}; presents=2; shutdown=ok",
