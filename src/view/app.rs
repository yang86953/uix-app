...
use crate::widget::scene::event_loop::run_widget_loop;
...
impl App {
    ...
    pub fn root(mut self, view: impl View) -> Self {
        crate::widget::state::begin_state_capture();
        let node = view.build();
        self.root = Some(node);
        self
    }
    ...
    pub fn run(mut self) {
        ...
        let root_node = match self.root.take() {
            Some(node) => node,
            None => {
                crate::platform::log::error_fn("App::run() 前必须调用 .root() 设置根 View");
                return;
            }
        };
        ...
        let mut platform = match create_platform() {
            Ok(p) => p,
            Err(e) => {
                crate::platform::log::error_fn(format!("create_platform 失败: {:?}", e));
                return;
            }
        };
        ...
        let mut platform_window = match platform.window_manager().create_window(&self.title, w, h) {
            Ok(w) => w,
            Err(e) => {
                crate::platform::log::error_fn(format!("create_window 失败: {:?}", e));
                return;
            }
        };
        ...
        if engine.initialize(w, h).is_err() {
            crate::platform::log::error_fn("SoftwareEngine 初始化失败");
            return;
        }
        ...
        if let Some(root) = tree.root_mut() {
            root.set_frame(crate::platform::Rect::new(0.0, 0.0, w as f32, h as f32));
        }
        tree.layout();
