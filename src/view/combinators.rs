...
use crate::widget::layout::FlexDirection;
use crate::view::{View, ViewNode};

use crate::api::widget::{WidgetCapabilities, WidgetComponent, WidgetLayout, WidgetRender};
use crate::widget::scene::RenderContext;
use crate::widget::state::{drain_pending_state_binds, StatePaintBind};
use crate::widget::{WidgetId, WidgetTree};
use std::any::Any;
use std::sync::Arc;
use crate::render::pipeline::InvalidationQueueHandle;
    ...
    I::Item: View,
{
    ...
    ViewNode::new(
        crate::widget::widgets::Container::new()
    ...
    I::Item: View,
{
    let children: Vec<ViewNode> = children.into_iter().map(|v| v.build()).collect();
    ViewNode::new(
        crate::widget::widgets::Container::new().dir(FlexDirection::Row),
        children,
    )
}
...
pub fn label(text: impl Into<String>) -> ViewNode {
    ViewNode::leaf(crate::widget::widgets::Label::new(text))
}
...
impl WidgetLayout for DynamicLabel {
    fn preferred_size(&self, _engine: Option<&dyn crate::render::traits::GraphicsEngine>) -> Size {
...
impl WidgetRender for DynamicLabel {
    fn render(&self, frame: Rect, ctx: &mut RenderContext, _tree: &WidgetTree) {
        ...
        if !text.is_empty() {
            ctx.draw_text(
                &text,
                crate::platform::Point::new(frame.x, frame.y),
...
pub fn space(height: f32) -> ViewNode {
    ViewNode::leaf(
        crate::widget::widgets::Space::new()
            .size(crate::widget::widgets::SpaceSize::Small)
            .height(height),
    )
}
...
pub struct ButtonBuilder {
    text: String,
    variant: crate::widget::widgets::ButtonVariant,
    danger: bool,
...
impl ButtonBuilder {
    ...
    pub fn primary(mut self) -> Self {
        self.variant = crate::widget::widgets::ButtonVariant::Primary;
        self
    }
...
impl View for ButtonBuilder {
    fn build(self) -> ViewNode {
        let mut btn = crate::widget::widgets::Button::new(self.text).variant(self.variant);
        if self.danger {
            btn = btn.danger(true);
        }
...
pub fn button(text: impl Into<String>) -> ButtonBuilder {
    ButtonBuilder {
        text: text.into(),
        variant: crate::widget::widgets::ButtonVariant::Default,
        danger: false,
        on_click: None,
    }
}
...
impl View for InputBuilder {
    fn build(self) -> ViewNode {
        let mut input = crate::widget::widgets::Input::new(self.placeholder);
