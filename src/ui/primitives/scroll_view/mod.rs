...
use crate::ui::primitives::binding::{capture_dependency, write_if_changed};
use crate::ui::reactive::state::State;
use crate::ui::{
    EventResult, KeyCode, MouseButton, PrimitiveSnapshot, SystemEvent, View, ViewNode, Widget,
    WidgetId, WidgetTree,
};
...
const fn scroll_view_background() -> ColorValue {
    ColorValue::Token("uix.background", crate::draw::Color::white())
}

const fn scrollbar_track_color() -> ColorValue {
    ColorValue::Token(
        "uix.scrollbar.track",
        crate::draw::Color::from_rgba(128, 128, 128, 32),
    )
}
...
const fn scrollbar_thumb_color() -> ColorValue {
    ColorValue::Token(
        "uix.scrollbar.thumb",
        crate::draw::Color::from_rgba(128, 128, 128, 128),
    )
}
...
const fn scrollbar_active_thumb_color() -> ColorValue {
    ColorValue::Token(
        ...
        crate::draw::Color::from_rgba(128, 128, 128, 192),
    )
}
...
widget! {
    ...
    viewport_overflow_axes => (&self) -> (bool, bool) { let direction = self.scroll_direction(); (direction.can_scroll_x(), direction.can_scroll_y()) }

    layout_size_locks => (&self) -> (bool, bool) { self.explicit_size_locks() }
}
...
impl ScrollView {
    ...
    pub(crate) fn snapshot_fields(&self) -> PrimitiveSnapshot {
        PrimitiveSnapshot::ScrollView {
...
fn build_scroll_view_uix_root(kernel: ScrollView) -> ViewNode {
    build_scroll_view_view(kernel, SCROLL_VIEW_VISUAL_REF)
}

const SCROLL_VIEW_VISUAL: ScrollViewVisual = ScrollViewVisual {
    default_width: 300.0,
    default_height: 200.0,
    default_flex_grow: 0.0,
    default_flex_shrink: 1.0,
    default_scrollbar_visible: true,
    default_direction: ScrollDirection::Vertical,
    background: ColorValue::Custom(crate::draw::Color::transparent()),
    scrollbar: ScrollbarVisual {
        thickness: 6.0,
        edge_padding: 2.0,
        thumb_min_extent: 18.0,
        corner_radius: 3.0,
        track_color: scrollbar_track_color(),
        thumb_color: scrollbar_thumb_color(),
        active_thumb_color: scrollbar_active_thumb_color(),
    },
};
const SCROLL_VIEW_VISUAL_REF: &ScrollViewVisual = &SCROLL_VIEW_VISUAL;
