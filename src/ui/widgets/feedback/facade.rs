...
pub fn register_feedback(message: MessageHandle, notification: NotificationHandle) {
    ...
    let mut current = registry().write().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn message() -> MessageFacade {
    MessageFacade {
        // 毒锁恢复：RwLock 中毒时取回内部值。
        handle: registry().read().unwrap_or_else(|poisoned| poisoned.into_inner()).message.clone(),
    }
}
