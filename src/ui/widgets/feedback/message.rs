...
define_widget! {
    ...
    render => (&self, frame: Rect, ctx: &mut RenderContext, _tree: &WidgetTree) {
        ...
        for item in queue.iter() {
            ...
            // 左侧强调色条
            ctx.fill_rect(Rect::new(start_x, y + 4.0, 3.0, 32.0), accent, Some(crate::draw::Radius::uniform(1.5)));
            let icon_y = ctx.visual_center_y(msg_rect, 14.0);
            ctx.draw_text(icon, crate::native::Point::new(start_x + 14.0, icon_y), accent, 14.0);
            // 修复：存储 text_x 避免冲突
            let text_x = start_x + 36.0;
            let content_y = ctx.visual_center_y(msg_rect, 13.0);
            ctx.draw_text(&item.content, crate::native::Point::new(text_x, content_y), text_c, 13.0);
            if item.closable {
                let close_y = ctx.visual_center_y(msg_rect, 12.0);
                ctx.draw_text("✕", crate::native::Point::new(start_x + msg_w - 22.0, close_y), ctx.tokens().color_text_quaternary(), 12.0);
            }
            y += 48.0;
        }
    }
