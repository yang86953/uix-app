...
mod tests {
    ...
    fn justify_aligns_fixed_tracks_as_grid_content() {
        // 两个十像素子项分别显式放入两列。
        let mut first = LayoutChild::new(ComponentId::new(1), Size::new(10.0, 10.0));
        // 首项进入第一格。
        first.grid_cell = Some(0);
        // 构造第二个十像素子项。
        let mut second = LayoutChild::new(ComponentId::new(2), Size::new(10.0, 10.0));
        // 次项进入第二格。
    ...
    fn justify_stretch_expands_auto_tracks() {
        // 构造首个十像素自然宽度子项。
        let mut first = LayoutChild::new(ComponentId::new(3), Size::new(10.0, 10.0));
        // 首项进入第一格。
        first.grid_cell = Some(0);
        // 构造次个十像素自然宽度子项。
        let mut second = LayoutChild::new(ComponentId::new(4), Size::new(10.0, 10.0));
        // 次项进入第二格。
        second.grid_cell = Some(1);
