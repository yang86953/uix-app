...
use crate::native::{KeyCode, Point, Rect, Size};
...
pub use crate::native::ControlSize as ButtonSize;
...
define_widget! {
    ...
    render => (&self, frame: Rect, ctx: &mut RenderContext, _tree: &WidgetTree) {
        ...
        crate::app::bridge::apply_style(ctx, btn_frame, &style);
        ...
        if !self.icon.is_empty() {
            ...
            let saved = *ctx.font();
            if let Some(fh) = crate::ui::widgets::icon::lucide_handle() { ctx.set_font(fh); }
...
impl Button {
    fn compute_style(&self, ctx: &RenderContext) -> Style {
        ...
        s.padding = crate::native::EdgeInsets::new(
    ...
    /// 设置外边距。
    pub fn margin(mut self, m: crate::native::EdgeInsets) -> Self {
        self.style.margin = m;
        self
    }

    /// 设置内边距。
    pub fn padding(mut self, p: crate::native::EdgeInsets) -> Self {
