...
/// DatePicker — 日期选择器（简化版，弹出 Calendar 面板）。
define_widget! {
    ...
    render => (&self, frame: Rect, ctx: &mut RenderContext, _tree: &WidgetTree) {
        ...
        let bc = if self.open { ctx.tokens().color_primary() } else { border };
        ...
        ctx.draw_text("📅", Point::new(frame.x + frame.w - 24.0, frame.y + 7.0), text_sec, 14.0);
        if self.open {
            ctx.draw_text("(日期面板弹出 - 暂未实现完整 Calendar)", Point::new(frame.x, frame.y + 40.0), text_sec, 11.0);
        }
    }
}

impl DatePicker {
    pub fn new() -> Self { Self { placeholder: "选择日期".into(), value: String::new(), open: false } }
...
impl Default for DatePicker { fn default() -> Self { Self::new() } }
