...
use super::geometry::{
    ...
    // 构造首次登记前的有限回退表面。
    autocomplete_fallback_surface,
    // 将绝对弹层转换为组件本地坐标。
    local_autocomplete_popup_rect,
    // 归一化当前逻辑表面与锚点。
    normalize_autocomplete_rect,
    // 解析受当前表面约束的绝对弹层。
    resolve_autocomplete_popup_rect,
};

// 为 AutoComplete 提供共享弹层缓存与实际视口方法。
impl AutoComplete {
    // 解析并缓存当前实际自动完成弹层矩形。
    pub(super) fn remember_popup_rect(
        ...
        // 使用共享解析器生成绝对弹层矩形。
        let absolute = resolve_autocomplete_popup_rect(frame, item_count, surface);
        // 转换为组件事件路径可复用的相对矩形。
        let local = local_autocomplete_popup_rect(frame, absolute);
        // 缓存最终相对弹层矩形。
        ...
        self.popup_anchor_frame
            // 保存归一化后的触发器矩形。
            .set(Some(normalize_autocomplete_rect(frame)));
        // 读取当前呈现周期已累计的绝对弹层脏区。
    ...
    pub(super) fn surface_or_fallback(&self, frame: Rect, item_count: usize) -> Rect {
        ...
        self.surface_rect
            ...
            .get()
            // 首次使用时按当前行数构造有限表面。
            .unwrap_or_else(|| autocomplete_fallback_surface(frame, item_count))
    }
    ...
    pub(super) fn interaction_popup_rect(
        ...
        // 首次登记前构造有限回退表面。
        let surface = autocomplete_fallback_surface(frame, item_count);
        // 解析回退绝对弹层。
        let absolute = resolve_autocomplete_popup_rect(frame, item_count, surface);
        // 只返回本地几何，等待正式登记记录绝对锚点与脏区。
        local_autocomplete_popup_rect(frame, absolute)
    }
    ...
    pub(super) fn damage_popup_rect(
        ...
        // 将当前相对矩形转换到窗口绝对坐标。
        let current = absolute_autocomplete_popup_rect(frame, current);
