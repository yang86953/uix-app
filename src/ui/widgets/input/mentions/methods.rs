...
impl Mentions {
    ...
    pub(super) fn remember_popup_rect(
        ...
        // 使用共享解析器生成绝对弹层矩形。
        let absolute = resolve_mentions_popup_rect(frame, item_count, surface);
        // 转换为组件事件路径可复用的相对矩形。
    ...
    pub(super) fn surface_or_fallback(&self, frame: Rect, item_count: usize) -> Rect {
        ...
        self.surface_rect
            ...
            .get()
            // 首次使用时按当前行数构造有限表面。
            .unwrap_or_else(|| mentions_fallback_surface(frame, item_count))
    }
    ...
    pub(super) fn interaction_popup_rect(
        ...
        // 首次登记前构造有限回退表面。
        let surface = mentions_fallback_surface(frame, item_count);
        // 解析回退绝对弹层。
        let absolute = resolve_mentions_popup_rect(frame, item_count, surface);
