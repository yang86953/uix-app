...
// 引入被扩展的树选择组件与最大视口规格。
use super::{TreeSelect, MAX_DROPDOWN_VIEWPORT_HEIGHT};
// 引入同一组件私有几何模块的解析与转换函数。
use super::geometry::{
    // 将相对弹层转换为窗口绝对坐标。
    absolute_tree_select_popup_rect,
    // 将绝对弹层转换为组件本地坐标。
    local_tree_select_popup_rect,
    // 归一化当前逻辑表面与锚点。
    normalize_tree_select_rect,
    // 解析受当前表面约束的绝对弹层。
    resolve_tree_select_popup_rect,
    // 构造首次登记前的有限回退表面。
    tree_select_fallback_surface,
};

// 为 TreeSelect 提供共享弹层缓存与实际视口方法。
impl TreeSelect {
    // 解析并缓存当前实际树选择弹层矩形。
    pub(super) fn remember_popup_rect(
        ...
        // 缓存最终相对弹层矩形。
        self.dropdown_rect.set(local);
        // 记录缓存对应的至少一行显示账本。
        self.dropdown_row_count.set(item_count.max(1));
        // 记录当前逻辑表面供 dirty、命中和旧入口复用。
        self.surface_rect.set(Some(surface));
        // 记录绝对锚点供行数变化后的事件重算复用。
        self.popup_anchor_frame
            // 保存归一化后的触发器矩形。
            .set(Some(normalize_tree_select_rect(frame)));
        // 读取当前呈现周期已累计的绝对弹层脏区。
        let damage = self.dropdown_damage_rect.get();
        // 只有可见弹层才进入历史脏区。
        if absolute.w > 0.0 && absolute.h > 0.0 {
            // 首次解析直接保存，后续变化合并旧新矩形。
            self.dropdown_damage_rect.set(
                // 已有有效脏区时合并。
    ...
    pub(super) fn interaction_popup_rect(
        ...
        if self.dropdown_row_count.get() == display_row_count {
            // 返回最近解析的最终本地矩形。
            return self.dropdown_rect.get();
        }
        // 真实表面与绝对锚点齐备时按当前行数重新解析。
    ...
    pub(super) fn damage_popup_rect(
        ...
        // 将当前相对矩形转换到窗口绝对坐标。
        let current = absolute_tree_select_popup_rect(frame, current);
        // 读取已经合并方向、尺寸与锚点变化的历史区域。
        let damage = self.dropdown_damage_rect.get();
        // 有效历史区域覆盖当前与此前弹层。
        if damage.w > 0.0 && damage.h > 0.0 {
            // 返回历史合并结果。
    ...
    // 返回滚动与键盘显露应使用的实际弹层高度。
    pub(super) fn effective_dropdown_viewport_height(&self, row_count: usize) -> f32 {
        // 事件路径与绘制共用最终弹层高度。
