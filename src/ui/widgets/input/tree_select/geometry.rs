...
// 引入 UIX 注入的树选择视觉表。
use super::presentation::TreeSelectVisual;

// 使用当前逻辑表面解析树选择弹层最终绝对矩形。
pub(super) fn resolve_tree_select_popup_rect(
    // 接收触发器绝对布局矩形。
    frame: Rect,
    // 接收当前可见行数。
    item_count: usize,
    ...
    surface: Rect,
    // 接收 UIX 声明的弹层视觉尺寸。
    visual: &TreeSelectVisual,
    // 返回限制在表面内的绝对弹层矩形。
) -> Rect {
    // 归一化触发器矩形。
    let frame = normalize_tree_select_rect(frame);
    // 归一化逻辑表面矩形。
    let surface = normalize_tree_select_rect(surface);
    // 计算既有规格要求的自然宽度。
    let natural_width = frame.w.max(visual.layout.min_dropdown_width);
    // 计算至少一行且不超过最大视口的自然高度。
    let natural_height = (item_count.max(1) as f32 * visual.layout.row_height)
        // 限制到既有最大自然视口高度。
        .min(visual.layout.max_dropdown_height);
    // 将弹层宽度限制在当前表面内。
...
pub(super) fn local_tree_select_popup_rect(
    // 接收触发器绝对布局矩形。
    frame: Rect,
    // 接收已经解析的绝对弹层矩形。
    popup: Rect,
...
pub(super) fn absolute_tree_select_popup_rect(
    // 接收触发器绝对布局矩形。
    frame: Rect,
    // 接收相对触发器的弹层矩形。
    popup: Rect,
...
// 计算触发器与弹层共同占用的表面内矩形。
pub(super) fn tree_select_surface_rect(
    // 接收触发器绝对布局矩形。
    frame: Rect,
    // 接收绝对弹层或保守脏区矩形。
    popup: Rect,
    ...
    let frame = normalize_tree_select_rect(frame);
    // 归一化弹层矩形。
    let popup = normalize_tree_select_rect(popup);
    // 归一化逻辑表面矩形。
    let surface = normalize_tree_select_rect(surface);
    // 合并触发器与弹层并裁到表面。
    frame
...
pub(super) fn tree_select_fallback_surface(
    frame: Rect,
    item_count: usize,
    visual: &TreeSelectVisual,
) -> Rect {
    ...
    let frame = normalize_tree_select_rect(frame);
    // 计算既有规格要求的自然宽度。
    let width = frame.w.max(visual.layout.min_dropdown_width);
    // 计算当前行数对应的自然视口高度。
    let height = (item_count.max(1) as f32 * visual.layout.row_height)
        // 限制到既有最大自然视口高度。
        .min(visual.layout.max_dropdown_height);
    // 在触发器上下各预留一份自然弹层空间。
    Rect::new(
...
// 归一化树选择相关矩形。
pub(super) fn normalize_tree_select_rect(rect: Rect) -> Rect {
    // 替换非有限坐标并收敛负尺寸。
    Rect::new(
        // 非有限横坐标回退到原点。
