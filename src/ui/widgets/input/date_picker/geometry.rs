...
pub(super) fn resolve_date_picker_popup_rect(frame: Rect, surface: Rect) -> Rect {
    // 归一化触发器矩形。
    ...
    let place_below = if CALENDAR_PANEL_HEIGHT <= available_below {
        // 下方可以完整容纳自然高度。
        true
    // 检查上方是否可以完整容纳自然高度。
    } else if CALENDAR_PANEL_HEIGHT <= available_above {
        // 上方可以完整容纳时翻转。
        false
    // 两侧都不足时比较可用空间。
    } else {
    ...
    // 将面板高度限制到最终方向的可用空间。
    let height = CALENDAR_PANEL_HEIGHT.min(available_height);
    // 按最终方向计算纵向起点。
...
// 构造首次正式登记前的有限回退表面。
pub(super) fn date_picker_fallback_surface(frame: Rect) -> Rect {
    // 归一化触发器矩形。
    let frame = normalize_date_picker_rect(frame);
    // 计算自然面板宽度。
    ...
    Rect::new(
        ...
        frame.x,
        // 向上预留完整面板和间距。
        frame.y - CALENDAR_PANEL_HEIGHT - POPUP_GAP,
        // 保留自然面板宽度。
        width,
        // 覆盖上下两份面板、两份间距和触发器。
        CALENDAR_PANEL_HEIGHT * 2.0 + POPUP_GAP * 2.0 + frame.h,
    )
}
