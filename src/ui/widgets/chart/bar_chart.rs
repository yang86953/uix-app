...
define_widget! {
    ...
    render => (&self, frame: Rect, ctx: &mut RenderContext, _tree: &WidgetTree) {
        ...
        let br = if self.bar_radius > 0.0 { Some(crate::graphics::Radius::uniform(self.bar_radius)) } else { None };
        ...
        for (i, bar) in self.data.iter().enumerate() {
            ...
            if self.show_value && bh > 10.0 {
                ...
                ctx.draw_text(&s, crate::base::Point::new(bx + (bar_w - sz.w) * 0.5, by - sz.h - 2.0), text_c, 10.0);
            }
            let sz = ctx.measure_text(&bar.label, 10.0);
            ctx.draw_text(&bar.label, crate::base::Point::new(bx + (bar_w - sz.w) * 0.5, baseline + 4.0), label_c, 10.0);
        }
    }
}
