...
use crate::lang::lsp::scanner;
use crate::lang::lsp::session::Session;
use serde_json::{Value, json};
use crate::lang::compiler::semantic_ir::{SemanticNodeInfo, SemanticNodeKind, TypedDeclarationKind};
use crate::lang::compiler::source_graph::SourceId;
use crate::lang::compiler::{CompilerSystem, projection_schema::UiProjectionSchema};

// 生成 textDocument/hover 响应。
pub(crate) fn hover(session: &mut Session, request: &Value) -> Value {
    if super::component_document(session, request) { return super::components::hover(session, request); }
    ...
    if !crate::lang::compiler::components::has_scope() {
        ...
        return match crate::lang::compiler::components::ComponentCatalog::for_project(path) {
            Ok(catalog) => std::sync::Arc::new(catalog).with(|| hover(session, request)),
            Err(_) => Value::Null,
        };
    }
...
fn semantic_hover(
    ir: &crate::lang::compiler::semantic_ir::TypedUiIr,
    source_id: SourceId,
    ...
    match node.kind {
        ...
        SemanticNodeKind::Element => {
            if let Some(component) = crate::lang::compiler::components::declaration(&node.name) { return Some(library_component_markdown(&node.name, &component)); }
...
// 生成组件登记的悬停文档。
fn component_markdown(component: &crate::lang::compiler::projection_schema::ComponentSpec) -> String {
    let capability = component
        .capability
        .map(|capability| format!("\n\n- 需要 capability `{capability}`"))
        .unwrap_or_default();
    let status = match component.status {
        crate::lang::compiler::projection_schema::RegistrationStatus::Available => "可用",
        crate::lang::compiler::projection_schema::RegistrationStatus::Planned => "已登记未开放",
    };
    format!(
        "**{}**（{}组件）\n\n- 状态：{status}\n- 生成入口：`{}`{capability}",
        component.name,
...
// 生成事件载荷的悬停文档。
fn event_markdown(event: &crate::lang::compiler::projection_schema::EventSpec) -> String {
...
fn attribute_markdown(schema: UiProjectionSchema, component: &str, attribute: &str) -> String {
    if let Some(component) = crate::lang::compiler::components::declaration(component) {
        if let Some(property) = component.properties.get(attribute) { return format!("**{attribute}** · {:?}{}\n\n所属单元 `{}`", property.kind, if property.required { " · 必填" } else { "" }, component.unit); }
        if let Some(event) = component.events.get(attribute) { return format!("**{attribute}** 事件\n\n`$event` 字段：{}", event.fields.join(", ")); }
    }
    // Widget 结构属性拥有语言级含义。
    if component == "Widget" {
        match attribute {
...
// 生成顶层声明的悬停文档。
fn declaration_markdown(ir: &crate::lang::compiler::semantic_ir::TypedUiIr, name: &str) -> String {
...
fn declaration_summary(
    ...
    let kind = match declaration.kind {
        ...
        TypedDeclarationKind::Media => "@media 窗口断点块",
    };
    Some(format!("**{name}**\n\n- {kind} 声明"))
}
...
fn enclosing_component(
    ...
    search_attribute_owner(
        &crate::lang::compiler::semantic_ir::TypedNode::Element(ir.root().clone()),
        source_id,
        node,
    )
}
...
fn search_attribute_owner(
    node: &crate::lang::compiler::semantic_ir::TypedNode,
    source_id: SourceId,
    target: &SemanticNodeInfo,
) -> Option<String> {
    let element = match node {
        crate::lang::compiler::semantic_ir::TypedNode::Element(element) => element,
        _ => return None,
    };
...
fn schema_word_hover(text: &str, offset: usize) -> Value {
    ...
    let schema = CompilerSystem::new().schema();
    let markdown = crate::lang::compiler::components::declaration(word).map(|c| library_component_markdown(word, &c))
        .or_else(|| schema.component(word).map(component_markdown))
...
fn library_component_markdown(name: &str, component: &crate::lang::compiler::components::ComponentDeclaration) -> String {
    format!("**{name}**（{}组件）\n\n所属单元 `{}`；属性：{}。", component.category.as_deref().unwrap_or("library"), component.unit, component.properties.keys().cloned().collect::<Vec<_>>().join(", "))
}
