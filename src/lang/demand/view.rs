...
pub(super) fn fingerprint(plan: &BuildPlan) -> Result<String, String> {
    ...
    for package in plan.packages.values() {
        hash_directory(&mut hash, &package.source, &package.source)?;
    }
...
fn ignored(path: &Path) -> bool {
    matches!(
        ...
        Some(".git" | "target" | "node_modules" | ".uix-cache")
    )
}
fn hash_directory(hash: &mut Sha256, root: &Path, directory: &Path) -> Result<(), String> {
    for path in entries(directory)? {
        ...
        if metadata.file_type().is_symlink() {
            return Err(format!(
                "build source contains a symlink; declare its target explicitly: {}",
                path.display()
            ));
        }
        if metadata.is_dir() {
...
pub(super) fn materialize(plan: &BuildPlan) -> Result<PathBuf, String> {
    ...
    if complete.is_file() {
        ...
        if previous.fingerprint == plan.fingerprint {
            return Ok(destination.join("application/Cargo.toml"));
        }
...
fn write_view(plan: &BuildPlan, root: &Path) -> Result<(), String> {
    let active: BTreeMap<_, _> = plan
        .packages
        .iter()
        .filter(|(_, p)| !p.selected.is_empty())
        .collect();
    for (name, package) in &active {
        ...
        descriptor.components.retain(|_, component| {
            component
                ...
                .is_some_and(|unit| package.selected.contains(unit))
        });
        let selected = |unit: &str| {
            unit.strip_prefix(&format!("{name}/"))
                .is_some_and(|unit| package.selected.contains(unit))
        };
        descriptor
            .data_constructors
            .retain(|_, data| selected(&data.unit));
        descriptor
            .value_types
            .retain(|_, value| selected(&value.unit));
        if descriptor
            .theme_factory
            .as_ref()
            .is_some_and(|factory| !selected(&factory.unit))
        {
            descriptor.theme_factory = None;
        }
        descriptor
            .theme_tokens
            .retain(|_, token| selected(&token.unit));
        if descriptor
            .text_factory
            .as_ref()
            .is_some_and(|factory| !selected(&factory.unit))
        ...
        fs::write(
            destination.join("uix-library.json"),
    ...
    for (path, template) in &package.descriptor.generated {
        ...
        while let Some(start) = source.find("{{uix:") {
            let end = source[start..]
                .find("}}")
                .ok_or("unterminated facade insertion point")?
                + start;
        ...
        syn::parse_file(&source)
            .map_err(|e| format!("generated facade {}: {e}", path.display()))?;
        let target = destination.join(path);
        fs::create_dir_all(target.parent().ok_or("facade path has no parent")?)
            .map_err(|e| e.to_string())?;
...
fn copy_application(from: &Path, to: &Path) -> Result<(), String> {
    ...
    for path in entries(from)? {
        ...
        if path.is_dir() {
            copy_application(&path, &destination)?;
        } else {
...
fn rewrite_manifest(
    ...
    let mut selected_features = selected
        .map(|p| p.features.iter().cloned().collect::<Vec<_>>())
        .unwrap_or_default();
    ...
    let removed_dependencies: Vec<_> = original_dependencies
        .difference(&remaining_dependencies)
        .collect();
    let features = table
        .entry("features")
    ...
    if selected.is_some() {
        features.insert("default".into(), toml::Value::Array(Vec::new()));
    }
...
fn rewrite_dependencies(
    ...
    selected: Option<&PackagePlan>,
    _host: bool,
    selected_features: &mut Vec<String>,
) -> Result<(), String> {
    for section in ["dependencies", "build-dependencies"] {
        let host = section == "build-dependencies";
        let Some(dependencies) = table.get_mut(section).and_then(toml::Value::as_table_mut) else {
            continue;
        };
        ...
        for alias in names {
            ...
            if let Some(version) = value.as_str().map(str::to_owned) {
                *value = toml::Value::Table(
                    [(String::from("version"), version.into())]
                        .into_iter()
                        .collect(),
                );
            }
            let spec = value
                .as_table_mut()
                .ok_or("dependency declaration must be a string or table")?;
            let name = spec
                .get("package")
            ...
            if let Some(package) = plan.packages.get(&name) {
                ...
                if package.selected.is_empty() {
                    return Err(format!(
                        "host dependency {name} has no selected package view"
                    ));
                }
                let relative = if selected.is_some() {
                ...
                if !host {
                    spec.insert(
                        "features".into(),
                        toml::Value::Array(vec!["__uix_selected".into()]),
                    );
                    if selected.is_none() {
            ...
            if !host {
                if let Some(package) = selected {
                    ...
                    if !plan.packages.contains_key(&name) {
                        spec.insert("optional".into(), true.into());
                        if package.dependencies.contains(&alias) {
                            selected_features.push(format!("dep:{alias}"));
                        }
                    }
                }
