...
fn default_descriptor() -> PathBuf {
    "uix-library.json".into()
}
...
pub struct LibraryDescriptor {
    #[serde(default)]
    pub theme_tokens: BTreeMap<String, crate::lang::compiler::projection_schema::ThemeTokenSpec>,
    pub version: u32,
    pub package: String,
...
impl BuildPlan {
    ...
    pub fn permits_dynamic(&self, unit: &str) -> bool {
        self.dynamic.contains(unit)
    }

    pub fn require_dynamic(&self, unit: &str) -> Result<(), String> {
        if self.permits_dynamic(unit) {
            Ok(())
        } else {
            Err(format!(
                "dynamic unit {unit:?} was not declared by this build"
            ))
        }
    }
...
pub fn plan(config_path: &Path, target_override: Option<&str>) -> Result<BuildPlan, String> {
    ...
    for input in &config.packages {
        let root = application
            .join(&input.path)
            .canonicalize()
            .map_err(|e| format!("{}: {e}", input.path.display()))?;
    ...
    let mut roots: BTreeSet<String> = config
        .rust
        .into_iter()
        .chain(config.dynamic.iter().cloned())
        .collect();
    ...
    for (name, package) in &packages {
        for (tag, component) in &package.descriptor.components {
            if let Some(old) = tags.insert(tag.clone(), component.unit.clone()) {
                if old != component.unit {
                    return Err(format!(
                        "ambiguous UIX tag {tag}: {old}, {}",
                        component.unit
                    ));
                }
            }
        ...
        for (unit_name, unit) in &package.descriptor.units {
            for tag in &unit.tags {
                if package.descriptor.components.contains_key(tag) {
                    continue;
                }
                let qualified = format!("{name}/{unit_name}");
                if let Some(old) = tags.insert(tag.clone(), qualified.clone()) {
                    if old != qualified {
                        return Err(format!("ambiguous UIX tag {tag}: {old}, {qualified}"));
                    }
                }
            }
        }
    ...
    std::sync::Arc::new(catalog).with(|| -> Result<(), String> {
        for source in &config.sources {
            ...
            if crate::lang::compiler::modules::recognizes_source(&text) {
                let (output, units) = crate::lang::compiler::components::capture_units(|| {
                    crate::lang::compiler::modules::check_file(&source)
                });
                output.map_err(|e| format!("{e:?}"))?;
                roots.extend(units);
            } else {
                let output = crate::lang::compiler::CompilerSystem::new()
                    .compile_file_auto(&source)
                    .map_err(|e| format!("{e:?}"))?;
                roots.extend(output.required_units);
            }
            for tag in source::uix_tags(&source)? {
                if let Some(unit) = tags.get(&tag) {
                    roots.insert(unit.clone());
                }
            }
        }
        Ok(())
    ...
    while let Some(qualified) = pending.pop() {
        ...
        let package = packages
            .get_mut(name)
            .ok_or_else(|| format!("unknown package in unit {qualified}"))?;
        let unit = package
            .descriptor
            .units
            .get(unit_name)
        ...
        if !package.selected.insert(unit_name.to_string()) {
            continue;
        }
    ...
    while let Some(qualified) = pending.pop() {
        if !dynamic.insert(qualified.clone()) {
            continue;
        }
        let (name, unit) = split_unit(&qualified)?;
        let package = &packages[name];
        pending.extend(
            package.descriptor.units[unit]
                .requires
                .iter()
                .map(|v| qualify(name, v)),
        );
        pending.extend(package.descriptor.always.iter().map(|v| qualify(name, v)));
    }
...
fn split_unit(unit: &str) -> Result<(&str, &str), String> {
    let (package, name) = unit
        .split_once('/')
        .ok_or_else(|| format!("unit must be package/name: {unit}"))?;
...
fn validate_descriptor(descriptor: &LibraryDescriptor) -> Result<(), String> {
    if descriptor.package.is_empty() {
        return Err("descriptor package is empty".into());
    }
    for (tag, component) in &descriptor.components {
        ...
        if package != descriptor.package || !descriptor.units.contains_key(unit) {
            return Err(format!(
                "component {tag} has unknown owning unit {}",
                component.unit
            ));
        }
    }
    for (name, owner) in descriptor
        .data_constructors
        .iter()
        .map(|(name, data)| (name.as_str(), data.unit.as_str()))
        .chain(
            descriptor
                .value_types
                .iter()
                .map(|(name, value)| (name.as_str(), value.unit.as_str())),
        )
        .chain(
            descriptor
                .theme_factory
                .iter()
                .map(|factory| ("theme_factory", factory.unit.as_str())),
        )
        .chain(
            descriptor
                .text_factory
                .iter()
                .map(|factory| ("text_factory", factory.unit.as_str())),
        )
    {
        let (package, unit) = split_unit(owner)?;
        if package != descriptor.package || !descriptor.units.contains_key(unit) {
            return Err(format!(
                "declaration {name} has unknown owning unit {owner}"
            ));
        }
    }
    for unit in descriptor.units.values() {
        for path in unit
            .modules
            .iter()
            .chain(&unit.resources)
            .chain(&descriptor.shared)
            .chain(descriptor.generated.keys())
        {
            if path.is_absolute()
            ...
            {
                return Err(format!(
                    "descriptor paths must stay inside the package: {}",
                    path.display()
                ));
            }
        }
    }
    for owner in descriptor
        .symbols
        .values()
        .chain(descriptor.enums.values().flat_map(|m| m.values()))
    {
        if !descriptor.units.contains_key(owner) {
            return Err(format!("unknown symbol owner {owner}"));
        }
    }
    Ok(())
}
