...
impl WidgetExpander {
    ...
    pub(super) fn begin_document_style_scope(
        ...
        if !self
            ...
            .styles
            // 把根包装为节点复用递归检测自动事实伪类。
            .nodes_use_auto_state_facts(&[Node::Element(document.root.clone())])
    ...
    // 解析元素引用的状态变体并降低自动事实与既有 disabled/checked 事实。
    pub(super) fn prepare_pseudo_style(
        ...
        if let Some(property) = styles
            ...
            .iter()
            // 串接 focus 属性。
            .chain(styles.focus.iter())
            // 串接 focus-visible 属性。
            .chain(styles.focus_visible.iter())
            // 串接 active 属性。
            .chain(styles.active.iter())
            // 串接 disabled 属性。
            ...
            .find(|property| property.name == "animation")
        {
            ...
            return Err(Diagnostic::new(
                ...
                // 指向状态变化的正式契约。
                "把 animation 放到基础样式；状态伪类的平滑变化使用 transition",
            ));
        }
        // 自动事实状态需要现有组件私有状态存储来跨 reconcile 保持事实。
        let auto_state_names = [
            (!styles.hover.is_empty()).then_some("hover"),
            (!styles.focus.is_empty()).then_some("focus"),
            (!styles.focus_visible.is_empty()).then_some("focus-visible"),
            (!styles.active.is_empty()).then_some("active"),
        ];
        // 命中任一自动事实状态时要求作用域。
        let hover_scope_name = if auto_state_names.iter().any(Option::is_some) {
            // 自动事实必须位于组件或文档根提供的既有状态作用域中。
        ...
        // :active 复用同一事实做「disabled 节点不进入 active」的按压抑制；
        // 元素没有 disabled 事实时不存在禁用态，无需抑制。
        ...
        } else if !styles.active.is_empty() {
            // 按元素已有 disabled 属性生成可选抑制条件。
            match element.attributes.iter().find(|attribute| attribute.name == "disabled") {
                // 复用现有事实降低抑制条件；差异字段为空。
                Some(attribute) => {
                    let condition =
                        self.lower_pseudo_condition(element, attribute, bindings, false)?;
                    Some((condition, Vec::new()))
                }
                // 无事实元素照常进入 active 层。
                None => None,
            }
        } else {
            // 没有 disabled 变体且没有 :active 抑制需求时不读取属性。
            None
        };
        ...
        Ok(Some(PseudoStyleBinding {
            ...
            // 保存 focus 差异字段。
            focus: styles.focus,
            // 保存 focus-visible 差异字段。
            focus_visible: styles.focus_visible,
            // 保存 active 差异字段。
