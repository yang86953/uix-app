...
pub(crate) fn value_type_tokens(value_type: WidgetValueType) -> TokenStream {
    ...
    match value_type {
        WidgetValueType::Library(name) => {
            let declaration = crate::lang::compiler::components::value_declaration(&name).expect("value declaration remains available during compilation");
            crate::lang::compiler::components::use_unit(&declaration.unit);
            let ty: syn::Type = syn::parse_str(&declaration.rust_type).expect("library type was validated while loading");
            quote! { #ty }
        }
        // String 对应拥有所有权的字符串。
