...
use super::{
    ...
    KeyframesDeclaration, MediaDeclaration, MediaLength, MediaQuery, SourceSpan,
    StyleClassDeclaration, ThemeDeclaration, parse_style_properties,
};
// 引入顶层名称去重集合。
...
pub(crate) fn parse_at_declaration(cursor: &mut Cursor<'_>) -> Result<Declaration, Diagnostic> {
    ...
    let (name, name_span) = cursor.identifier().ok_or_else(|| {
        ...
        Diagnostic::new(
            ...
            "顶层指令缺少名称",
            // 给出支持指令。
            "使用 @import、@export、@theme、@keyframes 或 @media",
        )
    })?;
    // 按指令名分派解析。
    match name.as_str() {
        ...
        // 解析窗口级媒体条件块。
        "media" => parse_media(cursor, start),
        // 其他 @ 指令不属于当前规范。
...
// 已登记可作为媒体阈值的 screen token 名。
const SCREEN_TOKENS: [&str; 6] = [
    "screenXS",
    "screenSM",
    "screenMD",
    "screenLG",
...
// 不允许出现在 @media 块内的结构与动效属性：它们不进入视觉 Style 字段。
const MEDIA_UNSUPPORTED_PROPERTIES: [&str; 12] = [
    "position",
    "top",
    "right",
    "bottom",
...
// 解析窗口级 @media 条件块：`@media (min-width: 800px) and (max-width: #screenXL) { card { … } }`。
fn parse_media(cursor: &mut Cursor<'_>, start: usize) -> Result<Declaration, Diagnostic> {
    // 指令名后允许换行与注释。
    cursor.skip_trivia()?;
    // 条件文本延伸到块起点。
    let condition_start = cursor.offset();
    while !cursor.starts_with("{") {
        if cursor.is_eof() {
            return Err(Diagnostic::new(
                cursor.span_from(start),
                "@media 缺少 { … } 块",
                "使用 @media (min-width: 800px) { card { padding: 24px; } }",
            ));
        }
        cursor.bump();
    }
    let condition_end = cursor.offset();
    let condition_span = cursor.span_between(condition_start, condition_end);
    let query = parse_media_query(
        cursor.source()[condition_start..condition_end].trim(),
        condition_span,
    )?;
    // 消费外围左花括号。
    cursor.consume("{");
    ...
    loop {
        ...
        // 块内复用样式类语法；状态伪类与 extends 在下方明确拒绝。
        let Declaration::StyleClass(class) = super::parse_style_class(cursor)? else {
            unreachable!("样式类解析器只返回 StyleClass");
        };
        if class.state.is_some() {
            return Err(Diagnostic::new(
                class.span,
                "@media 块内暂不支持状态伪类覆盖",
                "把状态样式放在顶层 class:hover { … }，@media 内只覆盖基础类字段",
            ));
        }
        if class.extends.is_some() {
            return Err(Diagnostic::new(
                class.span,
                "@media 块内的类覆盖不能声明 extends",
                "继承关系只在顶层基础类声明，@media 内只写需要按窗口变化的字段",
            ));
        }
        if let Some(property) = class
            .properties
            .iter()
            .find(|property| MEDIA_UNSUPPORTED_PROPERTIES.contains(&property.name.as_str()))
        {
            return Err(Diagnostic::new(
                property.span,
                format!("@media 块内暂不支持 {}", property.name),
                "本期媒体条件只覆盖进入 Style 的视觉与布局字段；定位、变换、光标、选择与动效请放在顶层类",
            ));
        }
        overrides.push(class);
    }
    if overrides.is_empty() {
        return Err(Diagnostic::new(
            cursor.span_from(start),
            "@media 块至少需要一个类覆盖",
            "在块内写 card { padding: 24px; }",
        ));
    }
    Ok(Declaration::Media(MediaDeclaration {
        query,
        overrides,
...
// 解析 `(min-width: …)`、`(max-width: …)` 及其 and 组合。
fn parse_media_query(source: &str, span: SourceSpan) -> Result<MediaQuery, Diagnostic> {
    let usage = "使用 (min-width: 800px)、(max-width: #screenLG) 或两者以 and 组合";
    if source.is_empty() {
        return Err(Diagnostic::new(span, "@media 缺少条件", usage));
    }
    let mut query = MediaQuery {
        min_width: None,
        max_width: None,
    };
    // 依次读取括号条件与 and 连接词；条件之间只允许 and。
    ...
    while !rest.is_empty() {
        if expect_condition {
            let Some(after_open) = rest.strip_prefix('(') else {
                ...
                return Err(Diagnostic::new(
                    span,
                    format!("@media 条件 {word:?} 必须写在圆括号内"),
                    usage,
                ));
            };
            let Some(close) = after_open.find(')') else {
                return Err(Diagnostic::new(span, "@media 条件缺少右括号", usage));
            };
            let inner = &after_open[..close];
            let Some((feature, value)) = inner.split_once(':') else {
                return Err(Diagnostic::new(
                    span,
                    format!("@media 条件 ({inner}) 缺少冒号"),
                    usage,
                ));
            };
            let feature = feature.trim();
            let slot = match feature {
                "min-width" => &mut query.min_width,
                "max-width" => &mut query.max_width,
                other => {
                    return Err(Diagnostic::new(
                        span,
                        format!("@media 暂不支持 {other}，本期只有窗口宽度断点"),
                        usage,
                    ));
                }
            };
            let length = parse_media_length(value.trim(), span, usage)?;
            if slot.is_some() {
                return Err(Diagnostic::new(
                    span,
                    format!("@media 重复声明 {feature}"),
                    usage,
                ));
            }
            *slot = Some(length);
            ...
            if word != "and" {
                return Err(Diagnostic::new(
                    span,
                    format!("@media 条件之间只能用 and 组合，遇到 {word:?}"),
                    usage,
                ));
            }
...
// 解析媒体阈值：非负有限 px 或已登记 screen token。
fn parse_media_length(
    source: &str,
    span: SourceSpan,
    usage: &str,
) -> Result<MediaLength, Diagnostic> {
    if let Some(token) = source.strip_prefix('#') {
        if !SCREEN_TOKENS.contains(&token) {
            return Err(Diagnostic::new(
                span,
                format!("@media 阈值只接受已登记的 screen token，{source} 不在其中"),
                "使用 #screenXS、#screenSM、#screenMD、#screenLG、#screenXL 或 #screenXXL",
            ));
        }
        return Ok(MediaLength::Token(token.to_string()));
    }
    let Some(number) = source.strip_suffix("px") else {
        return Err(Diagnostic::new(
            span,
            format!("@media 阈值 {source:?} 必须是 px 长度或 screen token"),
            usage,
        ));
    };
    let value = number
        ...
        .ok_or_else(|| {
            Diagnostic::new(
                span,
                format!("@media 阈值 {source:?} 不是非负有限 px"),
                usage,
            )
        })?;
    // 千分之一像素量化保持 AST 可比较，同时远低于渲染可感知阈值。
    Ok(MediaLength::Px((value * 1000.0).round() as u32))
}
    ...
    let state = if cursor.consume(":") {
        ...
        let (state, span) = cursor.identifier().ok_or_else(|| {
            ...
            Diagnostic::new(
                cursor.point_span(),
                "样式伪类缺少状态名称",
                "使用 :hover、:focus、:focus-visible、:active、:disabled 或 :checked",
            )
        })?;
        // 把闭合状态名称映射到 AST。
        // :focus-visible 的连字符不在标识符词法内，命中 focus 时定点补消费。
        let state = if state == "focus" && cursor.consume("-visible") {
            "focus-visible".to_string()
        } else {
            state
        };
        Some(match state.as_str() {
            ...
            // 登记焦点状态。
            "focus" => super::StylePseudoState::Focus,
            // 登记键盘焦点可见状态。
            "focus-visible" => super::StylePseudoState::FocusVisible,
            // 登记按压状态。
            "active" => super::StylePseudoState::Active,
            // 登记禁用状态。
