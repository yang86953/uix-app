...
pub(super) fn grid_tracks_field(
    ...
    // 复用与 <Grid columns> 相同的轨道语法，诊断指向样式值。
    let tracks = parse_grid_tracks(&property.value.source, &|message, hint| {
        value_diagnostic(property, message, hint)
    })?;
    // 把字段名转换为卫生标识符。
    let field = Ident::new(field, Span::call_site());
    // 生成轨道向量更新。
...
// 解析空白分隔的轨道列表：px、fr、auto 与有界 minmax(min, max)。
//
// 样式 gridTemplateColumns/Rows 与 <Grid columns/rows> 共用这一支持集合。
pub(super) fn parse_grid_tracks(
    source: &str,
    diagnose: &dyn Fn(String, &str) -> Diagnostic,
) -> Result<Vec<TokenStream>, Diagnostic> {
    let usage = "使用 100px、1fr、auto 或 minmax(240px, 1fr)，并以空格分隔轨道";
    // 保存有序轨道令牌。
    let mut tracks = Vec::new();
    // 按空白切分轨道列表，但保持 minmax(...) 括号内完整。
    for track in split_track_list(source) {
        // auto 映射到自动轨道。
        if track == "auto" {
            tracks.push(quote! { ::uix_app::prelude::GridTrack::Auto });
            continue;
        }
        // minmax(min, max) 映射到有界轨道。
        if let Some(inner) = track
            .strip_prefix("minmax(")
            .and_then(|rest| rest.strip_suffix(')'))
        {
            tracks.push(minmax_track(inner, diagnose)?);
            continue;
        }
        // 识别弹性比例后缀。
        let (number, constructor) = if let Some(number) = track.strip_suffix("fr") {
            ...
            return Err(diagnose(
                format!("Grid 轨道 {track:?} 缺少 px/fr 单位"),
                usage,
            ));
        };
        // 解析有限非负轨道数值。
        let value = track_number(number, track, diagnose)?;
    ...
    if tracks.is_empty() {
        return Err(diagnose(
            "Grid 轨道列表不能为空".to_string(),
            "至少声明一个 100px、1fr、auto 或 minmax(240px, 1fr) 轨道",
        ));
    }
    Ok(tracks)
}
...
fn track_number(
    ...
    let value = number.trim().parse::<f32>().map_err(|_| {
        diagnose(
            format!("Grid 轨道 {track:?} 不是有效数字"),
            "使用 100px、1fr、auto 或 minmax(240px, 1fr)",
        )
    })?;
    if !value.is_finite() || value < 0.0 {
        return Err(diagnose(
            format!("Grid 轨道 {track:?} 必须是有限非负值"),
            "使用非负有限的 px/fr/% 数值",
        ));
    }
...
// 按空白切分轨道列表，括号内的空白不切分。
fn split_track_list(source: &str) -> Vec<&str> {
    ...
    if let Some(begin) = start {
        items.push(&source[begin..]);
    }
    items
}

// 生成有界轨道：下界为 px 或百分比，上界为 px 或 fr；其余组合明确拒绝。
fn minmax_track(
    inner: &str,
    diagnose: &dyn Fn(String, &str) -> Diagnostic,
) -> Result<TokenStream, Diagnostic> {
    let usage = "使用 minmax(240px, 1fr)、minmax(20%, 400px) 等：下界 px/%，上界 px/fr";
    if inner.contains('(') {
        return Err(diagnose("minmax() 不支持嵌套函数".to_string(), usage));
    }
    let parts = inner.split(',').map(str::trim).collect::<Vec<_>>();
    let [min, max] = parts.as_slice() else {
        return Err(diagnose("minmax() 需要两个参数".to_string(), usage));
    };
    let track = format!("minmax({inner})");
    let min = if let Some(number) = min.strip_suffix('%') {
        let value = track_number(number, &track, diagnose)?;
        quote! { ::uix_app::prelude::GridTrackMin::Percent(#value) }
    } else if let Some(number) = min.strip_suffix("px") {
        let value = track_number(number, &track, diagnose)?;
        quote! { ::uix_app::prelude::GridTrackMin::Px(#value) }
    } else {
        return Err(diagnose(
            format!("minmax() 下界 {min:?} 只支持 px 或百分比，不支持 auto/fr"),
            usage,
        ));
    };
    let max = if let Some(number) = max.strip_suffix("fr") {
        let value = track_number(number, &track, diagnose)?;
        quote! { ::uix_app::prelude::GridTrackMax::Fr(#value) }
    } else if let Some(number) = max.strip_suffix("px") {
        let value = track_number(number, &track, diagnose)?;
        quote! { ::uix_app::prelude::GridTrackMax::Px(#value) }
    } else {
        return Err(diagnose(
            format!("minmax() 上界 {max:?} 只支持 px 或 fr，不支持 auto/百分比"),
            usage,
        ));
    };
    Ok(quote! { ::uix_app::prelude::GridTrack::MinMax(#min, #max) })
}
