...
pub(super) fn apply_common_attributes(
    ...
    let shorthands = attributes.iter().filter(|attribute| {
        attribute.name != "direction" && !consumed.contains(&attribute.name.as_str())
            && super::view_shorthand_codegen::CONTAINER_CONSUMED_ATTRIBUTES.contains(&attribute.name.as_str())
    }).cloned().collect::<Vec<_>>();
    view = super::view_shorthand_codegen::apply_container_shorthands(view, &shorthands)?;
    // 先应用非事件属性，避免提前捕获构建器。
    for attribute in attributes {
        ...
        if shorthands.iter().any(|shorthand| shorthand.name == attribute.name) { continue; }
        // 事件在第二轮统一处理。
...
pub(super) fn generate_node_view(node: &Node) -> Result<TokenStream, Diagnostic> {
    ...
    match node {
        ...
        Node::Text(text) => {
            ...
            // 返回文本 View。
            generate_text_node(quote! { #value }, text.span)
        }
        // 直接插值生成动态字符串 label。
        Node::Interpolation(expression) => {
            ...
            generate_text_node(quote! { ::std::string::ToString::to_string(&(#value)) }, expression.span)
        }
...
fn generate_text_node(content: TokenStream, span: SourceSpan) -> Result<TokenStream, Diagnostic> {
    let factory = crate::lang::compiler::components::text_factory().ok_or_else(|| Diagnostic::new(span,
        "the project has no text factory", "Load a library descriptor that declares text_factory"))?;
    crate::lang::compiler::components::use_unit(&factory.unit);
    super::component_codegen::template(&factory.rust, &std::collections::BTreeMap::from([("content".into(), content)]), span)
}
