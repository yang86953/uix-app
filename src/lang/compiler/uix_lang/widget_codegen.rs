...
impl WidgetExpander {
    ...
    pub(super) fn expand_element(
        ...
        for attribute in &mut expanded.attributes {
            ...
            if crate::lang::compiler::components::declaration(&element.name).and_then(|c| c.properties.get(&attribute.name).cloned()).is_some_and(|p| matches!(p.kind, crate::lang::compiler::components::PropertyKind::Identifier)) {
    ...
    fn expand_widget_call(
        ...
        // 预先判断当前组件是否需要自动事实私有状态（hover/focus 等）。
        let uses_hover_style = self.styles.nodes_use_auto_state_facts(&widget.children);
...
pub(super) fn is_state_handle_attribute(element_name: &str, attribute_name: &str) -> bool {
    ...
    crate::lang::compiler::components::declaration(element_name)
        .and_then(|component| component.properties.get(attribute_name).cloned())
        .is_some_and(|property| property.state_handle.is_some() || matches!(property.kind, crate::lang::compiler::components::PropertyKind::State))
        || UI_PROJECTION_SCHEMA.handle_slot(element_name, attribute_name).is_some()
}
