...
// 把 backgroundSize 关键字或一至两个轴尺寸映射为 Style 字段更新。
//
// 闭合取值：`auto`、`cover`、`contain`，或一至两个轴尺寸（`auto`、
// `<n>px`、`<n>%`）；单轴尺寸表示宽度、高度按固有比例推导；百分比参照
// 背景绘制的 border-box。只作用于图片来源，渐变不受影响。
pub(super) fn background_size_field(
    // 接收 map_style 闭包中的卫生 Style 标识符。
    style: &Ident,
    // 接收保留源码跨度的 backgroundSize 属性。
    property: &StyleProperty,
) -> Result<TokenStream, Diagnostic> {
    // 按连续空白切分分量。
    ...
    let value = match parts.as_slice() {
        // 单关键字整体策略。
        [single] => match *single {
            "auto" => quote! { ::uix_app::prelude::BackgroundSize::Auto },
            "cover" => quote! { ::uix_app::prelude::BackgroundSize::Cover },
            "contain" => quote! { ::uix_app::prelude::BackgroundSize::Contain },
            // 单轴尺寸：宽度显式、高度按固有比例推导。
            _ => {
                let width = size_axis(single, property)?;
                quote! {
                    ::uix_app::prelude::BackgroundSize::Explicit(
                        #width,
                        ::uix_app::prelude::BackgroundAxisSize::Auto,
                    )
                }
            }
        },
        // 两轴尺寸：每轴可为 auto、px 或百分比。
        [width_source, height_source] => {
            let width = size_axis(width_source, property)?;
            let height = size_axis(height_source, property)?;
            quote! {
                ::uix_app::prelude::BackgroundSize::Explicit(#width, #height)
            }
        }
        // 其余分量个数与关键字组合都明确拒绝。
        _ => {
            return Err(background_size_diagnostic(
                property,
                "backgroundSize 只支持 auto、cover、contain 或一至两个轴尺寸",
            ));
        }
    };
    Ok(quote! { #style.background_size = #value; })
}

// 解析单个轴尺寸分量；auto 关键字、px 长度与 0%-100% 百分比。
fn size_axis(
    // 接收单个轴尺寸源码。
    source: &str,
    // 接收诊断所属属性。
    ...
    // auto 轴保持固有比例或固有尺寸。
    if source == "auto" {
        return Ok(quote! { ::uix_app::prelude::BackgroundAxisSize::Auto });
    }
    // 百分比映射为零到一的背景盒比例。
    if let Some(number) = source.strip_suffix('%') {
        let value = finite_f32(number, property)?;
        if !(0.0..=100.0).contains(&value) {
            return Err(background_size_diagnostic(
                property,
                "backgroundSize 百分比必须在 0% 到 100% 之间",
            ));
        }
        let value = proc_macro2::Literal::f32_unsuffixed(value / 100.0);
        return Ok(quote! { ::uix_app::prelude::BackgroundAxisSize::Percent(#value) });
    }
    // 固定长度必须显式使用 px 单位。
    let Some(number) = source.strip_suffix("px") else {
        return Err(background_size_diagnostic(
            property,
            "backgroundSize 轴尺寸必须使用 px、百分比或 auto 关键字",
        ));
    };
    let value = finite_f32(number, property)?;
    if value < 0.0 {
        return Err(background_size_diagnostic(
            property,
            "backgroundSize 不接受负尺寸",
        ));
    }
    let value = proc_macro2::Literal::f32_unsuffixed(value);
    Ok(quote! { ::uix_app::prelude::BackgroundAxisSize::Px(#value) })
}

// 构造 backgroundSize 的统一诊断。
fn background_size_diagnostic(property: &StyleProperty, message: &str) -> Diagnostic {
    Diagnostic::new(
        property.value.span,
        message.to_owned(),
