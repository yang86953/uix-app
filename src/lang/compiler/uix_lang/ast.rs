...
pub(crate) struct Element {
    ...
    // 保存逐迭代准备语句引用的组件级声明名：惰性行工厂（VirtualScroll）
    // 闭包外需先克隆遮蔽，避免 move 闭包夺走组件体后续仍要使用的所有权。
    pub(crate) for_iteration_outer_captures: Vec<String>,
...
pub(crate) struct PseudoStyleBinding {
    // 任一自动事实状态存在时保存最近组件作用域名称。
    pub(crate) hover_scope_name: Option<String>,
    // 保存自动事实私有状态子作用域的稳定声明标识。
    pub(crate) declaration_id: u64,
    // 保存当前节点所属最近 For 实例路径。
    pub(crate) instance_path_name: Option<String>,
    // 保存 hover 只声明的差异字段。
