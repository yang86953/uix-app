...
pub enum ChildrenDeclaration {
    ...
    Text,
    TextFunction,
    Views { #[serde(default)] minimum: Option<usize>, #[serde(default)] maximum: Option<usize>, #[serde(default)] tags: Vec<String>, #[serde(default)] item_finish: Option<String> },
    Single { #[serde(default)] optional: bool, #[serde(default)] static_element: bool },
    Fold { initial: String, tags: Vec<String>, #[serde(default)] minimum: Option<usize> },
    Lazy { data_property: String, #[serde(default)] item_property: Option<String>, render: String, render_keyed: String },
    Records { tags: Vec<String>, #[serde(default)] minimum: Option<usize> },
}
...
fn default_constructor_method() -> String { "new".into() }
...
impl ComponentCatalog {
    ...
    pub fn for_project(path: &Path) -> Result<Self, String> {
        ...
        let directory = if absolute.is_dir() { absolute.as_path() } else { absolute.parent().unwrap_or(&absolute) };
        for directory in directory.ancestors() {
            let configuration = directory.join("uix.json");
            if configuration.is_file() {
                #[derive(Deserialize)]
                struct Package { path: std::path::PathBuf, #[serde(default)] descriptor: Option<std::path::PathBuf> }
                #[derive(Deserialize)]
                struct Configuration { #[serde(default)] packages: Vec<Package> }
                let configuration: Configuration = serde_json::from_slice(&std::fs::read(&configuration).map_err(|e| e.to_string())?).map_err(|e| e.to_string())?;
                let mut catalog = Self::new();
                for package in configuration.packages {
                    catalog.read_library(&directory.join(package.path).join(package.descriptor.unwrap_or_else(|| "uix-library.json".into())))?;
                }
                return Ok(catalog);
            }
            let descriptor = directory.join("uix-library.json");
            if descriptor.is_file() {
                let mut catalog = Self::new(); catalog.read_library(&descriptor)?; return Ok(catalog);
            }
        }
        Ok(Self::new())
    }

    pub fn insert(&mut self, name: String, declaration: ComponentDeclaration) -> Result<(), String> {
        if name.is_empty() || declaration.constructor.trim().is_empty() || declaration.unit.is_empty() {
            return Err("component name, unit and constructor must be nonempty".into());
        }
        if self.declarations.contains_key(&name) { return Err(format!("duplicate component declaration {name}")); }
    ...
    pub fn read_library_source(&mut self, source: &str) -> Result<(), String> {
        #[derive(Deserialize)]
        struct Library {
            #[serde(default)] components: BTreeMap<String, ComponentDeclaration>,
            #[serde(default)] theme_factory: Option<ThemeFactory>,
            #[serde(default)] text_factory: Option<ThemeFactory>,
            #[serde(default)] data_constructors: BTreeMap<String, DataDeclaration>,
            #[serde(default)] value_types: BTreeMap<String, ValueDeclaration>,
        }
        let library: Library = serde_json::from_str(source).map_err(|e| e.to_string())?;
        for (name, declaration) in library.data_constructors {
            syn::parse_str::<syn::Path>(&declaration.rust_path).map_err(|e| format!("invalid data constructor path {name}: {e}"))?;
            if self.data.insert(name.clone(), declaration).is_some() { return Err(format!("duplicate data constructor {name}")); }
        }
        for (name, declaration) in library.value_types {
            syn::parse_str::<syn::Type>(&declaration.rust_type).map_err(|e| format!("invalid value type {name}: {e}"))?;
            if self.values.insert(name.clone(), declaration).is_some() { return Err(format!("duplicate value type {name}")); }
        }
        for (name, declaration) in library.components { self.insert(name, declaration)?; }
        if let Some(text) = library.text_factory {
            if self.text.is_some() { return Err("multiple default text factories".into()); }
            self.text = Some(text);
        }
        if let Some(theme) = library.theme_factory {
            if self.theme.is_some() { return Err("multiple default theme factories; select one theme provider in the library catalog".into()); }
            self.theme = Some(theme);
        }
        Ok(())
    }

    pub fn declarations(&self) -> &BTreeMap<String, ComponentDeclaration> { &self.declarations }
    ...
    pub fn with<R>(self: &Arc<Self>, operation: impl FnOnce() -> R) -> R {
        ...
        impl Drop for Restore {
            fn drop(&mut self) { CURRENT.with(|current| { *current.borrow_mut() = self.0.take(); }); }
        }
...
pub(crate) fn names() -> Vec<String> {
    CURRENT.with(|current| current.borrow().as_ref().map(|c| c.declarations.keys().cloned().collect()).unwrap_or_default())
}
...
pub(crate) fn has_scope() -> bool { CURRENT.with(|current| current.borrow().is_some()) }

pub(crate) fn with_project<R>(path: &Path, operation: impl FnOnce() -> R) -> Result<R, super::CompilerDiagnostic> {
    let catalog = ComponentCatalog::for_project(path).map_err(|message| super::source_io_diagnostic(path, std::io::Error::other(message)))?;
...
pub(crate) fn identity() -> u64 {
    ...
    CURRENT.with(|current| {
        ...
        let Some(catalog) = current.as_ref() else { return 0; };
        if catalog.declarations.is_empty() && catalog.theme.is_none() && catalog.text.is_none() && catalog.data.is_empty() && catalog.values.is_empty() { return 0; }
        let mut hash = std::collections::hash_map::DefaultHasher::new();
        if let Ok(bytes) = serde_json::to_vec(&(&catalog.declarations, &catalog.theme, &catalog.text, &catalog.data, &catalog.values)) { hash.write(&bytes); }
        hash.finish()
    })
}
...
pub(crate) fn use_unit(unit: &str) {
    USED_UNITS.with(|scopes| { if let Some(scope) = scopes.borrow_mut().last_mut() { scope.insert(unit.into()); } });
}

pub(crate) fn capture_units<R>(operation: impl FnOnce() -> R) -> (R, std::collections::BTreeSet<String>) {
    struct Scope;
    impl Drop for Scope { fn drop(&mut self) { USED_UNITS.with(|scopes| { scopes.borrow_mut().pop(); }); } }
...
pub(crate) fn query_entries(kind: super::QueryKind) -> Vec<super::QueryEntry> {
    ...
    fn entry(id: String, name: &str, unit: &str, value: &impl Serialize) -> QueryEntry {
        QueryEntry::Library { id, name: name.into(), unit: unit.into(), declaration: serde_json::to_value(value).expect("declaration is serializable") }
    }
    CURRENT.with(|current| {
        let current = current.borrow();
