...
use super::*;
use crate::lang::compiler::source_graph::SourceGraphBuilder;
...
pub(super) fn file(
    ...
    let root = canonical(path).map_err(|e| crate::lang::compiler::source_io_diagnostic(path, e))?;
...
impl Loader<'_> {
    fn load(
        ...
        let name = path.to_str().ok_or_else(|| {
            crate::lang::compiler::source_io_diagnostic(
                path,
        ...
        let source = if let Some(source) = self.overlays.get(path) {
            ...
            let file =
                std::fs::File::open(path).map_err(|e| crate::lang::compiler::source_io_diagnostic(path, e))?;
            let mut source = String::new();
            file.take(1_048_577)
                .read_to_string(&mut source)
                .map_err(|e| crate::lang::compiler::source_io_diagnostic(path, e))?;
            source
        };
