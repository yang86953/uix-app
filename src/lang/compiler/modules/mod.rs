...
pub fn check_file_with_overlays(
    ...
    if !super::components::has_scope() { return super::components::with_project(path, || check_file_with_overlays(path, overlays))?; }
...
pub fn check_inline(source: &str, name: &str) -> Result<ModuleOutput, CompilerDiagnostic> {
    if !super::components::has_scope() { return super::components::with_project(Path::new(name), || check_inline(source, name))?; }
