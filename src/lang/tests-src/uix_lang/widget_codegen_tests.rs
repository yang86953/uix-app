...
fn generates_distinct_scopes_for_multiple_static_widget_calls() {
    ...
    assert_eq!(
        tokens
            .matches(":: uix_app :: ui :: __private :: uix_widget_scope")
            .count(),
        2
    );
...
// 验证六态伪类按统一层序叠加，focus-visible 读取键盘事实，active 携带禁用抑制。
#[test]
fn generates_six_state_layers_in_unified_order_with_focus_visible_fact() {
    // 声明基础类、六个状态差异与同时携带 checked/disabled 事实的复选框。
    let document = parse_document(
        r##"
        stateful { width: 60px; }
        stateful:hover { width: 120px; }
        stateful:focus { padding: 0px; }
        stateful:focus-visible { opacity: 0.9; }
        stateful:active { backgroundColor: #colorPrimaryActive; }
        stateful:checked { borderColor: red; }
        stateful:disabled { opacity: 0.5; }
        <Widget name="Stateful" state="checked: bool = false, disabled: bool = false">
          <Checkbox class="stateful" text="六态" checked={checked} disabled={disabled} />
        </Widget>
        <Stateful />
        "##,
    )
    ...
    let tokens = generate_document_view(&document)
        .expect("六态伪类应生成成功")
        .to_string();
    // 四个自动事实状态必须各自生成节点私有事实读取。
    for name in [
        "__uix_pseudo_hover_current",
        "__uix_pseudo_focus_current",
        "__uix_pseudo_focus_visible_current",
        "__uix_pseudo_active_current",
    ] {
        assert!(tokens.contains(name), "缺少自动事实 {name}");
    }
    // focus-visible 层必须读取进程级键盘焦点可见事实。
    assert!(tokens.contains("uix_keyboard_focus_visible_fact"));
    // 内层自动事实块按包裹深度嵌套：文本顺序 active → focus-visible →
    // focus → hover，证明叠加从 hover 开始、active 在 focus-visible 之后。
    let hover = tokens
        .find("__uix_pseudo_hover_current")
        .expect("应生成 hover 事实");
    let focus = tokens
        .find("__uix_pseudo_focus_current")
        .expect("应生成 focus 事实");
    let focus_visible = tokens
        .find("__uix_pseudo_focus_visible_current")
        .expect("应生成 focus-visible 事实");
    let active = tokens
        .find("__uix_pseudo_active_current")
        .expect("应生成 active 事实");
    assert!(active < focus_visible && focus_visible < focus && focus < hover);
    // active 层选择条件必须含 disabled 事实抑制。
    let guard_start = tokens[active..]
        .find("if __uix_pseudo_active_current")
        .expect("active 层应有选择分支")
        + active;
    let guard = &tokens[guard_start..];
    let guard_end = guard.find('{').expect("active 选择分支应有条件体");
    assert!(
        guard[..guard_end].contains("disabled"),
        "active 选择条件应包含 disabled 抑制：{}",
        &guard[..guard_end]
    );
    // checked 与 disabled 依次包裹在自动事实层之外；外层包裹的内层引用
    // 在文本上先出现，checked 差异（border_color）先于 disabled 差异
    // （opacity 0.5），disabled 保持在最高优先级。
    let checked_border = tokens[active..]
        .find("border_color")
        .expect("应生成 checked 差异")
        + active;
    let disabled_opacity = tokens[checked_border..]
        .find("0.5")
        .expect("应生成 disabled 差异")
        + checked_border;
    assert!(checked_border < disabled_opacity);
}

// 验证 :active 在元素没有 disabled 事实时照常编译，不生成抑制条件。
#[test]
fn active_state_without_disabled_fact_compiles_without_guard() {
    // 容器没有 disabled 属性；:active 不应要求补声明事实。
    let document = parse_document(
        r#"
        pressable { width: 60px; }
        pressable:active { width: 90px; }
        <Widget name="P"><Container class="pressable" automationId="p" /></Widget>
        <P />
        "#,
    )
    .expect("无 disabled 事实的 :active 应解析成功");
    // 生成必须成功且不携带 disabled 抑制。
    let tokens = generate_document_view(&document)
        .expect("无 disabled 事实的 :active 应生成成功")
        .to_string();
    // 按压事实存在。
    assert!(tokens.contains("__uix_pseudo_active_current"));
    // 无事实元素的选择条件使用常量假抑制（等价于不抑制）。
    assert!(tokens.contains("__uix_pseudo_active_current && ! (false)"));
}

// 验证新增自动事实伪类同样拒绝 animation 播放实例。
#[test]
fn rejects_animation_on_focus_and_active_states() {
    // focus 与 active 状态差异里声明 animation 必须给出组合诊断。
    for state in ["focus", "focus-visible", "active"] {
        let document = parse_document(&format!(
            "pressable {{ width: 60px; }} pressable:{state} {{ animation: spin 1s linear infinite; }} <Widget name=\"P\"><Container class=\"pressable\" /></Widget><P />"
        ))
        .expect("结构解析不校验 animation");
        // 生成期必须返回定向诊断。
        let error = generate_document_view(&document)
            .expect_err("状态伪类 animation 必须失败");
        // 诊断必须命中既有 transition 契约文案。
        assert!(
            error.message.contains("状态伪类暂不支持启动 animation"),
            "{state}: {}",
            error.message
        );
    }
...
// 验证 :focus-visible 连字符状态名可以解析并登记。
#[test]
fn parses_focus_visible_pseudo_state_name() {
    // 连字符名称不在标识符词法内，必须由伪类解析定点补消费。
    let document = parse_document(
        r#"
        kbd { width: 60px; }
        kbd:focus-visible { opacity: 0.9; }
        <Widget name="K"><Container class="kbd" /></Widget>
        <K />
        "#,
    )
    .expect(":focus-visible 应解析成功");
    // 生成必须成功并产出 focus-visible 层。
    let tokens = generate_document_view(&document)
        .expect(":focus-visible 应生成成功")
        .to_string();
    // 键盘焦点可见层存在。
    assert!(
        tokens.contains("__uix_pseudo_focus_visible_current")
            && tokens.contains("uix_keyboard_focus_visible_fact")
    );
}
