...
fn generates_core_view_snapshot_in_source_order() {
    ...
    assert_eq!(
        ...
        r#":: uix_app :: prelude :: View :: build (((:: uix_app :: prelude :: row ({ let mut __uix_children = :: std :: vec :: Vec :: < :: uix_app :: prelude :: ViewNode > :: new () ; __uix_children . push (:: uix_app :: prelude :: View :: build (({ let __uix_dynamic_text_capture_0 = :: std :: clone :: Clone :: clone (& (count)) ; :: uix_app :: prelude :: dynamic_label (move || { let mut __uix_text = :: std :: string :: String :: new () ; __uix_text . push_str ("Count: ") ; __uix_text . push_str (& :: std :: string :: ToString :: to_string (& ((__uix_dynamic_text_capture_0) . clone ()))) ; __uix_text }) }) . font_size (:: uix_app :: prelude :: TypographyToken :: Heading2))) ; __uix_children . push (:: uix_app :: prelude :: View :: build ((((:: uix_app :: prelude :: button ("Save")) . primary ()) . disabled (busy)) . on_click_fn (move || { let _ = { (onConfirm) () } ; }))) ; __uix_children . push (:: uix_app :: prelude :: View :: build (:: uix_app :: prelude :: ViewNode :: leaf ((:: uix_app :: prelude :: Icon :: new ("star")) . size (16.0)))) ; __uix_children })) . gap (8.0)) . align (:: uix_app :: prelude :: AlignItems :: Center))"#
    );
}
