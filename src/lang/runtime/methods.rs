//! 纯集合与字符串操作；输出分配前执行规模检查。

use crate::lang::runtime::*;
use crate::lang::runtime::value::type_error;
...
impl Frame<'_, '_> {
    pub fn method(&mut self, value: Value, name: &str, args: Vec<Value>) -> RuntimeResult<Value> {
        ...
        let expected = match name { "replace" => 2, "split" | "join" | "contains" | "push" | "get" | "unwrapOr" => 1, _ => 0 };
        if args.len() != expected { return Err(type_error("方法参数数量不符")); }
        let result = match (value, name) {
            ...
            (Value::String(v), "toUpperCase") => { self.allocation(v.len().saturating_mul(3))?; Value::String(v.to_uppercase()) }
            (Value::String(v), "toLowerCase") => { self.allocation(v.len().saturating_mul(3))?; Value::String(v.to_lowercase()) }
            (Value::String(v), "chars") => {
                self.allocation(v.chars().count().saturating_mul(std::mem::size_of::<Value>() + 4))?;
                Value::Array(v.chars().map(|c| Value::String(c.to_string())).collect())
            }
            (Value::String(v), "words") => {
                self.allocation(v.split_whitespace().count().saturating_mul(std::mem::size_of::<Value>()).saturating_add(v.len()))?;
                Value::Array(v.split_whitespace().map(|v| Value::String(v.to_string())).collect())
            }
            (Value::String(v), "lines") => {
                self.allocation(v.lines().count().saturating_mul(std::mem::size_of::<Value>()).saturating_add(v.len()))?;
                Value::Array(v.lines().map(|v| Value::String(v.to_string())).collect())
            }
            (Value::String(v), "split") => {
                let delimiter = argument(0)?.as_str()?;
                self.allocation(v.split(delimiter).count().saturating_mul(std::mem::size_of::<Value>()).saturating_add(v.len()))?;
                Value::Array(v.split(delimiter).map(|v| Value::String(v.to_string())).collect())
            }
            (Value::String(v), "replace") => {
                let from = argument(0)?.as_str()?;
                let to = argument(1)?.as_str()?;
                self.allocation(v.len().saturating_add(v.matches(from).count().saturating_mul(to.len())))?;
            ...
            (Value::Array(mut v), "push") => { v.push(argument(0)?.clone()); Value::Array(v) }
            (Value::Array(v), "join") => {
                let separator = argument(0)?.as_str()?;
                let strings = v.iter().map(Value::as_str).collect::<RuntimeResult<Vec<_>>>()?;
                let bytes = strings.iter().fold(0usize, |n, s| n.saturating_add(s.len()))
                    .saturating_add(separator.len().saturating_mul(strings.len().saturating_sub(1)));
        ...
        self.checked(result, &Location::default())
    }
}
