...
// Re-export layout enums so crate::widget::style::FlexDirection etc. work
pub use crate::widget::layout::{AlignItems, FlexDirection, JustifyContent};
...
macro_rules! style {
    ...
    (@inner $s:ident margin $v:expr) => { $s.margin = $crate::widget::style::edge_insets_from_expr($v); };
    (@inner $s:ident padding $v:expr) => { $s.padding = $crate::widget::style::edge_insets_from_expr($v); };
    // 元组语法：`border: (RED, 1)` — 使用括号而非 bracket，确保 `$val:expr` 正确捕获为元组
    (@inner $s:ident border $v:expr) => {
        let (c, w): (crate::render::Color, f32) = $v;
    ...
    (@inner $s:ident shadow $v:expr) => {
        let (sc, sb, sox, soy): (crate::render::Color, f32, f32, f32) = $v;
        $s.box_shadow = Some($crate::widget::style::BoxShadowDef::new(sc, sb, sox, soy));
    };
    ...
    ($($key:ident : [$($v:tt),+]),+ $(,)?) => {
        {
            let mut __style = $crate::widget::style::Style::default();
            $(
    ...
    () => {
        $crate::widget::style::Style::default()
    };
}
