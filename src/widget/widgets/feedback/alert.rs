...
pub use crate::platform::StatusLevel as AlertType;
...
define_widget! {
    ...
    render => (&self, frame: Rect, ctx: &mut RenderContext, _tree: &WidgetTree) {
        ...
        ctx.draw_text(&self.message, crate::platform::Point::new(text_x, msg_y), fg, 14.0);
        if !self.description.is_empty() {
            let desc_rect = Rect::new(frame.x, frame.y + frame.h * 0.5, frame.w, frame.h * 0.5);
            let desc_y = ctx.visual_center_y(desc_rect, 12.0);
            ctx.draw_text(&self.description, crate::platform::Point::new(text_x, desc_y), ctx.tokens().color_text_secondary(), 12.0);
        }
        if self.closable {
            let cx = frame.x + frame.w - 18.0;
            let cy = ctx.visual_center_y(frame, 14.0);
            ctx.draw_text("✕", crate::platform::Point::new(cx, cy), ctx.tokens().color_text_quaternary(), 14.0);
        }
    }
}
