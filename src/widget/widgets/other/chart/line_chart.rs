...
define_widget! {
    ...
    render => (&self, frame: Rect, ctx: &mut RenderContext, _tree: &WidgetTree) {
        ...
        let pts: Vec<crate::platform::Point> = self.data.iter().enumerate().map(|(i, d)|
            crate::platform::Point::new(chart_x + i as f32 * step, map_y(d.value))
        ).collect();
        ...
        for (i, d) in self.data.iter().enumerate() {
            ...
            ctx.draw_text(&d.label, crate::platform::Point::new(lx, ly), lbc, 10.0);
        }
    }
}
