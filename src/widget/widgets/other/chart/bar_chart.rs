...
define_widget! {
    ...
    render => (&self, frame: Rect, ctx: &mut RenderContext, _tree: &WidgetTree) {
        ...
        let br = if self.bar_radius > 0.0 { Some(crate::render::Radius::uniform(self.bar_radius)) } else { None };
        ...
        for (i, bar) in self.data.iter().enumerate() {
            ...
            if self.show_value && bh > 10.0 {
                ...
                ctx.draw_text(&s, crate::platform::Point::new(bx + (bar_w - sz.w) * 0.5, vy), text_c, 10.0);
            }
            ...
            ctx.draw_text(&bar.label, crate::platform::Point::new(lx, ly), label_c, 10.0);
        }
    }
}
