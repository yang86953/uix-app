...
use crate::widget::children::WidgetChildren;
...
#[cfg(test)]
use crate::api::widget::{WidgetCapabilities, WidgetEventHandler, WidgetLayout, WidgetRender};
use crate::render::painting::PaintPass;
use crate::platform::{Rect, Size};

/// Scroll direction for a ScrollView.
/// （已统一为 crate::platform::ScrollDirection。）
pub use crate::platform::ScrollDirection;
...
define_widget! {
        ...
        -> Vec<(WidgetId, Rect)>
    {
        ...
        crate::platform::log::debug_fn(format!("[ScrollView] layout_children: view=({:.0},{:.0}) content=({:.0},{:.0}) scroll=({:.0},{:.0})",
            frame.w, frame.h,
...
mod tests {
    ...
    use crate::widget::widgets::{Collapse, CollapsePanel, Container, Space};
    use crate::api::widget::{AlignItems, FlexDirection};
    ...
    impl WidgetRender for FixedWidget {
        fn render(
            &self,
            _frame: Rect,
            _ctx: &mut crate::widget::scene::RenderContext,
            _tree: &WidgetTree,
        ) {
        }
    }
    ...
    fn scrollview_not_handled_for_non_scroll_events() {
        ...
        let result = sv.on_event(&WidgetEvent::MouseDown {
            ...
            mods: crate::platform::KeyMod::NONE,
        });
    ...
    fn scrollview_expand_child_updates_content_bounds() {
        ...
        impl WidgetLayout for GrowWidget {
            fn preferred_size(&self, _: Option<&dyn crate::render::traits::GraphicsEngine>) -> Size {
