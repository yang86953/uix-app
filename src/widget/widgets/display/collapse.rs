...
define_widget! {
        ...
        if let WidgetEvent::MouseDown { pos, .. } = event {
            ...
            for i in 0..panel_count {
                ...
                if pos.y >= cy && pos.y <= cy + header_h {
                    ...
                    crate::platform::log::debug_fn(format!("[Collapse] 面板 \"{}\" 切换 expanded: {} → {}",
    ...
    render => (&self, frame: Rect, ctx: &mut RenderContext, _tree: &WidgetTree) {
        ...
        for p in &self.panels {
            ...
            ctx.draw_text(arrow, crate::platform::Point::new(frame.x + 10.0, arrow_y), text_secondary, 12.0);
            let header_y = ctx.visual_center_y(header_rect, 14.0);
            ctx.draw_text(&p.header, crate::platform::Point::new(frame.x + 28.0, header_y), text_color, 14.0);
            y += 36.0;

            if p.expanded {
                let content_y = y + 8.0;
                ctx.draw_text(&p.content, crate::platform::Point::new(frame.x + 16.0, content_y), text_secondary, 12.0);
