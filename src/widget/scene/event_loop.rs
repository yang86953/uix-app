...
use crate::render::pipeline::{FrameRenderInput, FrameRenderer, InvalidationSource, RenderMetrics};
...
use crate::platform::{Platform, PlatformWindow, Point, Rect};
    ...
    F: Fn(&mut WidgetTree, &mut dyn GraphicsEngine, &mut dyn Platform),
{
    ...
    {
        let c: &mut dyn crate::platform::IClipboard = platform.clipboard();
        let wide: *mut dyn crate::platform::IClipboard = c;
        ...
        match outcome {
            RenderOutcome::Present(damage) => {
                ...
                if engine_capabilities.uses_external_presenter() {
                    ...
                    {
                        crate::platform::log::error_fn(format!(
...
mod tests {
    use super::*;
    use crate::widget::widgets::container::Container;
    use std::cell::Cell;
    use crate::render::NullEngine;
