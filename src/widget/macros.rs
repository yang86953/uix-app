...
macro_rules! wc_upcast {
    ($T:ty; WidgetRender) => {
        fn as_render(&self) -> Option<&dyn $crate::api::widget::WidgetRender> {
            Some(self)
        }
        fn as_render_mut(&mut self) -> Option<&mut dyn $crate::api::widget::WidgetRender> {
            Some(self)
        }
    };
    ($T:ty; WidgetEventHandler) => {
        fn as_event(&self) -> Option<&dyn $crate::api::widget::WidgetEventHandler> {
            Some(self)
        }
        fn as_event_mut(&mut self) -> Option<&mut dyn $crate::api::widget::WidgetEventHandler> {
            Some(self)
        }
    };
    ($T:ty; WidgetLifecycle) => {
        fn as_lifecycle(&self) -> Option<&dyn $crate::api::widget::WidgetLifecycle> {
            Some(self)
        }
        fn as_lifecycle_mut(&mut self) -> Option<&mut dyn $crate::api::widget::WidgetLifecycle> {
            Some(self)
        }
    };
    ($T:ty; WidgetLayout) => {
        fn as_layout(&self) -> Option<&dyn $crate::api::widget::WidgetLayout> {
            Some(self)
        }
    };
}
...
macro_rules! define_widget {
    ...
    (
        ...
        impl $crate::api::widget::WidgetComponent for $name {
            fn as_any(&self) -> &dyn std::any::Any { self }
            fn as_any_mut(&mut self) -> &mut dyn std::any::Any { self }
            fn capabilities(&self) -> $crate::api::widget::WidgetCapabilities {
                let mut c = $crate::api::widget::WidgetCapabilities::new();
                $(
                    match stringify!($method) {
                        "preferred_size" | "flex_grow" | "flex_shrink" | "layout_children" | "build" =>
                            c.insert($crate::api::widget::WidgetCapabilities::LAYOUT),
                        "render" | "dirty_rect" | "children_clip" | "draw_margin" =>
                            c.insert($crate::api::widget::WidgetCapabilities::RENDER),
                        "on_event" | "needs_continuous_update" | "scroll_delta" | "scroll_delta_for_dirty" | "viewport_scroll_offset" | "hit_test_frame" =>
                            c.insert($crate::api::widget::WidgetCapabilities::EVENT),
                        "on_init" | "on_mount" | "on_unmount" | "on_update" =>
                            c.insert($crate::api::widget::WidgetCapabilities::LIFECYCLE),
                        _ => {}
                    }
                )*
                c
...
macro_rules! __define_widget_grouped_impl {
    ...
    (WidgetLayout, $T:ty, [$($allowed:ident)*], [$(($method:ident, ($($p:tt)*) $(-> $ret:ty)? $body:block))*]) => {
        impl $crate::api::widget::WidgetLayout for $T {
            $(
    ...
    (WidgetRender, $T:ty, [$($allowed:ident)*], [$(($method:ident, ($($p:tt)*) $(-> $ret:ty)? $body:block))*]) => {
        impl $crate::api::widget::WidgetRender for $T {
            $(
    ...
    (WidgetEventHandler, $T:ty, [$($allowed:ident)*], [$(($method:ident, ($($p:tt)*) $(-> $ret:ty)? $body:block))*]) => {
        impl $crate::api::widget::WidgetEventHandler for $T {
            $(
    ...
    (WidgetLifecycle, $T:ty, [$($allowed:ident)*], [$(($method:ident, ($($p:tt)*) $(-> $ret:ty)? $body:block))*]) => {
        impl $crate::api::widget::WidgetLifecycle for $T {
            $(
