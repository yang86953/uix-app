...
pub use ui_project::{UiEvent, UiEventPayload, UiEventSender, UiUpdate};
...
pub struct ExtensionHost {
    ...
    ui_decoder: Option<Box<dyn Fn(&ExtensionValue) -> Result<UiNode, ExtensionError> + Send + Sync>>,
...
impl ExtensionHost {
    ...
    /// Registers a library-owned declaration decoder. Strict builds require its
    /// compilation unit in the application's dynamic allowlist.
    pub fn with_ui_decoder(
        mut self,
        unit: &str,
        decode: impl Fn(&ExtensionValue) -> Result<UiNode, ExtensionError> + Send + Sync + 'static,
    ) -> Result<Self, ExtensionError> {
        crate::build_policy::require_dynamic(unit).map_err(ExtensionError::CapabilityDenied)?;
        self.ui_decoder = Some(Box::new(decode));
        Ok(self)
    }

    /// 冻结包、解析入口、候选求值与暂存注册；不发布任何注册。
    pub fn prepare(&self, package: &ExtensionPackage) -> Result<PreparedExtension, ExtensionError> {
        crate::build_policy::require_dynamic("uix-app/extensions").map_err(ExtensionError::CapabilityDenied)?;
