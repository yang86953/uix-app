    ...
    fn drain_side_effects(&mut self, extension: &str) {
        loop {
            ...
            for (mount, declaration) in submissions {
                ...
                *revision += 1;
                let decoded = bounded_ui_value(&declaration).and_then(|value| {
                    let decode = self.host.ui_decoder.as_ref().ok_or_else(|| super::ExtensionError::Argument("no UI declaration decoder was registered".into()))?;
                    decode(&value)
                });
...
// Bound traversal before converting engine-owned graph values into portable data.
// This also rejects cyclic pairs/vectors without following them indefinitely.
fn bounded_ui_value(value: &super::engine::Value) -> Result<super::ExtensionValue, super::ExtensionError> {
    use super::engine::Value;
    let mut pending = vec![(value.clone(), 0usize)];
    let mut visited = 0usize;
    while let Some((value, depth)) = pending.pop() {
        visited += 1;
        if visited > 65_536 || depth > 128 {
            return Err(super::ExtensionError::Quota("UI declaration exceeds traversal budget".into()));
        }
        match value {
            Value::Pair(pair) => {
                let pair = pair.borrow();
                pending.push((pair.cdr.clone(), depth));
                pending.push((pair.car.clone(), depth + 1));
            }
            Value::Vector(items) => {
                let items = items.borrow();
                if items.len() > 65_536 - visited { return Err(super::ExtensionError::Quota("UI declaration exceeds traversal budget".into())); }
                pending.extend(items.iter().cloned().map(|item| (item, depth + 1)));
            }
            _ => {}
        }
    }
    super::extension_value_from_engine(value)
}
