...
impl RasterRenderer {
    ...
    pub fn stroke_path(
        ...
        let c = self.apply_opa(Self::premul(color));
        let stroked = crate::render::stroker::stroke_path(path, opts);
        if stroked.is_empty() {
            return;
        }
        let polys = crate::render::flattener::flatten(stroked.segments(), 0.25);
        let mut global_edges = Vec::new();
        let mut active_edges = Vec::new();
        let clip = self.clip_rect;
        crate::render::rasterizer::polygon::fill_polygons(
            &polys,
            pixels,
            surface_w,
            surface_h,
