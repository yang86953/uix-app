...
use crate::platform::{Error, IGraphicsContext};

use crate::render::backend::{
    create_backend, BackendCapabilities, BackendKind, CpuBackend, GpuBackend, NullBackend,
    RenderBackend,
};
use crate::render::engine::RenderOutcome;
use crate::render::traits::{Canvas2D, GraphicsCapabilities, UpdateStrategy};
...
impl RenderSession {
    ...
    pub fn resize(&mut self, width: i32, height: i32) {
        ...
        if self.backend.resize(width, height).is_err() {
            crate::platform::log::error_fn("RenderSession::resize 失败");
        }
    }
    ...
    pub fn begin_frame(&mut self, strategy: UpdateStrategy) -> RenderOutcome {
        if self.force_full_frame {
            ...
            return crate::render::pipeline::frame::begin_frame(
        ...
        let caps = self.backend.capabilities();
        crate::render::pipeline::frame::begin_frame(
            strategy,
            self.backend.surface(),
            self.width,
            self.height,
    ...
    pub fn end_frame(&mut self) -> RenderOutcome {
        crate::render::pipeline::frame::end_frame(self.backend.surface())
    }
