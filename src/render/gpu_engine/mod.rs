...
use crate::render::backend::{DamageRegion, GpuBackend, RenderBackend};
use crate::render::engine::RenderOutcome;
use crate::render::pipeline::RenderSession;
use crate::render::traits::{Canvas2D, GraphicsCapabilities, GraphicsEngine, UpdateStrategy};
use crate::platform::Error;
use crate::platform::IGraphicsContext;
    ...
    fn end_frame(&mut self) -> RenderOutcome {
        ...
        if let Some(gpu) = self.session.gpu_backend_mut() {
            if gpu.present(&DamageRegion::full()).is_err() {
                crate::platform::log::error_fn("GpuEngine present 失败");
            }
        }
        outcome
    }
