...
use crate::render::text_backend::{self as tb, GlyphRaster, TextLayoutOptions};
...
impl FontService {
    ...
    pub fn new() -> Self {
        Self {
            text_backend: Box::new(crate::render::text_backends::ab_glyph::AbGlyphBackend::new()),
            registry: Vec::new(),
    ...
    pub fn load_default_system_font(
        &mut self,
        size: f32,
        system_info: &dyn crate::platform::ISystemInfo,
    ) {
            ...
            if let Some(path) = fallback_paths.first() {
                if let Ok(data) = std::fs::read(path) {
                    if self.load_raw_font(data, size).is_some() {
                        crate::platform::log::info_fn(format!(
        ...
        crate::platform::log::info_fn("No primary font found via platform, scanning for fallback...");
        ...
        crate::platform::log::info_fn("No primary font found, using bitmap fallback");
    }

    /// 加载 CJK 回退字体（通过平台层探测）。
    /// 如果主字体本身已经包含中文字形则跳过，避免重复加载。
    fn load_cjk_fallback(&mut self, size: f32, system_info: &dyn crate::platform::ISystemInfo) {
        let cjk_test = ['中', '国', '文'];
        ...
        match cjk_path {
            ...
            None => {
                crate::platform::log::info_fn("No CJK fallback font found via platform");
            }
        }
    }
        ...
        if let Some(p) = system_info.probe_family_font_path(family) {
            if let Ok(data) = std::fs::read(&p) {
                if self.load_raw_font(data, size).is_some() {
                    ...
                    crate::platform::log::info_fn(format!("Loaded family font '{}': {}", family, p));
                    return Some(());
                }
            }
        }
