...
impl DisplayList {
    ...
    // 测试目标保留绘制操作只读观测入口，供 display-list 语义测试与
    // test-harness 整树录制观察按需调用。
        ...
        for op in self.ops.iter() {
            match op {
                ...
                PaintOp::FillLinearGradient {
                    ...
                    Some(radius) => {
                        ctx.fill_linear_gradient_rounded(*rect, *color_a, *color_b, *dir, *radius)
                    }
                    None => ctx.fill_linear_gradient(*rect, *color_a, *color_b, *dir),
                },
                PaintOp::FillRadialGradient {
                    ...
                    clip_rect,
                    radius,
                } => match (clip_rect, radius) {
                    (Some(clip_rect), Some(radius)) => ctx.fill_radial_gradient_rounded(
                        *cx,
                        *cy,
                        *inner_r,
                        *outer_r,
        ...
        for op in self.ops.iter() {
            match op {
                ...
                PaintOp::FillLinearGradient {
                    ...
                    None => canvas.fill_linear_gradient(*rect, *color_a, *color_b, *dir),
                },
                PaintOp::FillRadialGradient {
                    cx,
                    cy,
                    ...
                    (Some(clip_rect), Some(radius)) => canvas.fill_radial_gradient_rounded(
                        *cx,
                        *cy,
                        *inner_r,
                        *outer_r,
...
#[cfg(test)]
#[path = "../../../tests-src/draw/painting/display_list_tests.rs"]
mod tests;
