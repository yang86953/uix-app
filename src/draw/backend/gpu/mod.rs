...
/// Native Canvas2D. GPU backends choose strict-native or ordered hybrid
/// execution when constructing the backend-owned canvas.
...
impl NativeGpuCanvas2D {
    ...
    fn queue_ellipse_mesh(&mut self, rect: Rect, color: Color) {
        if !rect.x.is_finite()
            || !rect.y.is_finite()
            || !rect.w.is_finite()
            || !rect.h.is_finite()
