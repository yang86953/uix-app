...
//!
//! 使用 `uix_platform::test_harness::FakePlatform` 验证 UI 组件与平台服务的交互。
//!
//! 运行方式：
//! ```bash
...
fn clipboard_with_fake_platform() {
    ...
    uix_ui::clipboard::copy_to_clipboard("hello from ui");
...
fn clipboard_multiple_writes() {
    ...
    uix_ui::clipboard::copy_to_clipboard("first");
    assert_eq!(pf.clipboard.text(), "first");

    uix_ui::clipboard::copy_to_clipboard("second");
...
fn clipboard_has_text_after_copy() {
    ...
    uix_ui::clipboard::copy_to_clipboard("now has text");
    assert!(pf.clipboard.has_text());
}
...
fn clipboard_handler_replacement() {
    ...
    uix_ui::clipboard::copy_to_clipboard("before re-inject");
    assert_eq!(pf.clipboard.text(), "before re-inject");

    // 重新注入（模拟窗口重建等场景）
    inject_clipboard_handler(&mut pf);

    uix_ui::clipboard::copy_to_clipboard("after re-inject");
...
fn key_event_injection() {
    use uix_platform::{KeyCode, KeyMod};
